...
- Prod infra = **Cloudflare**: 4 portals on Workers (OpenNext), services on Containers. The self-hosted Coolify box is **RETIRED** — never deploy, SSH, or point anything at it, whatever older docs say.
...
- **CI gates exist but PR-time coverage is narrow** (`.github/workflows/`): `ai-quality-gate.yml` and `policy-quality.yml` trigger on `pull_request`, running policy checks, an affected build/type/lint/test pass, and (for iOS-touching PRs) advisory iOS gates. `integration.yml` (DB reset, SQL tests, edge-function tests, portal e2e) is `workflow_dispatch`/`workflow_call`/nightly-cron only — it does not run on PRs. `deploy-production.yml` is `workflow_dispatch`-only — merging to main does not deploy. Local verification is still required for full coverage (**patina-verification**).
