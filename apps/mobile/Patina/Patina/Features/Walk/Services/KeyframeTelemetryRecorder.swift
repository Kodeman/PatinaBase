...
final class KeyframeTelemetryRecorder: CaptureFrameSink {
    ...
    func capture(frame: ARFrame, timestampSeconds: TimeInterval) {
        ...
        // No encoder behind this lane — release the slot in the same turn so the
        // in-flight guard cannot wedge the gate closed. See the file header.
        if case .fire = decision { sequencer.noteEncodeFinished() }
    }
