...
// MARK: - Studio identity (Wave 6 / D2)

/// The studio brand line under the project header: a small logo (or a
/// monogram fallback when there's no logo) plus the studio/business name,
/// resolved via `StudioIdentityService`. Fetches independently of the rest
/// of the header on its own `.task` — renders nothing while resolving or
/// when the resolver has no brand to show (e.g. `logoUrl` null, or a solo
/// designer with only a personal name), so it never blocks or jarringly
/// reflows the surrounding content.
private struct StudioIdentityLine: View {
    let projectId: String
    ...
    private func monogram(for name: String) -> some View {
        ZStack {
            ...
            Text(Self.initials(for: name))
                // Deliberately fixed-size, same rationale as the profile
                // monogram in DailyGreetingHeader: a glyph decoration inside
                // a fixed 20pt avatar circle, not running text — scaling it
                // with Dynamic Type would overflow the circle.
                .font(.custom("PlayfairDisplay-Medium", size: 9))
