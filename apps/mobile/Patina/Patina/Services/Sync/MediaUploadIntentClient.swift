...
/// rather than a singleton reaching for `SupabaseClientManager`: the whole point
/// of this lane is that the leg is exercisable without a signed-in app, and the
/// primary upload path must not gain a new global.
public struct MediaUploadIntentClient: Sendable {
    ...
    /// and bundle archives that are hundreds of megabytes, and it runs on the
