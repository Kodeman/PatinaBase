...
public final class PostHogService {
    ...
    /// Whether a flag read here can be trusted at all. `false` means
    /// analytics is off (no API key) or PostHog was never set up, in which
    /// case every key answers `false` and always will.
    ///
    /// posthog-ios persists its last flag payload
    /// (`PostHogRemoteConfig.setCachedFeatureFlags` →
    /// `storage.setDictionary(forKey: .enabledFeatureFlags)`) and reads it
    /// back on the first access after `setup()`
    /// (`getCachedFeatureFlags()`, `PostHogRemoteConfig.swift:568-573`), so
    /// `isFeatureEnabled` answers from the previous session's payload
