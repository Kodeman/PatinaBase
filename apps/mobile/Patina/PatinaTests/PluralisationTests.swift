...
//
//  `ProfileView.swift` is L1-C's, so that site pin is wrapped and its read
//  is hoisted (`RL1E2-15`). The claim-sheet half is L1-A's and is pinned in
//  `GuestPromiseTests`.
//
...
struct PluralisationTests {
    ...
    /// `RL1E2-10`: `C-30` word for word, on a second screen — the board row
    /// printed `"1 items"` in the visible label while its accessibility
    /// label inflected correctly, and used the noun `C5-09` retires into the
    /// bargain. `Features/Collections/Views/**` has no W1 owner
    /// (steward.md §5.1's residue row), so this one is L1-E's.
    @Test("the board row inflects its visible count and names the piece")
    func boardRowInflectsItsVisibleCount() throws {
        let source = try SourcePin.read("Patina/Features/Collections/Views/CollectionsView.swift")
        #expect(!source.contains("\\(board.itemCount) items"))
        #expect(source.contains("\\(board.itemCount) piece\\(board.itemCount == 1 ? \"\" : \"s\")"))
    }
}
