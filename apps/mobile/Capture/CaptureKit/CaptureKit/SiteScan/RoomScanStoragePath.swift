...
//  RoomScanSyncService.uploadUSDZ), with a per-scan segment added after the
//  RLS-significant room segment:
//  `{artifactType}/{userId}/{roomId}/{scanId}/{filename}`.
//
...
//  object is `photos/{uid}/{roomId}/{scanId}/{filename}` exactly like
//  usdz/captured_room. The scan segment prevents two scans of one room from
...
//                                     scan: 33333333-3333-3333-3333-333333333333,
//                                     filename: "scan.usdz")
//      == "usdz/11111111-1111-1111-1111-111111111111/22222222-2222-2222-2222-222222222222/33333333-3333-3333-3333-333333333333/scan.usdz"
//         segments → [1]=usdz  [2]=<userId>  [3]=<roomId>  [4]=<scanId>  [5]=scan.usdz
//         ✓ RLS [2]=user, [3]=room
...
public enum RoomScanStoragePath {
    /// `{folder}/{userId}/{roomId}/{scanId}/{filename}`, with every UUID segment
    /// lowercased to match the canonical text form Postgres compares against.
    /// The first three segments remain the storage-RLS contract; `scanId` makes
    /// repeated scans assigned to one room collision-free.
    ...
    public static func object(
        ...
        "\(folder)/\(userID.uuidString.lowercased())/"
            + "\(roomID.uuidString.lowercased())/"
            + "\(scanID.uuidString.lowercased())/\(filename)"
    }
