//
//  CaptureContrast.swift
//  CaptureTests
//
//  The instrument the Field contrast pins measure with (SQ-225 / W1A-06).
//  Modelled directly on `PatinaTests/PatinaContrast.swift`: a Patina/Capture
//  semantic token is a `Color` built over a trait-aware `UIColor` provider
//  (or, for the fixed tokens this lane cares about, a plain sRGB literal), so
//  its value is a function of the appearance it is resolved in. Reading the
//  hex literal out of the source would measure the wrong thing. This resolves
//  the real token against a real `UITraitCollection` and computes WCAG 2.x
//  relative luminance from the resolved sRGB components.
//
...
enum CaptureContrast {
    ...
    /// appearances — the definition of "adaptive" the two W1A-06 findings
    /// turned on (a dynamic ink drawn on a fixed fill).
