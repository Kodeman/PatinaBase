...
- **D4 — zero shadows.** No `box-shadow`, no `drop-shadow`, no Tailwind `shadow-*` in this app. Add the stylelint/lint rule in your first PR and make it CI-blocking. Object depth = value contrast + flat stacked edges + tab (recipes in spec §10). *(Scope during phase-in: see DECISIONS.md O4.)* R126 relaxed this to exactly one token, `--elevation-sheet`, at exactly three sites — margin chips, the open ledger sheet, the studio drawer — enforced by `lib/document/__tests__/shadow-gate.test.ts` at the CSS level. **The portal-polish program (R139–R142, I153) adopts no depth at all**: no new site, no new token, and the eslint and CSS shadow gates both stay.
...
- **Typography-first.** Hierarchy via Playfair/Inter/DM Mono weight, size, and color — not cards-within-cards, not tab bars. Strata Mark rules as section devices. Use the repo's existing brand token source; do not redefine tokens locally. The type and rhythm contract is **`docs/design/house-sheet/SPEC.md`** (I153, PP-5) — seven type steps plus the 15px `.t-money` step, a 24px module, three radii, three paper stocks, state pigments only, wrap never truncate — applied surface by surface as each is touched. No new hex literal outside that sheet.
...
Leah opens `/desk` on a real Tuesday and sees her actual two-to-four folders with truthful need lines. She picks up a real project, resolves a real decision, and watches the stamp, margin, and Desk update from one act. She puts it down and adjusts the time log up because she'd been sketching. At no point does she see a shadow, a zone, a badge, or a dashboard; the only filled control she ever sees is a terminal act where money moves or a paper is signed. If any slice can't produce that experience against real data, the slice isn't done.
