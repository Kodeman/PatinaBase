...
test("task 5 — search the prior job, tick four, one confirm", async ({
  ...
  await expect(
    page
      .getByText(/Worked 1 prior project, Lindqvist kitchen, closed 2025\./)
      .first(),
  ).toBeVisible();
  ...
  await expect(consequence).toContainText(
    `Adds four seats to the ${projectName}.`,
  );
  await expect(consequence).toContainText(
    "Pete Rusk arrives opted out of texting.",
  );
  ...
  expect((seats ?? []).map((s) => s.studio_contact_id).sort()).toEqual(
    [CARDS.dana, CARDS.pete, CARDS.ingrid, CARDS.claire].sort(),
  );
