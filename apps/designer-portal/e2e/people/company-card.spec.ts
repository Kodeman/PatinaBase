...
async function firstFirm() {
  ...
  return (data ?? []).find(
    (c) => !['lender', 'inspector'].includes(String(c.company_kind ?? c.contact_kind)),
  );
}
...
test.describe('the company card', () => {
  test('Chase the renewal files a draft awaiting review, and sends nothing', async ({
    ...
    await adminDb.from('agent_tasks').delete().eq('task_type', 'compliance_chase').eq('entity_id', companyId);
    ...
    await expect(
      page.getByText('Filed for your review. Nothing is sent until you send it.'),
    ).toBeVisible();
