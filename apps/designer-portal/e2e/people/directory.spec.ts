...
test.describe("the Directory", () => {
  test("the head counts cards, and names both nouns", async ({
    ...
    await expect(
      page.getByRole("heading", { name: "The People Room" }),
    ).toBeVisible();
    await expect(
      page.getByText(/\d+ (people|person) · \d+ firms?/),
    ).toBeVisible();
  });
  ...
  test("six chips, in one order, inside a group that says what it narrows", async ({
    ...
    await expect(
      group.getByRole("button", { name: "Everyone" }),
  ...
  test("the trade line appears under Crew and nowhere else", async ({
    ...
    await page.goto("/people");
    await expect(
      page.getByRole("group", { name: "Narrow by trade" }),
    ).toHaveCount(0);
  ...
  test("nothing under a narrowing says so in the room’s own words", async ({
    ...
    await expect(
      page.getByText("Nobody under this narrowing yet."),
    ).toBeVisible();
  });
});
