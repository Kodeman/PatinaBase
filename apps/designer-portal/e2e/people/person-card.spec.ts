...
async function addSub(page: import('@playwright/test').Page, name: string, trade: string) {
  ...
  await page.getByLabel('Email').fill(`${name.split(' ')[0].toLowerCase()}@twincitiesdrywall.com`);
...
test.describe('the person card', () => {
  test('task 4 — do not contact, routed to somebody reachable', async ({
    ...
    try {
      ...
      await page.getByLabel('Why').fill('He is deaf to the phone. Rosa runs his book.');
      await page.getByRole('button', { name: 'Save the rule' }).click();

      await expect
        .poll(async () => (await ruleForSubject(frankCard))?.route_to_person_id ?? null, {
          timeout: 15_000,
        })
      ...
      await expect
        .poll(async () =>
          ((await ruleForSubject(frankCard))?.channels_forbidden ?? []).slice().sort(),
        )
        .toEqual(['email', 'sms']);
  ...
  test('R-V — every region prints, and an absent record says so in words', async ({
    ...
    try {
      ...
      for (const head of ['Reach & access', 'Seats on projects', 'Past seats', 'History']) {
