...
export function useQRAuth(redirectTo: string, baseUrl = ''): QRAuthResult {
    ...
    const poll = async () => {
      try {
        ...
        if (data.status === 'approved' && data.tokenHash && data.email) {
          ...
          try {
            // Complete authentication via verifyOtp
            ...
            // Hard redirect to ensure session cookies are sent on the new page load
        ...
        // Silently ignore poll network errors — we'll retry next interval
      }
    };
  ...
  }, [state, sessionToken, cleanup, redirectTo]);
