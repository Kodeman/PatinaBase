...
interface AddBookkeeperModalProps {
  projectId: string;
  open: boolean;
  onClose: () => void;
}

export function AddBookkeeperModal({ projectId, open, onClose }: AddBookkeeperModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lookupError) {
      setErrorMessage('Error looking up user. Please try again.');
      return;
    }

    if (!profile) {
      setErrorMessage('No user found with that email — they must register first.');
      return;
    }
  ...
  return (
    ...
    >
      <h2 className="type-item-name mb-6">Add Bookkeeper</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
        ...
        <div className="flex gap-3 pt-4">
          <PortalButton variant="primary" type="submit" disabled={isPending || !email.trim()}>
            {isLooking ? 'Looking up…' : addTeamMember.isPending ? 'Adding…' : 'Add Bookkeeper'}
          </PortalButton>
