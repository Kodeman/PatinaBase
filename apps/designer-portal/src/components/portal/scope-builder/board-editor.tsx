...
import { ProductPickerModal, type ProductPickResult } from '../proposals/product-picker-modal';
...
export function BoardEditor({ proposalId, projectId, boardId }: BoardEditorProps) {
  ...
  const { data: scheduleItems = [] } = useProposalScheduleItems(isProject ? undefined : proposalId);
  ...
  const { queue: queueLayoutPatch, state: layoutAutosaveState } = useBufferedAutosave<string, BoardLayoutPatch>({
  ...
  const queueLayout = useCallback(
    (changedItems: ProposalBoardItem[]) => {
      const activeItems = changedItems.filter((item) => !retiredLayoutItemIdsRef.current.has(item.id));
  ...
  layoutAutosaveBlocksSyncRef.current =
    layoutAutosaveState === 'dirty' || layoutAutosaveState === 'saving' || layoutAutosaveState === 'error';
  ...
  const mergeLocal = useCallback((itemId: string, patch: Partial<ProposalBoardItem>) => {
    const next = itemsRef.current.map((item) => (item.id === itemId ? { ...item, ...patch } : item));
  ...
  const mergeLocalLayout = useCallback(
    (itemId: string, patch: Partial<ProposalBoardItem>) => {
      if (structuralPendingRef.current || retiredLayoutItemIdsRef.current.has(itemId)) {
        return;
      }
    ...
    (itemId: string, patch: Partial<ProposalBoardItem>) => {
      void runStructuralMutation('The item could not be updated.', async () => {
        await updateItem.mutateAsync({
          ...
          ...(patch.height !== undefined ? { height: patch.height === null ? null : Number(patch.height) } : {}),
  ...
  const nextZ = useCallback(() => itemsRef.current.reduce((max, i) => Math.max(max, i.z_index), 0) + 1, []);
  ...
  const handleDeleteItem = useCallback(
    (itemId: string) => {
      void runStructuralMutation('The item could not be deleted.', async () => {
        // Retire the id before the delete request. Any late BoardCanvas event
        // or detached drain is filtered from the layout upsert, so the deleted
        // row cannot be recreated after the mutation wins.
  ...
  const handleArrange = useCallback(() => {
    ...
    void runStructuralMutation('The board could not be arranged.', async () => {
      ...
      const nextItems = itemsRef.current.map((item) => {
        ...
        if (!position || (Number(item.x) === position.x && Number(item.y) === position.y)) {
          return item;
        }
        const changed = { ...item, x: position.x, y: position.y };
  ...
  const handleItemsChange = useCallback(
    (next: BoardItem[]) => {
      ...
      const nextItems = itemsRef.current.map((p) => {
        ...
        if (nx !== Number(p.x) || ny !== Number(p.y) || nz !== p.z_index || nr !== Number(p.rotation)) {
          const changed = {
            ...p,
            x: nx,
            y: ny,
          ...
          <>
            {selected ? (
              <ItemInspector
                ...
                disabled={structuralPending}
                sendToSchedule={
                  !isProject && proposalId && (selected.type === 'product' || selected.type === 'capture') ? (
                    <SendToScheduleControl
                      proposalId={proposalId}
    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {viewMode === 'edit' && (
        <div className="flex items-center gap-1.5">
          ...
          <Button variant="secondary" size="sm" onClick={onArrange} disabled={disabled || itemCount === 0}>
            Arrange
          </Button>
        </div>
      )}
...
function BoardSectionsPanel({
  ...
  return (
    <SidebarSection title="Sections">
      ...
      <Button variant="ghost" size="sm" onClick={() => onChange(addSection(sections, ''))}>
        + Add section
      </Button>
    </SidebarSection>
  );
      ...
      try {
        ...
        onAdd({
          ...
          data: {
            ...
            ).map((s) => ({ hex: s.hex, role: s.role, name: s.name })),
          },
        });
      } finally {
        setAddingId(null);
        ...
        <div className="space-y-1.5">
          {palettes.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-2">
              ...
              <Button variant="ghost" size="sm" disabled={addingId === p.id} onClick={() => void handleAdd(p)}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {withThumbs.map((scan) => (
          ...
          >
            ...
            <img src={scan.thumbnail_url ?? ''} alt={scan.name} loading="lazy" className="h-full w-full object-cover" />
          </button>
        ))}
      </div>
      <p className="mt-2 text-[0.65rem] text-[var(--text-muted)]">Click a scan to drop it on the board.</p>
    </SidebarSection>
  );
}
...
function ItemInspector({
  ...
  const currentSectionId = (item.data as { section_id?: string | null } | null)?.section_id ?? '';
    ...
    <div className="rounded-md border border-[var(--accent-primary)] bg-[var(--bg-surface)] p-4">
      ...
      <fieldset disabled={disabled} className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5 border-t border-[var(--border-default)] pt-3">
          <Button variant="ghost" size="sm" onClick={() => onCommitField(item.id, { locked: !item.locked })}>
          ...
          <Button variant="ghost" size="sm" className="!text-red-700" onClick={() => onDelete(item.id)}>
            Delete
          </Button>
        </div>
        ...
        {sendToSchedule && <div className="border-t border-[var(--border-default)] pt-3">{sendToSchedule}</div>}
...
function NoteContentEditor({ content, onCommit }: { content: string; onCommit: (next: string) => void }) {
...
function ExtractPalettePanel({
  ...
  return (
    <div className="border-t border-[var(--border-default)] pt-3">
      ...
      {open && (
        <div className="mt-2">
          <ImagePaletteExtractor imageUrl={imageUrl} k={5} onExtracted={(swatches) => void handleExtracted(swatches)} />
          {created && (
...
function SendToScheduleControl({
  ...
  return (
    <div>
      <Button variant="ghost" size="sm" disabled={addItem.isPending} onClick={() => void handleSend()}>
