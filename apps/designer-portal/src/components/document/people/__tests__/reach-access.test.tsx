...
const consentData: { current: Record<string, unknown> | null } = { current: null };
...
describe('a channel row', () => {
  ...
  it('prints its kind, its markers, its number as a tel: link and its consent word', () => {
    ...
    renderReach();
    expect(screen.getByText('Mobile · preferred · verified 12 Oct 2026')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: '(612) 555-0111' })).toHaveAttribute(
      'href',
      'tel:+16125550111',
    );
  ...
  it('R-Q — the consent sentence reads the one wording, with source, date and job', () => {
    ...
    expect(
      screen.getByText('Written consent, 2 May 2025, on the Lindqvist kitchen.'),
    ).toBeInTheDocument();
  });
  ...
  it('a held channel keeps its row, its ground and its reason in words', () => {
    ...
    renderReach();
    const row = document.querySelector('[data-reach-channel-held="true"]') as HTMLElement;
    expect(row).toBeInTheDocument();
    expect(within(row).getByText(/This address bounced back, 12 March 2026\./)).toBeInTheDocument();
  });
  ...
  it('records consent through the studio’s own ledger, with a method and evidence', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Put it on the books' }));
...
describe('a grant row', () => {
  it('prints the tier, what it opens, the dates and a Revoke that confirms first', () => {
    ...
    expect(
      screen.getByText('Ends with the job, 13 August 2027. Renews when they use it.'),
    ).toBeInTheDocument();
    const revoke = screen.getByRole('button', { name: 'Revoke' });
    expect(revoke).toHaveAttribute('aria-expanded', 'false');
    fireEvent.click(revoke);
    expect(revoke).toHaveAttribute('aria-expanded', 'true');
    expect(screen.getByRole('button', { name: 'Close this door' })).toBeInTheDocument();
  });
});
...
describe('the pure parts', () => {
  ...
  it('heldChannelReason says why, and what still reaches them', () => {
    expect(
      heldChannelReason({ status: 'bounced', status_at: '2026-03-12T00:00:00Z' } as never),
    ).toBe('This address bounced back, 12 March 2026. Texts and calls still reach them.');
  });
  ...
  it('a grant inside fourteen days adds the count, and nothing outside it does', () => {
    expect(grantEndsSentence('2026-10-27T00:00:00Z', NOW)).toContain('7 days left.');
    expect(grantEndsSentence('2027-08-13T00:00:00Z', NOW)).not.toContain('days left');
  });
