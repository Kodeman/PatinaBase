...
jest.mock('@patina/supabase', () => ({
  usePeopleDirectory: (...args: unknown[]) => mockUsePeopleDirectory(...(args as [])),
  useStudioContacts: (...args: unknown[]) => mockUseStudioContacts(...(args as [])),
  usePeopleSeats: () => ({ data: [] }),
}));
...
describe('the six chips', () => {
  ...
  it('shows the trade line only under Crew and Makers', () => {
    ...
    expect(screen.getByRole('group', { name: 'Narrow by trade' })).toBeInTheDocument();
    unmount();
...
describe('one list, two entry types (PR-g)', () => {
  it('prints people and firms together under Everyone', () => {
    ...
    expect(screen.getByRole('button', { name: 'Dana Kowalski' })).toBeInTheDocument();
  ...
  it('the Firms chip shows firms only', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Dana Kowalski' })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Northgate Electric' })).toBeInTheDocument();
  });

  it('R-H — a lender firm shows under both chips with no paper word', () => {
    const { unmount } = renderDirectory([BANK]);
    expect(screen.getByRole('button', { name: 'Great Northern Bank' })).toBeInTheDocument();
...
describe('narrowing', () => {
  it('matches phone digits', () => {
    ...
    expect(screen.getByRole('button', { name: 'Dana Kowalski' })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Adaeze Okonkwo' })).not.toBeInTheDocument();
  });
  ...
  it('says so in the room’s own words when nothing is left', () => {
    ...
    expect(screen.getByText(/Nobody under this narrowing yet\./)).toBeInTheDocument();
  });
});
...
describe('the duplicate band', () => {
  it('names the two cards and opens each, and carries no merge act (R-Y)', () => {
    ...
    expect(within(band).getByRole('button', { name: 'Adaeze Okonkwo' })).toBeInTheDocument();
    expect(within(band).getByRole('button', { name: 'Chidi Okonkwo' })).toBeInTheDocument();
    expect(screen.queryByText(/Compare/)).not.toBeInTheDocument();
    fireEvent.click(within(band).getByRole('button', { name: 'Chidi Okonkwo' }));
