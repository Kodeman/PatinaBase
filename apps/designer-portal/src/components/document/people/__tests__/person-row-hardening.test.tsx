...
describe('the word columns', () => {
  it('carries exactly three — reach, consent, paper — and never a stage word', () => {
    ...
    expect(
      bordered.querySelector('[data-state-family="stage"]'),
    ).toBeNull();
  });
  ...
  it('R-M — line 2 prints the same three words plain at 390, on every row', () => {
    ...
    const plain = container.querySelector('[data-row-words-390]') as HTMLElement;
  ...
  it('R-A — a lender prints no paper word in either place', () => {
    const { container } = render(
      <ul>
        <PersonRow
          person={person({ meta: { contact_kind: 'lender' }, paper_state: 'not_on_file' })}
          onOpen={jest.fn()}
        />
      </ul>,
    );
  ...
  it('prints NOTHING for a consent record the studio does not hold (R-BB)', () => {
    render(
      <ul>
        <PersonRow person={person({ consent_status: null })} onOpen={jest.fn()} />
      </ul>,
    );
