...
describe('the six regions', () => {
  ...
  it('the identity line counts the crew, the jobs and the warranty', () => {
    renderCard();
    expect(
      screen.getByText('Electrical sub · 1 person · 2 projects · warranty through 21 November 2026'),
    ).toBeInTheDocument();
  });
...
describe('R-P — the Paper region, in one fixed order', () => {
  it('prints the table, the leading-rule clause, the sentence, then the acts', () => {
    renderCard();
    const region = document.querySelector('[data-company-paper]') as HTMLElement;
  ...
  it('C21 — a firm with no paper still prints the region, the word and the act', () => {
    ...
    expect(screen.getByRole('button', { name: 'Record a document' })).toBeInTheDocument();
  });

  it('C13 — a lender prints one line and no act at all', () => {
    cardData.current = { ...(cardData.current as object), company_kind: 'lender' };
    renderCard();
    expect(screen.getByText('No paper is held for this firm.')).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Record a document' })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: 'Chase the renewal' })).not.toBeInTheDocument();
  });
});
