...
jest.mock('@patina/supabase', () => ({
  ...
  useArchiveStudioContact: () => ({ mutateAsync: archiveMutate, isPending: false }),
  useRestoreStudioContact: () => ({ mutateAsync: restoreMutate, isPending: false }),
}));
...
describe('ArchiveCardDoor', () => {
  it('puts a live card away', async () => {
    ...
    await waitFor(() => expect(archiveMutate).toHaveBeenCalledWith({ id: 'card-dana' }));
  });
  ...
  it('brings an archived card back, and says it is away', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Bring this card back' }));
    await waitFor(() => expect(restoreMutate).toHaveBeenCalledWith({ id: 'card-dana' }));
  });
