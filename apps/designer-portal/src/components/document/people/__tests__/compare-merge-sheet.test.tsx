...
jest.mock('@patina/supabase', () => ({
  ...
  usePeopleSeats: () => ({ data: [] }),
  useMergeStudioContacts: () => ({ mutateAsync: mergeMutate, isPending: false }),
...
describe('mergeConsequenceSentence', () => {
  it('names what moves, and says consent does not', () => {
    const sentence = mergeConsequenceSentence('Adaeze Okonkwo', 'Chidi Okonkwo');
...
describe('CompareMergeSheet', () => {
  ...
  it('merges the pair the studio chose, with the evidence it named', async () => {
    render(<CompareMergeSheet {...props} />);
    fireEvent.change(screen.getByLabelText('What makes these the same person'), {
      target: { value: 'email' },
    });
    fireEvent.click(
      screen.getByRole('button', { name: 'Merge into Adaeze Okonkwo' }),
  ...
  it('merges the OTHER way once the pick is flipped', async () => {
    render(<CompareMergeSheet {...props} />);
    fireEvent.click(
      document.querySelector('[data-survivor-pick="card-chidi"]') as HTMLElement,
    );
    fireEvent.click(
