...
export function archivedSentence(archivedAt: string | null | undefined): string {
...
export function ArchiveCardDoor({
  ...
  return (
    <div data-archive-door={contactId} className={className}>
      ...
      >
        <DocumentAction
          actionKey={isArchived ? 'restore-studio-contact' : 'archive-studio-contact'}
          variant="tertiary"
          disabled={!canArchive}
          held={!canArchive}
      ...
      {!canArchive && (
        <p id={reasonId} className="mt-1 text-[0.7rem] text-[var(--ink-subtle)]">
          {STUDIO_CONTACT_ARCHIVE_STANDING_SENTENCE}
        </p>
      )}

      {error && (
        <p role="alert" className="mt-1 text-[0.72rem] text-[var(--terracotta-ink)]">
          {error}
        </p>
      )}
    </div>
