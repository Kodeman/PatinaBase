...
export const REVOKE_REASON_PROMPT = 'Say why the door closes. Optional, kept with the record.';

/** The tier's own word, or the raw token where a base table drifted. */
function tierLabel(tier: string): string {
  return (
    ACCESS_GRANT_TIER_LABELS[tier as keyof typeof ACCESS_GRANT_TIER_LABELS] ?? tier
  );
}

function tierOpens(tier: string): string | null {
  return ACCESS_GRANT_TIER_OPENS[tier as keyof typeof ACCESS_GRANT_TIER_OPENS] ?? null;
}
...
function GrantRow({
  ...
  const close = () => {
    setError(null);
    revoke.mutate(
      { grantId: grant.grant_id, tier: grant.tier, reason: reason.trim() || null },
      {
        onSuccess: () => {
          setConfirming(false);
          setReason('');
        ...
        onError: (e) =>
          setError(e instanceof Error ? e.message : 'Could not close that door just now.'),
      },
    );
  };

  return (
    <li data-access-grant={grant.grant_id} className="border-t border-[var(--hairline)] py-3">
      <p className="t-body-sm text-[var(--ink)]">{grantRowParts(grant).join(' · ')}</p>
      ...
      {revokable ? (
        <>
          ...
          <div id={confirmId} hidden={!confirming} className="mt-2">
            <label htmlFor={reasonId} className="t-head block text-[var(--ink-subtle)]">
              Why it closes
            </label>
            <p className="t-body-sm mt-1 text-[var(--ink-subtle)]">{REVOKE_REASON_PROMPT}</p>
            <input
              id={reasonId}
              type="text"
              value={reason}
...
export function AccessGrantList({
  ...
  if (!grants || grants.length === 0) {
    return <p className="t-body-sm text-[var(--ink-subtle)]">{NO_GRANT_SENTENCE}</p>;
  }
  return (
