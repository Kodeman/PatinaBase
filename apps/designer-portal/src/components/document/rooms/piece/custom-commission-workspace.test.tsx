...
jest.mock('../../document-action', () => ({
  ...
  DocumentActionGroup: ({ children }: { children: React.ReactNode }) => <>{children}</>,
}));

jest.mock('../room-sheet', () => ({
  RoomSheet: ({ children, open }: { children: React.ReactNode; open: boolean }) =>
...
describe('CustomCommissionWorkspace revision safety', () => {
  it('records a quote and status in one atomic transition call', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Record quote revision' }));
  ...
  it('persists both approvals with the approved transition', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Record both approvals' }));

    await waitFor(() =>
      expect(value.onTransition).toHaveBeenCalledWith('revision-2', 'approved', {
        note: undefined,
  ...
  it('submits the commission but does not auto-send for a free-text maker', async () => {
    ...
    await waitFor(() =>
      expect(value.onTransition).toHaveBeenCalledWith('revision-3', 'submitted', {
    ...
    expect(value.onPrepareQuoteRequest).toHaveBeenCalledWith(
      'configuration-1',
      'revision-3',
      expect.objectContaining({ fabricator: 'Northstar Millwork', fabricatorVendorId: '' }),
    );
    expect(
      await screen.findByText(/Match the named fabricator to a maker before creating the RFQ draft/),
    ).toBeInTheDocument();
  });
});
