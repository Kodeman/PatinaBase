...
describe("custom commission lifecycle", () => {
  it("carries a cabinetry draft through quote, approval, and issued lock", () => {
    expect(
      hasCommissionBriefErrors(validateCommissionBrief(cabinetryBrief)),
    ).toBe(false);
    ...
    expect(assertCommissionTransition("client_review", "approved")).toBe(
      "approved",
    );
  ...
  it("round-trips durable custom requirements without losing quote or drawings", () => {
    ...
    expect(restored).toMatchObject({
      ...
      allowance: "28500.00",
      quote: {
        tradeAmount: "31840.00",
        retailAmount: "41200.00",
        leadTimeWeeks: "14",
      },
    });
  });
  ...
  it("summarizes an approved custom commission embedded in the frozen project snapshot", () => {
    expect(
      summarizeCommissionSnapshot({
        productName: "Cabinetry template",
        configurationMode: "custom",
        ...
        tradePriceCents: 3184000,
        leadTimeWeeks: 14,
        dimensions: { width: 156, depth: 24, height: 108 },
        customCommission: {
          revisionNumber: 4,
          brief: {
            summary: "Library wall cabinetry",
          ...
          drawings: [{ name: "A-602", url: "/drawings/a-602.pdf" }],
          quote: { tradePriceCents: 3184000, leadTimeWeeks: 14 },
        },
      }),
    ).toMatchObject({
      title: "Library wall cabinetry",
