...
const ALL_FACETS = ['identity', 'piece', 'story', 'categorization', 'commerce', 'lifecycle', 'sourcing', 'seo', 'eye'];
...
export function PieceRoom({ productId }: { productId: string }) {
  ...
  const studioIds = useMemo(
    () =>
      new Set(
        ((orgs ?? []) as Array<{ id?: string; organization_id?: string; membership?: { role?: string } }>)
          .filter((o) => ['owner', 'admin', 'member'].includes(o.membership?.role ?? ''))
  ...
  const studioId = useMemo(() => {
    const list = (orgs ?? []) as Array<{ id?: string; organization_id?: string; type?: string }>;
  ...
  const canEdit = useMemo(() => {
    ...
    if (p.layer === 'studio') return !!p.studio_id && studioIds.has(p.studio_id);
    return false;
  ...
  if (error || !p) {
    return (
      <RoomShell title="A piece" backTo="/library" backLabel="the Library">
        <div role="alert" className="mx-auto max-w-[520px] px-6 pt-16 text-center">
          <p className="font-heading text-[1.3rem] italic text-[var(--color-charcoal)]">This piece isn’t on a shelf.</p>
          <p className="mt-2 text-[0.84rem] text-[var(--color-aged-oak)]">
            It may have been merged or removed, or it failed to load. Try again, or head back to the Library.
          </p>
          <button
            type="button"
            onClick={() => void refetch()}
            className="mt-4 rounded-[5px] border border-[var(--doc-ink-border)] bg-white px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.08em] text-[var(--color-charcoal)] transition-colors hover:border-[var(--color-clay)]"
          >
            Try again
          </button>
        </div>
  ...
  const retail =
    p.price_retail != null ? (p.price_retail / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : null;
  const trade =
    p.price_trade != null ? (p.price_trade / 100).toLocaleString('en-US', { style: 'currency', currency: 'USD' }) : null;
  ...
  const saveToMyLibrary = async () => {
    ...
    try {
      ...
      setToast('Saved to My Library — a copy is on your shelf to edit and teach.');
    } catch {
    ...
    >
      <div className="mx-auto max-w-[1080px] px-6 pb-12 sm:px-9">
        ...
        <section className="grid gap-7 pt-7 min-[900px]:grid-cols-[minmax(0,440px)_minmax(0,1fr)]">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Chip tone={p.status === 'published' ? 'sage' : 'oak'}>{labelStatus(p.status)}</Chip>
              {readOnly && layer === 'catalog' && <Chip tone="oak">read only</Chip>}
            </div>

            <h1 className="mt-3 font-heading text-[2rem] leading-tight text-[var(--color-charcoal)]">{p.name}</h1>
            {p.brand && <p className="mt-0.5 font-heading text-[1.05rem] italic text-[var(--color-aged-oak)]">{p.brand}</p>}
            ...
            {(retail || trade) && (
              <p className="mt-3 text-[0.92rem] text-[var(--color-charcoal)]">
                ...
                {retail && <span className="text-[0.7rem] text-[var(--color-aged-oak)]"> retail</span>}
                {trade && (
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <span className="font-mono text-[0.6rem] tracking-[0.04em] text-[var(--color-aged-oak)]">
                <b className="font-heading text-[0.95rem] text-[var(--color-charcoal)]">{pct}</b>% · {state}
              </span>
            </div>

            {/* Action rail */}
            <div className="mt-5 flex flex-wrap gap-2">
              <RailButton onClick={() => setAddOpen(true)}>＋ Add to a project</RailButton>
              {layer !== 'personal' && <RailButton onClick={() => void saveToMyLibrary()}>↓ Save to My Library</RailButton>}
              {layer === 'personal' && canEdit && (
                <RailButton primary onClick={() => setPromoteOpen(true)}>
                  ↑ Promote to Studio
                </RailButton>
              )}
            ...
            {readOnly && layer === 'catalog' && (
              <p className="mt-3 max-w-[44ch] text-[0.72rem] italic text-[var(--color-aged-oak)]">
                A maker’s piece, curated by Patina. Save a copy to your library to adapt it, or add it straight to a
                project.
              </p>
            )}
          </div>
        ...
        <Movement name="The record" meta="Line 1 · what it is" hue="var(--color-mocha)">
          <PieceFacet
            ...
            readOnly={readOnly}
          >
            <FacetText productId={p.id} column="name" serverValue={p.name} label="Name" readOnly={readOnly} />
            <div className="flex flex-col gap-0 sm:flex-row sm:gap-3">
              <div className="flex-1">
                <FacetText productId={p.id} column="brand" serverValue={p.brand} label="Maker" placeholder="Nordic Atelier" readOnly={readOnly} />
              </div>
              <div className="flex-1">
                <FacetText productId={p.id} column="sku" serverValue={p.sku} label="SKU" placeholder="optional" readOnly={readOnly} />
              </div>
            </div>
            <FacetText productId={p.id} column="source_url" serverValue={p.source_url} label="Source URL" placeholder="paste, or from a capture" readOnly={readOnly} />
            {p.slug && (
              <div className="mt-3">
                <span className="mb-1 block font-mono text-[0.5rem] font-semibold uppercase tracking-[0.08em] text-[var(--color-aged-oak)]">
                  Slug
                </span>
                <p className="font-mono text-[0.72rem] text-[var(--color-aged-oak)]">{p.slug}</p>
              </div>
            )}
          </PieceFacet>
          ...
          <PieceFacet
            ...
            readOnly={readOnly}
          >
            <FacetDimensions productId={p.id} serverValue={p.dimensions} readOnly={readOnly} />
            <FacetChips productId={p.id} column="materials" serverValue={p.materials} label="Materials" placeholder="solid white oak…" readOnly={readOnly} />
            <FacetChips productId={p.id} column="colors" serverValue={p.colors} label="Colors" swatch readOnly={readOnly} />
            <FacetChips productId={p.id} column="available_colors" serverValue={p.available_colors} label="Available colors" swatch readOnly={readOnly} />
            <FacetText productId={p.id} column="finish" serverValue={p.finish} label="Finish" placeholder="matte, hand-rubbed oil…" readOnly={readOnly} />
          </PieceFacet>

          <PieceFacet
            name="The story"
            status={p.short_description || p.description ? 'written' : 'unwritten'}
            done={!!(p.short_description || p.description)}
            open={open.has('story')}
            onToggle={() => toggle('story')}
            readOnly={readOnly}
          >
            <FacetTextarea productId={p.id} column="short_description" serverValue={p.short_description} label="Short description" placeholder="the card blurb" rows={2} readOnly={readOnly} />
            <FacetTextarea productId={p.id} column="description" serverValue={p.description} label="Description" placeholder="the full story of the piece" rows={4} readOnly={readOnly} />
          </PieceFacet>
          ...
          <PieceFacet
            ...
            readOnly={readOnly}
          >
            <FacetSelect productId={p.id} column="category" serverValue={p.category} label="Category" options={CATEGORY_OPTIONS} needed={studioNeeds} readOnly={readOnly} />
            <FacetText productId={p.id} column="subcategory" serverValue={p.subcategory} label="Subcategory" readOnly={readOnly} />
            <FacetChips productId={p.id} column="tags" serverValue={p.tags} label="Tags" readOnly={readOnly} />
            <FacetChips productId={p.id} column="material_tags" serverValue={p.material_tags} label="Material tags" readOnly={readOnly} />
            <FacetChips productId={p.id} column="style_tags" serverValue={p.style_tags} label="Style tags" readOnly={readOnly} />
          </PieceFacet>
        </Movement>
        ...
        <Movement name="The catalog" meta="Line 2 · into the marketplace" hue="var(--color-clay)">
          ...
          >
            <div className="flex flex-col gap-0 sm:flex-row sm:gap-3">
              <div className="flex-1">
                <FacetMoney productId={p.id} column="price_trade" serverCents={p.price_trade} label="Trade price" note="On the maker’s side, this is theirs — one page, two authors." readOnly={readOnly} />
              </div>
              <div className="flex-1">
                <FacetMoney productId={p.id} column="price_retail" serverCents={p.price_retail} label="Retail price" readOnly={readOnly} />
              </div>
            </div>
            <FacetNumber productId={p.id} column="commission_rate" serverValue={p.commission_rate} label="Commission rate" suffix="%" step="0.01" min={0} max={99.99} readOnly={readOnly} />
          </PieceFacet>
          ...
          {canEdit && (
            <PieceFacet
              ...
              readOnly={false}
            >
              <FacetSelect productId={p.id} column="status" serverValue={p.status} label="Status" options={STATUS_OPTIONS} required readOnly={false} />
            </PieceFacet>
          )}

          <PieceFacet
            name="Sourcing"
            status={p.vendor_id || p.lead_time_weeks ? 'sourced' : studioNeeds ? 'needed to share' : 'optional'}
          ...
          >
            ...
            <FacetVendorPicker
              ...
              readOnly={readOnly}
            />
            <FacetVendorContact productId={p.id} serverValue={p.vendor_contact} needed={studioNeeds} readOnly={readOnly} />
            <div className="flex flex-col gap-0 sm:flex-row sm:gap-3">
              <div className="flex-1">
                <FacetNumber productId={p.id} column="lead_time_weeks" serverValue={p.lead_time_weeks} label="Lead time" suffix="weeks" needed={studioNeeds} readOnly={readOnly} />
              </div>
              <div className="flex-1">
                <FacetSelect productId={p.id} column="payment_terms" serverValue={p.payment_terms} label="Payment terms" options={PAYMENT_OPTIONS} needed={studioNeeds} readOnly={readOnly} />
              </div>
            </div>
          ...
          {showSeo && (
            <PieceFacet
              ...
              readOnly={readOnly}
            >
              <FacetText productId={p.id} column="seo_title" serverValue={p.seo_title} label="SEO title" readOnly={readOnly} />
              <FacetTextarea productId={p.id} column="seo_description" serverValue={p.seo_description} label="SEO description" rows={2} readOnly={readOnly} />
            </PieceFacet>
          )}
        </Movement>
        ...
        <Movement name="The eye" meta="Line 3 · the teaching" hue="var(--color-dusty-blue)">
          <PieceFacet
            ...
            status={hasTeaching ? `taught · ${taughtStyles.length} trait${taughtStyles.length > 1 ? 's' : ''}` : 'untaught'}
            done={hasTeaching}
            open={open.has('eye')}
            onToggle={() => toggle('eye')}
            readOnly={false}
          >
            ...
            <div className="mt-4 flex items-center gap-4">
              <button
                ...
                className="rounded-[5px] border border-[var(--color-clay)] px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.06em] text-[var(--color-clay)] transition-colors hover:bg-[var(--color-clay)] hover:text-white"
              >
            ...
            <p className="mt-3 border-t border-[var(--doc-ink-border)] pt-2.5 text-[0.7rem] italic text-[var(--color-aged-oak)]">
              The teaching is the same act as Quick Tags on the shelf — here, the full sitting.
            </p>
          </PieceFacet>
        </Movement>
      ...
      {deepOpen && (
        <DeepAnalysisSheet
          ...
          onSaved={() => setToast(`Taught — “${p.name}” is mapped. Your eye, learned.`)}
        />
      )}
      ...
      <AddToProjectSheet
        ...
        piece={{ id: p.id, name: p.name, price_trade: p.price_trade, price_retail: p.price_retail }}
...
function Chip({ children, tone = 'oak' }: { children: React.ReactNode; tone?: 'oak' | 'sage' }) {
  return (
    <span
      className="rounded-[3px] border px-2 py-0.5 font-mono text-[0.5rem] uppercase tracking-[0.1em]"
      style={
        tone === 'sage'
          ? { color: 'var(--color-sage)', borderColor: 'var(--color-sage)', background: 'rgba(168,181,160,0.12)' }
          : { color: 'var(--color-aged-oak)', borderColor: 'var(--color-pearl)' }
      }
    >
      {children}
    </span>
...
function RailButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-[5px] px-3 py-2 font-mono text-[9px] font-semibold uppercase tracking-[0.08em] transition-colors ${
        primary
          ? 'bg-[var(--color-charcoal)] text-[var(--color-off-white)] hover:opacity-85'
          : 'border border-[var(--doc-ink-border)] bg-white text-[var(--color-charcoal)] hover:border-[var(--color-clay)]'
      }`}
    >
      {children}
    </button>
...
function Colophon({ p }: { p: PieceProduct }) {
  ...
  const fmt = (d: string | null) => (d ? new Date(d).toLocaleDateString('en-US', { dateStyle: 'medium' }) : null);
  ...
  return (
    <section className="mt-10 border-t border-[var(--doc-ink-border)] pt-5">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex items-center gap-2 font-mono text-[0.5rem] font-semibold uppercase tracking-[0.1em] text-[var(--color-aged-oak)] hover:text-[var(--color-mocha)]"
      >
      ...
      {open && (
        <dl className="mt-3 grid grid-cols-2 gap-x-8 gap-y-2 sm:grid-cols-3">
          {rows
            ...
            .map(([k, v]) => (
              <div key={k}>
                <dt className="font-mono text-[0.46rem] uppercase tracking-[0.08em] text-[var(--color-aged-oak)]">{k}</dt>
                <dd className="text-[0.78rem] text-[var(--color-mocha)]">{v}</dd>
              </div>
            ))}
        </dl>
      )}
