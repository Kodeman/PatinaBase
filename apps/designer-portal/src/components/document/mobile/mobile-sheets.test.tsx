...
describe('the Margin sheet (D-B30 / W5-R1, the whole margin)', () => {
  ...
  it('W5-R1/W5-C14 — a line-anchored row takes L-10 first, then lands on its line', async () => {
    ...
    mountBarAndSheets({
      doc: { ...heldDocument, onJumpRegion },
      marginCount: 1,
    });
    fireEvent.click(openMore().getByRole('button', { name: 'Margin · 1' }));
    fireEvent.click(screen.getByText('Living room fabric'));

    // The press order the rail's own jump takes: unfold + promote through
    // `ffe` — which is what MOUNTS the line — before anything scrolls.
    expect(onJumpRegion).toHaveBeenCalledWith('ffe');
    // The item sheet opens at once; the line's own landing refines it two
    // frames later, once the promoted bodies have heights.
    expect(screen.getByRole('dialog', { name: 'Margin item' })).toBeInTheDocument();
    await waitFor(() =>
