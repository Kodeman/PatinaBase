...
describe("unified mobile edge owner", () => {
  ...
  it("keeps the signed-proposal activation action above fallback guide messaging", () => {
    ...
    expect(
      mobileBar.queryByRole("button", { name: "Review signing controls" }),
    ).not.toBeInTheDocument();
    fireEvent.click(
      mobileBar.getByRole("button", { name: "Open the project" }),
    );
  ...
  it("uses the latest guide input count when a stable mobile action is selected", () => {
    ...
    fireEvent.click(
      within(screen.getByTestId("mobile-bar")).getByRole("button", {
        name: "Continue Discovery",
      }),
    );
    expect(mockGuideSelected).toHaveBeenLastCalledWith(
      expect.objectContaining({ input_count: 1 }),
    );
  });
