...
/** 1.5px dashed vertical at a moved boundary. `leftPct` is pre-clamped by the caller. */
...
/** 8px rotate-45 dashed-border diamond, no fill, at a moved milestone. */
export function GhostDiamond({ leftPct, top, tone }: { leftPct: number; top: number; tone: GhostTone }) {
  return (
    <span
      className="absolute rotate-45"
      style={{ left: `${leftPct}%`, top, width: 8, height: 8, marginLeft: -4, background: 'none', border: `1.5px dashed ${TONE_COLOR[tone]}` }}
    />
  );
}
