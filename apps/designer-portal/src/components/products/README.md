...
```tsx
import { ProductFilters } from '@/components/products/product-filters';
import { useProducts, useCategories, useVendors } from '@/hooks/use-products';
import type { FilterState } from '@/types/product-filters';
...
describe('ProductFilters', () => {
  it('debounces search input', async () => {
    ...
    await waitFor(() => {
      expect(onFiltersChange).toHaveBeenCalledWith({ search: 'chair' });
    }, { timeout: 600 });
  });
});
```
