...
export interface RuleBar {
   ...
   *  phase chains off it) ⇒ the boundary handle is the way to edit that end. */
  hasInternalEndBoundary: boolean;
  /** That boundary handle is LOCKED (its downstream phase is anchored, so the
   *  handle refuses every drag). Without this, such a phase's duration would be
   *  editable by nothing at all — see `barCanResize`. `false` when there is no
   *  boundary on this end. */
  endBoundaryLocked: boolean;
}

/**
 * May this bar edit its own duration? Yes when no boundary handle stands on its
 * end — and ALSO yes when one does but is LOCKED: a locked handle refuses every
 * drag, so leaving the bar out of it would make the phase's duration
 * uneditable by any affordance at all. The resolver and the ripple already model
 * a duration eating slack toward an anchored downstream start, and the confirm
 * strip's `anchorViolation` gate is what refuses a commit that cannot fit — the
 * affordance does not need to pre-refuse it.
 */
export function barCanResize(
  bar: Pick<RuleBar, 'hasInternalEndBoundary' | 'endBoundaryLocked'>,
): boolean {
  return !bar.hasInternalEndBoundary || bar.endBoundaryLocked;
}
...
export function ruleBars(
  ...
  const boundedEnds = new Map(
    ruleBoundaries(phases, chain, scale).map((b) => [b.upstreamPhaseId, b.locked]),
  );
  ...
  for (const p of phases) {
    ...
    out.push({
      ...
      endBoundaryLocked: boundedEnds.get(p.id) ?? false,
    });
  }
  return out;
}
 ...
 * minus the offset the pointer held into the bar when the grab began. Grabbing
 * the bar's middle and dragging one day right therefore previews a one-day
 * move, not a start snapped onto the cursor. (Nothing visibly follows the
 * cursor — the committed bar holds its ground and the terracotta ghost layer
 * carries the preview, exactly as a boundary-handle drag does.)
 *
...
/**
 * Hold an epoch day inside the scale's padded domain — the same discipline
 * `xToEpochDay` applies to a pointer x, carried to the values a bar derives
 * AFTER that read (a move's grab-offset subtraction, a keyboard nudge) so no
 * gesture can stage a date off the instrument. Reversed bounds are tolerated
 * (the tighter of the two wins) rather than inverting the clamp.
 */
export function clampBarEpochDay(epochDay: number, minEpochDay: number, maxEpochDay: number): number {
  const lo = Math.min(Math.round(minEpochDay), Math.round(maxEpochDay));
  const hi = Math.max(Math.round(minEpochDay), Math.round(maxEpochDay));
  return Math.max(lo, Math.min(hi, Math.round(epochDay)));
}
