/* ── One date, printed one way ───────────────────────────────────────────────
   The house prints two date idioms and no others (PP-2, R140). H6 put them on
   the client's threshold; this is the same pair, in the same shapes, for the
   Desk:
   ...
   composes its own `Intl.DateTimeFormat`: a Desk that says "respond by
   11 September" one line above "respond by September 11" is telling a designer
   two things.
...
/**
 * Read a date the way the studio's own calendar reads it.
 *
...
/**
 * A month standing alone — "September" — as a span speaks it. Not a date
 * idiom: no day, no year, nothing to be read as a deadline. Kept here so the
 * surface holds one locale and not two.
 */
export const MONTH_NAME_FORMAT = new Intl.DateTimeFormat('en-GB', { month: 'long' });

/**
 * The weekday standing alone — "Tuesday" — which the Desk's greeting says
 * before the date. Same reason as MONTH_NAME_FORMAT: not an idiom, kept here
 * so one locale answers for every date word on the surface.
 */
