...
describe('pickerHistoryLine (PR-i)', () => {
  ...
  it('says so plainly when there is no history at all', () => {
    ...
    expect(
      pickerHistoryLine({ projectCount: 0, lastProjectName: null }),
    ).toBe('Never on a job yet');
  });
...
describe('bringForwardConsequence (SPEC §5.7 #7)', () => {
  it('says what the act does, who arrives refused, and which paper is down', () => {
    expect(
      bringForwardConsequence('Okonkwo residence', [
        { name: 'Dana Kowalski', consent: 'granted', firmName: 'Northgate Electric', paperClause: 'Northgate Electric’s insurance lapsed 31 March 2026.' },
        { name: 'Pete Rusk', consent: 'opted_out', firmName: 'Rusk Mechanical', paperClause: null },
        { name: 'Ingrid Halvorsen', consent: 'not_asked', firmName: null, paperClause: null },
        { name: 'Claire Bissett', consent: 'not_asked', firmName: null, paperClause: null },
      ]),
    ).toBe(
  ...
  it('says one paper once, however many of its crew were picked', () => {
    ...
    const sentence = bringForwardConsequence('Okonkwo residence', [
      { name: 'Dana Kowalski', consent: 'granted', firmName: 'Northgate Electric', paperClause: clause },
      { name: 'Sam Vo', consent: 'granted', firmName: 'Northgate Electric', paperClause: clause },
    ]);
...
describe('carriedConsentNotice (direction §5.2 Birth rule, F-12)', () => {
  ...
  it('names no job where the record names none', () => {
    expect(
      carriedConsentNotice({ optOutSource: 'studio_recorded', optOutAt: '2025-12-03' }),
