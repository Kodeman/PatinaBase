...
describe('expiryNoticeClause', () => {
  it('spells out direction §3.8’s own paper word for paper about to go', () => {
    ...
    ).toBe('Northgate Electric’s insurance lapses in 30 days, on 6 October 2026.');
  });
...
describe('noticedPaperClause', () => {
  ...
  it('reads the person’s own paper beside the firm’s (R-BA)', () => {
    const clause = noticedPaperClause(
      ...
      'Dana Kowalski',
      [doc({ id: 'doc-3', holder_type: 'person', holder_id: 'card-dana', doc_type: 'license' })],
      new Map([['doc-3', notice('doc-3', 'lapses_soon')]]),
      LABELS,
    );
    expect(clause).toBe('Dana Kowalski’s licence lapses in 30 days, on 6 October 2026.');
  });

  it('answers nothing where there is no holder to ask about', () => {
    expect(
      noticedPaperClause([null, undefined], 'Nobody', [doc()], new Map(), LABELS),
    ).toBeNull();
  });
});
