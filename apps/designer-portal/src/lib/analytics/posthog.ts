 ...
 *
 * `initPostHog()` is called from an effect at the top of the React tree, and
 * React runs *child* effects before *parent* effects — so every consumer
 * mounted underneath it (feature-flag hooks in the layout, the pageview and
 * identify trackers) runs its first effect while `initialized` is still
 * false. Consumers that treat "not enabled" as a terminal state settle
 * permanently wrong. This queue is how they wait for init instead.
 */
