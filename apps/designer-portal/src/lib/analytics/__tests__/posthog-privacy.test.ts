...
const token = 'opaque_field_link_token_1234567890abcd';
...
describe('PostHog Field bearer privacy boundary', () => {
  it('redacts Field bearer paths from nested pageview, referrer, and autocapture properties', () => {
    ...
    const event = sanitizePostHogEvent({
      event: '$autocapture',
      properties: {
        $current_url: `https://app.patina.cloud/field/${token}?from=sms`,
        $referrer: `https://app.patina.cloud/field/${token}`,
        $elements: [
          {
            tag_name: 'a',
            attributes: {
  ...
  // J1. The pay-link token is the one bearer on this list that reaches a
  // TILL, and it is permanent — it stays live for the invoice's life.
  // invoice-folio.tsx renders the raw `clientInvoiceUrl` (an absolute
  // `https://client.patina.cloud/pay/<token>` link) as both an anchor's href
  // and its visible text, with autocapture left on — so a click on that link
  // must never leave this boundary with the token attached.
  it('redacts a root-relative pay-link bearer from an autocaptured href', () => {
    const event = sanitizePostHogEvent({
      event: '$autocapture',
      properties: {
        $elements: [{
    ...
    expect(serialized).toContain('/pay/[redacted]');
  });

  it('redacts pay-link bearers from an absolute pageview, referrer, autocapture, and nested values', () => {
    const event = sanitizePostHogEvent({
      event: '$autocapture',
      properties: {
        $current_url: `https://client.patina.cloud/pay/${payToken}?from=email`,
        $referrer: `https://client.patina.cloud/pay/${payToken}`,
        $elements: [{
          tag_name: 'a',
          attributes: {
            href: `https://client.patina.cloud/pay/${payToken}`,
            $el_text: `https://client.patina.cloud/pay/${payToken}`,
    ...
    const serialized = JSON.stringify(event);
    expect(serialized).not.toContain(payToken);
    expect(serialized.match(/\/pay\/\[redacted\]/g)?.length).toBeGreaterThanOrEqual(5);
    expect(event.properties?.already_redacted).toBe('/pay/[redacted]');
  });

  it('redacts the pay-link return-flow shape /pay/return/<token>', () => {
    const event = sanitizePostHogEvent({
      event: '$autocapture',
      properties: {
        $current_url: `https://client.patina.cloud/pay/return/${payToken}`,
        $referrer: `/pay/return/${payToken}`,
        $elements: [{
          tag_name: 'a',
          attributes: {
            href: `/pay/return/${payToken}`,
          },
        }],
      },
    });
