...
describe('captureTeachingEvent (posthog-js mocked)', () => {
  ...
  it('inits one named personless instance with the shared key and host', () => {
    ...
    expect(config.property_denylist).toEqual(expect.arrayContaining(LOCATING_KEYS));
  ...
  it('puts no location, referrer, session or device on the wire from a /doc/<projectId> page', () => {
    ...
    try {
      ...
      jest.isolateModules(() => {
        ...
        const sdk = (jest.requireActual('posthog-js') as typeof import('posthog-js')).default;
        mockInit.mockReset();
      ...
      // The taxonomy's own keys survive the route filter.
      ...
      for (const key of LOCATING_KEYS) expect(properties).not.toHaveProperty(key);
      expect(Object.keys(properties).filter((key) => key.startsWith('$initial_'))).toEqual([]);
      ...
      const rest = Object.entries(properties).filter(([key]) => key !== 'distinct_id');
