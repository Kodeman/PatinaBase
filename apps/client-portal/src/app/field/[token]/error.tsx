...
interface FieldLinkErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

/**
 * Field guest error boundary (Field Coordination). Guests reach this page
 * from a no-login token link — there is no session, no "your projects" to
 * go back to — so this stays retry-only, unlike projects/error.tsx which
 * offers a home link for a signed-in client.
 */
export default function FieldLinkError({ error, reset }: FieldLinkErrorProps) {
  useEffect(() => {
    console.error('Field link page error:', error);
  }, [error]);
    ...
    <div className="flex min-h-screen items-center justify-center bg-[var(--bg-primary)] px-6">
      <div className="w-full max-w-md space-y-6 text-center">
        <div className="space-y-2">
          <h1 className="type-section-head">Something went wrong</h1>
          <p className="type-body mx-auto">
            We&rsquo;re having trouble loading this page. This might be a temporary issue.
          </p>
        </div>
        ...
        <div className="border-l-2 border-patina-dusty-blue pl-4 text-left">
          ...
          <ul className="mt-2 space-y-1 type-body-small">
            ...
            <li>Contact your designer if the issue persists</li>
          </ul>
        </div>
