...
describe('ProjectReviewEdition', () => {
  it('records a preference only and explains it is not authorization', () => {
    ...
    review.mockReturnValue({ isLoading: false, isError: false, data: { projectId: 'project-1', editionId: 'edition-1', status: 'published', publishedAt: null, items: [{ id: 'item-1', name: 'Chair', roomName: 'Living room', imageUrl: null, clientPriceCents: 120000, currency: 'USD', verdict: null, comment: null, mediaAssetIds: [] }] } });
    feedback.mockReturnValue({ mutate, isPending: false });
    render(<ProjectReviewEdition projectId="project-1" editionId="edition-1" />);
  ...
  it('requires a written question before recording the comment verdict', () => {
    const mutate = jest.fn();
    review.mockReturnValue({ isLoading: false, isError: false, data: { projectId: 'project-1', editionId: 'edition-1', status: 'published', publishedAt: null, items: [{ id: 'item-1', name: 'Chair', roomName: 'Living room', imageUrl: null, clientPriceCents: null, currency: 'USD', verdict: null, comment: null, mediaAssetIds: [] }] } });
    feedback.mockReturnValue({ mutate, isPending: false });
    render(<ProjectReviewEdition projectId="project-1" editionId="edition-1" />);
    ...
    expect(mutate).toHaveBeenCalledWith({ reviewItemId: 'item-1', verdict: 'comment', comment: 'Could this be lighter?' });
  });

  it('does not offer verdict controls after an edition is closed', () => {
    review.mockReturnValue({ isLoading: false, isError: false, data: { projectId: 'project-1', editionId: 'edition-1', status: 'finalized', publishedAt: null, items: [{ id: 'item-1', name: 'Chair', roomName: 'Living room', imageUrl: null, clientPriceCents: null, currency: 'USD', verdict: 'approved', comment: null, mediaAssetIds: [] }] } });
