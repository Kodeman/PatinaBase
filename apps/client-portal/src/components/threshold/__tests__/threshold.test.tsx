...
beforeEach(() => {
  ...
  identityMock.mockReturnValue(
    settled({ name: "Quist Interiors", source: "studio" }),
  );
  teamMock.mockReturnValue(
    settled([
      { id: "tm-1", role: "lead_designer", user: { full_name: "Nora Quist" } },
    ]),
  );
  ...
  pendingReviewMock.mockReturnValue({ data: [], isLoading: false });
  submittedReviewMock.mockReturnValue({ data: [], isLoading: false });
  submitReviewMock.mockReturnValue({ mutate: jest.fn(), isPending: false });
  scopeChangesMock.mockReturnValue({ data: [], isLoading: false });
  approveScopeChangeMock.mockReturnValue({
    mutate: jest.fn(),
    isPending: false,
  });
  declineScopeChangeMock.mockReturnValue({
    mutate: jest.fn(),
    isPending: false,
  });
  createScopeChangeMock.mockReturnValue({
    mutate: jest.fn(),
    isPending: false,
  });
...
describe("Threshold — which house", () => {
  ...
  it("stands the story pole beside the whole house, as a sticky left rail", () => {
    ...
    expect(
      pole.compareDocumentPosition(doorstep) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    ...
    expect(rail.parentElement!.contains(container.querySelector("#mat"))).toBe(
      true,
    );
  });
...
describe("Threshold — the five facts", () => {
  ...
  it("addresses the plate to the client the session names, and to nobody without one", () => {
    ...
    renderThreshold();
    expect(screen.getByTestId("doorplate-line")).not.toHaveTextContent(
      "prepared for",
    );
  });
...
describe("Threshold — the doorstep’s own asks", () => {
  it("stands a phase approval on the doorstep, with its own act", () => {
    ...
    expect(gate).toHaveTextContent(
      "Library elevations · Edition 3 · Due August 20",
    );
  ...
  it("reads a draft approval as a review rather than a response", () => {
    ...
    expect(
      screen.getByRole("link", { name: /review exact edition/i }),
    ).toBeInTheDocument();
  });
  ...
  it("stands a roomless paper on the doorstep, not inside a band", () => {
    ...
    proposalsMock.mockReturnValue(
      settled([AUTHORIZATION, roomless, SIGNED_AGREEMENT]),
    );
...
describe("Threshold — the acts the house owes", () => {
  ...
  it("resolves an invoice enclosure on the note from the invoice cache", () => {
    notesMock.mockReturnValue(
      settled([
        { ...NOTE, enclosures: [{ kind: "invoice" as const, id: "inv-4" }] },
      ]),
    );
    renderThreshold();

    expect(screen.getByTestId("note-enclosures")).toHaveTextContent(
      "Invoice No. 4",
    );
  });
});
...
const TILE_BUNDLE = {
  ...
  tradeScope: {
    ...
    progress: { state: "substantially_complete" },
    draws: [
      { amountCents: 96000, gatesOnAcceptance: true, invoicePaidCents: 0 },
    ],
  },
};
...
describe("Threshold — the held draws", () => {
  it("totals what two instruments hold, one bundle read each", () => {
    selectionsMock.mockReturnValue(
      settled({
        origin: "commercial",
        selections: [CREDENZA, PAINTWORK, SECOND_TRADE],
      }),
    );
    renderThreshold();
...
describe("Threshold — a room’s target", () => {
  ...
  it("stands on the agreed figure alone when nothing is planned", () => {
    ...
    expect(screen.getByTestId("house-ledger-top")).not.toHaveTextContent(
      "planned",
    );
  });
});
...
describe("Threshold — what moved since", () => {
  it("ticks the band a moved piece belongs to", () => {
    ...
    expect(container.querySelector(`#room-${LIBRARY}`)).toHaveAttribute(
      "data-changed",
      "true",
    );
    expect(container.querySelector(`#room-${ENTRY}`)).not.toHaveAttribute(
      "data-changed",
    );
  });
  ...
  it("ticks nothing when no line carries a timestamp", () => {
    ...
    expect(container.querySelector(`#room-${LIBRARY}`)).not.toHaveAttribute(
      "data-changed",
    );
  });
});
...
describe("Threshold — never reverse", () => {
  it("says nothing about the letterbox while the invoices are in flight", () => {
    ...
    expect(
      screen.queryByText(/nothing in the letterbox/i),
    ).not.toBeInTheDocument();
    expect(screen.getByTestId("threshold-hold")).toBeInTheDocument();
  });
  ...
  it("says nothing about the drawing while the papers are in flight", () => {
    ...
    expect(screen.queryByTestId("plan-key")).not.toBeInTheDocument();
    expect(
      screen.queryByText(/stands open on this drawing/i),
    ).not.toBeInTheDocument();
  });
  ...
  it("holds the house while the plan is in flight — targets decide every variance", () => {
    ...
    expect(
      screen.queryByTestId("house-ledger-overage"),
    ).not.toBeInTheDocument();
  ...
  it("holds the doorplate up throughout — the house is always named", () => {
    ...
    expect(screen.getByTestId("doorplate-title")).toHaveTextContent(
      "The Vale Residence",
    );
    expect(screen.getByTestId("doorstep-sentence-pending")).toBeInTheDocument();
  });
...
describe("Threshold — enclosures and history", () => {
  it("keeps a signed enclosure in the letter, pointing at Previously", () => {
    notesMock.mockReturnValue(
      settled([
        { ...NOTE, enclosures: [{ kind: "proposal" as const, id: "prop-1" }] },
      ]),
    );
    ...
    expect(enclosures.querySelector("a")).toHaveAttribute(
      "href",
      "#previously",
    );
  });
  ...
  it("encloses a trade scope by its own name, not by one of its lines", () => {
    notesMock.mockReturnValue(
      settled([
        {
          ...NOTE,
          enclosures: [{ kind: "trade_scope" as const, id: "prop-paint" }],
        },
      ]),
    );
    renderThreshold();
    ...
    expect(screen.getByTestId("note-enclosures")).not.toHaveTextContent(
      "The paintwork",
    );
  });
...
describe("Threshold — L6, the review and scope-change asks mounted in place", () => {
  ...
  it('offers "Ask for a change" on the mat', () => {
    ...
    expect(
      within(screen.getByTestId("mat")).getByText("Ask for a change"),
    ).toBeInTheDocument();
  });
  ...
  it('offers "Ask for a change" inside a room band, naming the room', () => {
    ...
    expect(
      within(band as HTMLElement).getByText(/Ask for a change in/),
    ).toBeInTheDocument();
  });
});
