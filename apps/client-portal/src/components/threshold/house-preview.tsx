...
import { GOODS_JOURNEY_STAGES, journeyStageIndexForStatus } from '@/components/commercial/journey-stepper';
...
export function HousePreview({ bands }: { bands: RoomBandModel[] }) {
  ...
  return (
    <section className="house-preview" aria-labelledby={headingId} data-testid="house-preview">
      ...
      <div className="house-preview-body">
          ...
          <div className="house-preview-image">
            {hasImage ? (
              ...
              <div className="house-preview-placeholder" data-testid="house-preview-placeholder">
                <span aria-hidden="true" className="house-preview-monogram">{room.name.trim().charAt(0)}</span>
                <p>{piece ? 'An image is not available for this piece yet.' : 'The room is here. Its pieces will follow.'}</p>
              </div>
            )}
          </div>
          <figcaption>
        ...
        <div className="house-preview-detail" aria-live="polite" aria-atomic="true">
          <p className="material-eyebrow">{piece ? 'In this room' : 'A place for what comes next'}</p>
          <h3>{piece?.name ?? room.name}</h3>
          {piece && (
            <>
              <p className="house-preview-status">{GOODS_JOURNEY_STAGES[journeyStageIndexForStatus(piece.logisticsStatus)]}</p>
              {piece.clientLineTotalCents > 0 && (
                <p className="house-preview-price">{moneyInWords(piece.clientLineTotalCents)} <span>for {piece.quantity === 1 ? 'this piece' : `${piece.quantity} pieces`}</span></p>
              )}
            </>
          )}
          {room.pieces.length > 1 && (
            <div className="house-piece-picker" role="group" aria-label={`Preview pieces in ${room.name}`}>
              {room.pieces.slice(0, 4).map((selection) => (
                <button
                  key={selection.id}
                  type="button"
