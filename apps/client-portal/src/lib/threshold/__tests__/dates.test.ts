...
describe('legalDate — the day a paper is dated by', () => {
  ...
  it('reads a date-only string in the client’s own calendar', () => {
  ...
  it('reads a full timestamp', () => {
    expect(legalDate(new Date(2026, 0, 1, 9, 30).toISOString())).toBe('1 January 2026');
  });

  it('holds the first of the month', () => {
    expect(legalDate('2026-01-01')).toBe('1 January 2026');
  });

  it('holds the last of the year', () => {
...
describe('the surface prints one date style', () => {
