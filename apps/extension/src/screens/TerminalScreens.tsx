...
import { runCommit, classifySaveError, deriveRetryKind } from '../state/effects';
...
export function SavedScreen() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="mt-1 max-w-[28ch] text-[0.85rem] text-ink-soft">{placementMessage}</p>
      <button
        type="button"
        onClick={() => dispatch({ type: 'CAPTURE_NEXT' })}
        className="mt-5 rounded-md bg-verdigris px-4 py-2 font-mono text-[0.7rem] uppercase tracking-[0.08em] text-paper hover:bg-verdigris-ink"
...
export function ErrorScreen() {
  ...
  // only Snapshot and by-hand remain, even if a draft happens to be sitting
  // in state (a known-bad refusal always wins the body/actions shown).
  const isKnownBad = io.error === KNOWN_BAD_DOMAIN_MESSAGE;
  const isSaveError = draft !== null && !isKnownBad;
  ...
  const retrySave = async () => {
    ...
    const captureTimeMs = captureStartedAt != null ? Date.now() - captureStartedAt : undefined;
    try {
      const duplicateMode =
      ...
      // All three classes stay on R5 with a live Retry — see CommitBar's
      // catch for why an expired-session save doesn't route off the draft.
