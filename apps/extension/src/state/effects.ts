...
async function runProjectPlacement(
  ...
  try {
    const outcome = await placeProductInProject(
      productId,
      route,
      placementSource(draft),
      { duplicateMode },
    );
...
export async function retrySpecBookPlacement(
  ...
  const route = routing.specBookPlacement;
  if (!route || route.kind === "library")
    return { productId, placementOutcome: null };
  const placementOutcome = await runProjectPlacement(
    productId,
    route,
    draft,
    false,
...
export async function saveToInbox(
  ...
  const { rawPayload, thumbnailUrl } = captureInput(
    draft,
    routing,
    user.id,
    "",
...
export async function saveAsDecision(
  ...
  const decisionInsert = buildDecisionInsertPayload(
    decisionInput(draft, routing),
  );
...
export async function updateExisting(
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      // D6 adds sku to productRow; this line lands with it
      capture_provenance: withCaptureNote(
        existingProduct?.capture_provenance as
...
/** The distinct save paths CommitBar's footer can trigger. */
...
/**
 * Reconstructs "the same commit target" for a Retry from R5, purely from
 * state — CommitTarget only distinguishes library/inbox, so the dedup-match
 * branches (update/reuse) are re-derived the same way CommitBar's own render
 * picks its primary button.
 */
 ...
 * else, per the R5 routing rules (CL W3-E10). `auth` routes to the
 * signed-out screen instead of R5 — see CommitBar's catch.
 */
