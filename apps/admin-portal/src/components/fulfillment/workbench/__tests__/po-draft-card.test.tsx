...
function makeLine(overrides: Partial<FulfillmentWorkbenchLine>): FulfillmentWorkbenchLine {
  return {
    id: 'line-1',
    orderId: 'order-1',
    productId: 'prod-1',
...
describe('PoDraftCard — R7 line-detail trail (WP4 Track 3)', () => {
  it('shows a collapsed Lifecycle disclosure per line that reveals the stamp trail', () => {
    ...
    const liveStepRow = disclosure.querySelector('li[data-trail-step="acknowledged"]');
    expect(liveStepRow).toHaveAttribute('data-trail-state', 'live');
    expect(within(liveStepRow as HTMLElement).getByText('Acknowledged')).toBeInTheDocument();
  });
  ...
  it('renders a cancelled line as its operational word, with no trail position at all', () => {
    ...
    expect(screen.queryByTestId('wb-po-line-lifecycle')).not.toBeInTheDocument();
