...
function fmtDay(iso: string): string {
  const asDate = /^\d{4}-\d{2}-\d{2}$/.test(iso) ? new Date(`${iso}T00:00:00`) : new Date(iso);
  return new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(asDate);
}
...
function TrailStamp({ label, color, ink }: { label: string; color: string; ink?: string }) {
  return (
    <span
      ...
      style={{ fontFamily: 'var(--font-meta)', borderColor: color, color: ink ?? color }}
    >
      {label}
    </span>
  );
...
function StepRow({ step, label }: { step: ProcurementStepReading; label: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0">
        {step.state === 'settled' && (
          <TrailStamp label={on ? `${label} · ${on}` : label} color="var(--color-mocha)" />
        )}
        {step.state === 'live' && (
          <TrailStamp
...
function GateBar({ gate, label }: { gate: ProcurementGateReading; label: string }) {
  ...
  return (
    <li
      ...
      style={{ borderColor: reached ? 'var(--color-aged-oak)' : 'var(--color-pearl)' }}
    >
      <span className="flex flex-wrap items-baseline gap-x-2">
        ...
        {gate.state === 'settled' && <TrailStamp label="Settled" color="var(--color-sage)" />}
...
export function ProcurementTrail({ reading }: { reading: ProcurementLifecycleReading }) {
  return (
    <div data-procurement-trail className="mb-3">
      ...
      <ol className="list-none">
        {PROCUREMENT_LIFECYCLE_STEPS.map((step, index) => {
          const gate = PROCUREMENT_LIFECYCLE_GATES.find((g) => g.afterStep === step.ordinal);
          const gateReading = gate ? reading.gates.find((g) => g.key === gate.key) : undefined;
          return (
            <Fragment key={step.key}>
              <StepRow step={reading.steps[index]} label={step.label} />
              {gate && gateReading && <GateBar gate={gateReading} label={gate.label} />}
            </Fragment>
          );
        })}
      </ol>
