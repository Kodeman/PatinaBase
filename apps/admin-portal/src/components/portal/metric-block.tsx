...
function formatAnimatedNumber(current: number, original: string | number): string {
  const parsed = parseNumericValue(original);
  if (!parsed.isNumeric) return String(original);

  const rounded = Math.round(current);
  const formatted = parsed.num >= 1000 ? rounded.toLocaleString() : String(rounded);
  return `${parsed.prefix}${formatted}${parsed.suffix}`;
}

function AnimatedNumber({ value, original }: { value: number; original: string | number }) {
  ...
  const display = useTransform(spring, (v) => formatAnimatedNumber(v, original));
  ...
  return (
    <div className="flex flex-col">
      ...
      <span className="type-data-large mb-1">
        {parsed.isNumeric ? <AnimatedNumber value={parsed.num} original={value} /> : value}
      </span>
