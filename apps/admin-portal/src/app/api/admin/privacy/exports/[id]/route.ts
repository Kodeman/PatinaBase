...
export async function PATCH(
  ...
  try {
    ...
    if (!before) return notFound(`Export request ${id} not found`);
    ...
    // mig 00113 columns may not be in generated types yet; cast.
    ...
    await createAuditLog(adminClient, {
      userId: user.id,
      action: body.action === 'approve' ? 'privacy.export.approve' : 'privacy.export.unapprove',
    ...
    return serverError((err as Error).message ?? 'Failed to update export request');
  }
}
