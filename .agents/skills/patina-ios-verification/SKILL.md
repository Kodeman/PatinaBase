...
Don't use when:
- App Store Connect submission, TestFlight, release, metadata, screenshots, IAP/subscriptions, or signing/provisioning — use the existing `asc-*` skills under `apps/mobile/Patina/.Codex/skills/` (25 of them: `asc-release-flow`, `asc-testflight-orchestration`, `asc-xcode-build`, `asc-metadata-sync`, `asc-crash-triage`, `asc-signing-setup`, and more — scoped to that directory). This skill covers only build-and-verify discipline for code changes, not ASC mechanics. Note: `apps/mobile/Capture` (Patina Field) has no equivalent `.Codex/skills/` directory at all — no scoped ASC skill library exists for it yet.
...
2. **Read the app's own doc before writing Swift**: `apps/mobile/AGENTS.md` (split overview), then `apps/mobile/Patina/AGENTS.md` or `apps/mobile/Capture/README.md` for the app you're in.
...
- [ ] Identified the app (Patina vs Capture/Patina Field) and read its AGENTS.md/README first
...
| Need an ASC/TestFlight/signing task done for iOS | Improvise submission steps here | Use the scoped `asc-*` skills under `apps/mobile/Patina/.Codex/skills/` (none exist yet for Capture) |
