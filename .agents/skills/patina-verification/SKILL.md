...
### CI reality
No CI quality gate exists. The retired Docker publishing workflow is removed. `extension-beta.yml` / `extension-cws.yml` are tag-triggered only (`extension-beta-v*` / `extension-v*`) — ordinary pushes and PRs do not run test/type gates. Local affected verification is the only quality verification.
