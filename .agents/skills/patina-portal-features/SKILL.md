...
- **Client portal (`apps/client-portal`) is a different shape entirely — do not apply the zone/route-group pattern above to it.** Ruled 2026-09-04 (`docs/design/the-document/DECISIONS.md` R135; `docs/vision/VISION-DECISIONS.md` V8): the client page is The Document's homeowner face, not a fourth portal with its own zones. Its authenticated surface is one page — `/` (the client's active project) and `/projects/[projectId]` — both rendering `<Threshold>` (`src/components/threshold/threshold.tsx`); there is no header, no nav, no project switcher outside the mat, and no feature flag. Every other authenticated path a new feature might be tempted to add as its own route is instead a **redirect to an anchor on that page** (`src/middleware.ts`) — a new client-facing act belongs in `components/threshold/` as an in-place instrument (unfold, lift, or laid-in sheet), never a new `src/app/<name>/page.tsx` tree. See `apps/client-portal/README.md`'s route map and `docs/design/the-client-page/README.md` before adding anything here.
