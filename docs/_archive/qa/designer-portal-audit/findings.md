...
| SMOKE-04 | (repo health) | P2 | render | `pnpm --filter @patina/designer-portal type-check` fails with ~3089 errors — all in OTHER packages (api-client, patina-design-system `*.stories.tsx`, types/utils `*.test.ts` missing `expect`/test globals). The portal's own src is clean, but the workspace tsc can't pass, so type-check can't gate CI. | /tmp/tc.log | Fix test/story tsconfig globals or exclude from type-check project |
| SMOKE-05 | (repo health) | P3 | render | `pnpm --filter @patina/designer-portal lint` (`next lint`) is deprecated and drops into an interactive ESLint-setup prompt → fails in CI/non-TTY. | /tmp/lint.log | Migrate to ESLint CLI / add eslint config |
