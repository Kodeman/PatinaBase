...
| ~~00530–00535~~ **CLOSED/EXHAUSTED** | Field Companion (`docs/design/field-companion/`). **Corrected 2026-08-31 (wave 4 census, W4-C1):** this band is no longer available — `00530` MINTED 2026-08-24 (wave 1) and `00532` DRAWN 2026-08-25 (wave 3, the visit spine) are Field Companion's, `00531` is the unrelated `uuid_generate_v5` grant hotfix, but **`00533`, `00534` and `00535` were drawn by OTHER lanes** (`00533_piece_detail_contract.sql`, `00534_client_attention_notifications.sql`, `00535_saved_items_price_snapshot.sql` — none of them Field Companion) before wave 4 landed, so the "symbolic reservation" this row previously described for `00533–00535` was already false on the filesystem by the time wave 4 ran its landing census. **Wave 4 draws `00543–00545` above the current head (`00542`) instead** — see the new band row below. **Not measured at the same census** — the filesystem/`git log --all`/local-applied-ledger census this row otherwise describes cannot see Strata at all. Per `supabase migration list --project-ref bkvcixdmuyejfzcijpdg`, run separately during wave 4 pre-flight on 2026-08-31: `00530`, `00531` and `00532` **are applied to Strata**; `00533–00540` are on `main` but **not** applied to Strata. |
| 00543–00545 | Field Companion wave 4 (`docs/design/field-companion/plans/wave-4-plan.md`, conductor override W4-C1). **Drawn 2026-08-31** after a census confirming all three free on the filesystem, in `git log --all -- 'supabase/migrations/0054*.sql'`, and in the local applied ledger (head `00542` at draw time). `00543` = the margin migration, `00544` = the punch back-reference migration, `00545` = the time-entry migration — see the new section below. |
...
| 00532  | `00532_field_capture_visit_and_suggestion.sql` | **DRAWN 2026-08-25** on branch `feat/field-companion-w3` (wave 3, the visit spine); census re-run at landing — `ls supabase/migrations` head `00531`, and `git log --all -- 'supabase/migrations/0053*'` showed `00532` free on every ref. Adds ten additive columns to `field_captures` (`visit_*` ×6, `suggested_*` ×2, `suggestion_basis`, `suggestion_confidence`), the partial index `idx_field_captures_visit`, and the BEFORE INSERT OR UPDATE projection trigger `trg_field_captures_visit_projection`. ⚠ **Deliberately does NOT touch `commit_field_capture`** — it projects the columns out of `raw_payload` in a trigger precisely to stay off that two-author shared object, so it cannot collide with 00516 or 00530 in either order. Local replay only — NOT applied to staging or prod |

`00530` is minted, `00531` is drawn (the `uuid_generate_v5` grant hotfix,
branch `hotfix/uuid-generate-v5-grant`) and `00532` is drawn (wave 3's
visit/suggestion migration). **Corrected 2026-08-31 (wave 4 census):**
`00533–00535` were NOT reserved symbolically in practice — the filesystem
shows `00533_piece_detail_contract.sql`, `00534_client_attention_notifications.sql`
and `00535_saved_items_price_snapshot.sql` were drawn by other, unrelated
lanes before wave 4 landed, and `main` head had advanced to `00542` by wave
4's landing census. The band `00530–00535` is therefore CLOSED/EXHAUSTED for
Field Companion; wave 4 drew `00543–00545` above the head instead (see below).
Every address is still claimed at the landing of the file that needs it,
after re-checking this file, the filesystem, `git log --all`, and (where the
task allows it) `supabase migration list`.

### Drawn from 00543–00545 (Field Companion wave 4)

| Number | File | Landed state |
| ------ | ---- | ------------ |
| 00543  | `00543_margin_notes_field_capture.sql` | **DRAWN 2026-08-31** on branch `feat/field-companion-w4`; census re-run at landing (filesystem head `00542`, `0054*` clear on every ref in `git log --all`, clear in the local applied ledger). Adds `margin_notes.field_capture_id` and recreates `margin_items` (00282:606-909 verbatim except the `note` branch, which now carries the note's full body plus the field-capture lane in `payload`). Local replay only — NOT applied to staging or prod. ⚠ **PROD PUSH WARNING (F8):** Strata has `00530–00532` applied and `00533–00540` **NOT** applied (a deliberate selective-apply gap — see the `00530–00535` row above), while `00541`/`00542` **are** applied. `supabase db push` reconciles by version: pushing `00543` the normal way would also drag the eight deferred `00533–00540` Daily Return migrations to prod. This requires a **selective apply** (`supabase db push --include-all` is NOT what you want here — target this file specifically); do not run a plain `supabase db push` until the `00533–00540` gap is closed deliberately by its owner. |
| 00544  | `00544_project_task_field_capture_ref.sql` | **DRAWN 2026-08-31** on branch `feat/field-companion-w4`; same census as 00543. Adds `project_tasks.field_capture_id` (FC-R7 + FC-R15: a Field punch item is a `project_tasks` row, never a `client_decisions` row). No new RPC, no policy change, no ACL block — FC-R7 removed all four from the punch path. Local replay only — NOT applied to staging or prod. ⚠ **PROD PUSH WARNING (F8):** same gap as the 00543 row above — Strata has `00533–00540` unapplied (deliberate selective-apply gap) while `00541`/`00542` are applied. A plain `supabase db push` reconciles by version and would drag those eight deferred migrations to prod along with this one. Requires a **selective apply**; do not push this file with a plain `db push` until the `00533–00540` gap is closed deliberately by its owner. |
| 00545  | `00545_time_entry_field_visit_source.sql` | **DRAWN 2026-08-31** on branch `feat/field-companion-w4`; same census as 00543. Widens `project_time_entries.source` to admit `'field_visit'` via a new named constraint `project_time_entries_source_ck`, resolving 00198's unnamed inline CHECK from `pg_constraint` by content rather than a guessed name. Local replay only — NOT applied to staging or prod. ⚠ **PROD PUSH WARNING (F8):** same gap as the 00543 row above — Strata has `00533–00540` unapplied (deliberate selective-apply gap) while `00541`/`00542` are applied. A plain `supabase db push` reconciles by version and would drag those eight deferred migrations to prod along with this one. Requires a **selective apply**; do not push this file with a plain `db push` until the `00533–00540` gap is closed deliberately by its owner. |
