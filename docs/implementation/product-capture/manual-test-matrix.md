...
1. **Build a prod-config binary** — either:
   - **CI dry-run artifact** (matches exactly what ships — packaged with the
     ...
     gh run watch "$RUN_ID"
     gh run download "$RUN_ID" -n "patina-capture-$(node -p "require('./apps/extension/package.json').version")" -D "$TMPDIR/patina-capture-dl"
     unzip -o "$TMPDIR"/patina-capture-dl/*.zip -d "$TMPDIR/patina-capture-dl/unpacked"
     ```
     (Run `node -p ...` from the repo root — the artifact name is derived
     from `apps/extension/package.json`'s `version` field, not hardcoded, so
     this stays correct across the 0.3.0 bump landing in W4. Same build path
     as `e2e-prod-walk.md`'s setup section — this matrix reuses it verbatim.)
...
The offline and expired-session rows below depend on lane **W3-E10** (branch
`capture-launch/w3-e10`, `859ffb0cd`), which is merging into
`capture-launch/integration` next — confirm it has landed on your checkout
before running these two rows; the rest of the matrix doesn't depend on it.

| Case | Setup | Expected |
|---|---|---|
| **Pinterest pin** (known-bad domain) | Navigate to any `pinterest.com/pin/...` URL (e.g. `https://www.pinterest.com/pin/378724649918852625/`, the harvested fixture — a board/roundup pin, not a product) and open the panel | The R5 error screen (`TerminalScreens.tsx`) shows **exactly** `"This page doesn't carry product details. Snapshot it, or add the piece by hand."` (`KNOWN_BAD_DOMAIN_MESSAGE`, `src/lib/mode-detection.ts:68-69`) — with **Snapshot** and **By hand** buttons but **no Retry** button (`isKnownBad` check; retrying a known-bad domain is a dead end by design, CL-R14). This is the `draft === null` branch of R5 (extraction failure) — distinct from the two save-error rows below. |
| **Offline save** | Open a product page, let extraction complete (so a draft exists), then DevTools → Network → Offline, then attempt any save | R5 now handles save errors too (CL W3-E10): the panel navigates to R5 with the title **"Couldn't save"** (`isSaveError = draft !== null && !isKnownBad`, `TerminalScreens.tsx`), body text **exactly** `"You're offline — your draft is kept. Retry when you're back."` (`classifySaveError`'s `offline` branch, `state/effects.ts` — matches on `navigator.onLine === false` or a `/failed to fetch\|networkerror\|network request failed\|load failed/i` error message), and **two** buttons: a **live Retry** (verdigris-ink) and a secondary **"Edit the record"** (both `disabled` while retrying). While a retry is in flight, the body text itself swaps to **"Retrying your save…"** (not just the button label, which separately reads "Retrying…"). Go back online and click Retry — it must **re-run the exact same commit target** you originally clicked (`retrySave` → `deriveRetryKind` → `runCommit`, not a fresh extraction), landing on the matching terminal screen (S4/S5) with no re-typing needed — the draft is never cleared by a `SAVE_ERROR`. Click **"Edit the record"** instead to confirm it navigates back to C2 (`dispatch({ type: 'NAV', screen: 'C2' })`) with every field you already filled in still present — nothing is lost. **No Snapshot/By hand buttons on this row** — those are extraction-only escapes; offering them here would discard the very draft you're trying to save. |
| **Expired session** | Force the extension's Supabase JWT to look expired before saving — e.g. let the 30-min `refresh-token` alarm miss its window with the machine asleep, or directly edit the stored session in `chrome.storage` to an already-expired `expires_at`/garbage `access_token` — then attempt any save | Same R5 "Couldn't save" screen, but body text **exactly** `"Your session expired — sign in to finish saving."` (`classifySaveError`'s `auth` branch — triggers on Postgrest code `PGRST301`, HTTP `401`, or a message matching `/jwt/i` **and** `/expired/i`). Same **Retry** + **"Edit the record"** button pair, same in-flight **"Retrying your save…"** body-text swap, same re-run-the-same-target behavior on Retry — an expired-session save is treated as "needs a token refresh on the next attempt," not a sign-out, so (per the code comment in `CommitBar.tsx`'s catch) it deliberately does **not** route you off the draft or back to `AuthScreen.tsx`. |
| **Duplicate URL** | Save any product from row 1–6 to the library once, then re-open the panel on the **same URL** | Exact-URL dedup fires (`state/reducer.ts` "dedup" block); `RecordScreen.tsx:22-31` shows the brass "Looks like one you have" banner naming the existing item, and `CommitBar.tsx`'s footer swaps to `Update "{name}"` (brass primary) / `Save as new` (secondary) — confirm the literal quoted product name matches the earlier capture, and that clicking Update reaches S4 with the existing product's row updated in place (`updateExisting` in `state/effects.ts`), not a second product row created. |
