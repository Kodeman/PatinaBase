...
### I152 · The Smart Lens — waves W0–W6, deviations, debts — 2026-08-30
...
### R136 · Studio invoices — an invoice with no house — 2026-09-05

**Asked by Kody for Middle West Studio, ruled the same day ("go with your recommendations"), against the deck `artifacts/studio-invoices-2026-09-05/proposal.html`.** A studio bills things that are not a project — a consultation, a site visit, a paid design review, a retainer, a reimbursable — and today the only way to bill one is to open a phantom project and litter the Desk, the ledger and the homeowner's page with a house that is not a house. Ruled: **a studio invoice is the same invoice with no house.** `invoices.project_id` becomes nullable and gains a `title` — the regarding line — and everything downstream stays exactly where it was: same numbering, same folio, same five acts, same Checkout rail, same earnings.

The twelve rulings the deck put up, as adopted: **S1** first-class project-less invoice, not a hidden shell project and not R79; **S2** the name is *studio invoice*, "ad-hoc" stays the line kind (R74); **S3** drawn in the same composer, whose first section becomes *for* and whose select carries "the studio · no house" ahead of the houses — one verb, one sheet, one habit; **S4** billed to a household on the roster, R73 invite-on-send, no public pay page; **S5** paid in the letterbox of the adopted house, no new client route (R135); **S6** ad-hoc lines only — milestones, time and FF&E are all house-bound; **S7** it earns design fees like any other invoice (R36/R37); **S8** an explicit studio line only when the designer belongs to more than one active studio, silent otherwise; **S9** Receivables-page dunning in v1, no Desk need line — there is no folder to hang one on; **S10** flagged on the designer side only, `studio-invoice`, fail-closed; **S11** iOS gets null-safety only; **S12** the regarding line is required — it is the only thing that names the letter on the ledger, in the subject line, and on the client's mat.

**The real gate is not the RPC, it is the trigger.** `public.set_invoice_studio_id()` raised on any NULL-project row on both INSERT and UPDATE, *before* its service_role early return, so issue, record payment, void, chase, the reminder cadence and the Stripe webhook's settle would all have failed. The migration adds a studio branch to both arms and leaves every project-path line byte-identical; `apply_invoice_payment_effects` moves to a LEFT JOIN so the earnings row survives a houseless invoice. A new `create_draft_studio_invoice` stands beside `create_draft_invoice` rather than overloading it.

*Entries add: R136 · last id = R136*

### R137 · The invoice, standing alone — `/pay/<token>`, and the toll leaves the page — 2026-09-06

**Asked and ruled by Kody, 2026-09-06 (K1–K11), against `artifacts/invoice-standalone-2026-09-06/`.** An issued invoice gets a permanent, account-less address of its own: **`https://client.patina.cloud/pay/<token>`** — the whole invoice on one page, and the till on it. The 64-hex token is the credential; there is no expiry (K7), no `pay.patina.cloud` (K6), and no feature flag — the page is public and additive, and the letters' switch to `/pay` is the cutover.

**This supersedes R135's "the acts never leave the page — including the toll."** That ruling removed `SpineToll`'s outbound `href` on the reasoning that the caller no longer existed and settling in place was the only way the act is taken. The fact it rested on has changed: the person who most needs to pay a Patina invoice is a homeowner who has **no account and no house on the platform** — a studio invoice's billed party (R136 · S4), or anyone the studio simply mails an invoice to. Settle-in-place can only ever serve someone already signed in. The toll now has somewhere to go, and going there is the point. R135's rule still governs every *other* act on the client page: only the invoice leaves it, and only to its own address.

Consequences recorded so nobody re-opens them:

- **The letters address the link.** `invoice-send`, `invoice-reminders`, and the `stripe-webhook`'s receipt and failure letters build their portal URL from `ensure_invoice_link` per letter, never cached, so a regenerated link is honored by the next send. A null answer falls back to today's `/invoices/<id>` rather than shipping a broken address.
- **I1 — the iOS receipt link is reversed by name.** The comment at `stripe-webhook/index.ts` recorded the opposite decision at the exact line now edited: `/invoices/<id>` was kept *because* the Patina iOS app claims `/invoices/*` in its applinks entitlement, so a client with the app installed opened the native invoice. After this change a signed-in iOS client's receipt link opens Safari, not the app. That is a deliberate consequence of K1, not a side effect — and `/pay/*` is **absent from the AASA on purpose**: a claimed `/pay/*` would open an app with no pay screen and dead-end the one person the feature is for.
- **I2 — the inbox and the email now differ.** `notification_log.metadata.deep_link` stays `/invoices/<id>` because it routes the iOS inbox by id. The emailed link and the in-app row for the same event land on different surfaces. Harmless; stated so nobody "fixes" it later.
- **K5 — void is a withdrawn sheet, not a dead page.** A voided invoice's link closes and resolves to a minimal withdrawn payload — studio, number, regarding line, contact; no amounts, no lines, no chooser. Only a *regenerated* (revoked) or unknown token is dead.
...
### R139 · I107 amended — the tertiary rest rule is unconditional, and a fourth tier called `terminal` — 2026-09-08

**Ruled by Kody, 2026-09-08** (`artifacts/portal-polish-review-2026-09-08/rulings.md` **PP-3**; deck `artifacts/portal-polish-review-2026-09-08/deck/index.html` sheets 10 and 12, evidence on sheets 4–9), on the panel's third principle — *every act shows its weight and its consequence*. I107 (`:6584-6611`) retired boxes and fills from `DocumentAction` and put four roles on a proofreader's score. Two clauses of it are amended. Everything else in I107 — the 12px label floor from I91, the invisible 44px `.da-hit` halo, the wet-ink press, the clay caret focus, R15 and D4 intact — stands untouched.

**One. The tertiary rest rule is unconditional.** I107 reads:

> tertiary = unscored until hover, when its score draws in

That clause is retired. A tertiary act carries a resting rule **at all times**, drawn at 1px in `--color-aged-oak` `#8B7355` — **4.20:1**, above the 3:1 that SC 1.4.11 asks of a non-text control — which thickens and inks clay on hover. The panel put the same defect up three times: **IX03** finds `.da-tertiary` at `transform: scaleX(0)` at rest (`globals.css:690-701`), so "PRINT" on the doorstep and "ASK FOR A CHANGE" in the mat are typographically identical to the metadata beside them; **IX04** finds the same `scaleX(0)` under every roster job name (`desk-roster.tsx:110`, `globals.css:409-428`), so on a phone the Desk's sixteen primary destinations carry no interactive mark at all; **B01** measures the secondary's resting score at `rgba(44, 41, 38, 0.28)` = **1.75:1** and names the standard it fails. Hover is not a state a touch device has. An act that is only an act under a pointer is not an act. This is a floor, not a restyle: **the two-score secondary is unchanged**, and no tier gains a box, a border or a background by it.

**Two. A fourth tier exists: `terminal`.** Filled charcoal `#2C2926`, paper ink, 3px radius, and **the amount in the label** — "Pay $4,060.00", "Accept the finished work · $2,980.00". Its label is Inter 500 at 16px in sentence case with tabular figures; tertiary and secondary keep DM Mono caps (`docs/design/house-sheet/SPEC.md` §A5 and §F-C). It is spent **only where money moves or a paper is signed** — pay, accept, sign, authorize — and nowhere else, and it is never olive. This does not reopen I107's retirement of the box: the fill *is* the consequence, it is the only filled control a designer or a homeowner ever meets, and a tier whose whole meaning is "this one is different" cannot be drawn in the same ink as the fifteen acts around it. `bg-[var(--color-charcoal)]` therefore stays **retired chrome** for `primary` and `inked` and is **correct grammar** for `terminal` alone; `document-action.test.tsx`'s variant table is the one place that records the distinction, so the next reader finds it beside the rule it qualifies rather than in a comment.

**Three, as a consequence of the second: `/pay/<token>`'s Pay act joins the tier rather than keeping its own grammar.** **IX10** counts three unrelated action languages shipping inside one product — scored ink on the Desk and the Threshold, a filled charcoal button plus selectable radio rows plus an underlined text link on the invoice, and outlined glyph pills (`✓ APPROVE` / `✗ FLAG` / `✎ NOTE`) on the client verdicts. R137 gave the standalone invoice an address of its own; it did not give it an action language of its own. The invoice's filled Pay button was Patina-native by accident and is Patina-native by rule from here; the verdict pills come onto the same scale, at the tier their consequence earns.

Carried with the tier, all from PP-3: one consequence sentence stands above every terminal act in every state (R141); `aria-disabled` with a named reason, never `disabled` — **IX02** sampled the wall gate's accept act at full primary ink (44, 41, 38) while `wall-gate.tsx:279` intends it disabled, which is a control lying about whether it can be pressed; the press-and-hold announced to pointer users, not only to the keyboard; and the act replaced by its dated record once taken (R51's discipline — the surface answers by being true).

*Entries add: R139 · last id = R139*

### R140 · R126 amended — the type contract, plate sizes, and the owed figure outranks the agreed figure — 2026-09-08

**Ruled by Kody, 2026-09-08** (**PP-2**, **PP-4**, **PP-5**; deck sheets 10, 11 and 13). R126 (`:9981-10108`) ruled Direction A — Ink on Paper — and closed its typography with a fence: *"Typography goes no further than the mockup."* That sentence did the work it was written for, which was to stop a restyle running ahead of a ruling. It is now spent, because it names a mockup as the contract where a written sheet is needed. R126 is amended on three points and on no others.

**One. The type contract is the seven-step scale plus the 15px money step.** `docs/design/house-sheet/SPEC.md` §A3 names the seven — `.t-d1` 34, `.t-d2` 26, `.t-d3` 20, `.t-body` 16, `.t-body-sm` 14, `.t-meta` 12, `.t-head` 11 — with `.t-authorship` and `.t-money` as additions rather than new steps. `.t-money` is DM Mono 15px/1.5, tabular, tracked .02em, and it carries **every ledger figure**. Money, dates, party names and consequence sentences sit at a **15px floor in sentence case**; running heads and captions stay 11–12px metadata, which is what metadata is for. **One family per figure**: the single *announced* figure at the head of a money block takes `.t-d2` and every other figure in that block takes `.t-money` — no block ever shows one payment in three families. One date style throughout: *11 September 2026*. No inline `font-size` survives this.

**Two. Plate sizes.** A piece plate is **96–120px** on desktop above a value threshold — 96px in a client room band, 120px for a decision hero, 64px in a dense schedule row or at ≤600px (§A10). **VC-44** counted five of six FF&E thumbnails rendering as the no-image placeholder and named that column the page's dominant left-edge texture; **L6** and **IA-32** put the same finding from the studio's and the wayfinding side. R126's 48px catalog crop is a *catalog-linked line crop* and is not a piece plate; it is unaffected (see below).

**Three. The owed figure outranks the agreed figure.** On the house page the figure a homeowner came to read is what she owes now, not what the project totals. The owed amount takes the block's one announced step, Playfair 26 (`.t-d2`); the agreed total sits under it in `.t-money`. The reconciling sentence between them is `.t-body` with its figures in `.t-money` spans, so the arithmetic reads as a sentence and not as a second table.

**What this amendment does NOT touch, stated so nobody reopens it.** R126's **colour-at-three-sites** clause (`:9992`) stands word for word: the filled stamps at ~1.18:1 with a charcoal word, the six saturated stage tabs printing as a small plate on a roster head, and 48px product crops on catalog-linked lines — three sites and nowhere else. R126's **elevation clause** (`:10023-10036`) stands entirely: one `--elevation-sheet` token at exactly three code sites, the CSS-level `lib/document/__tests__/shadow-gate.test.ts` with its frozen per-stylesheet inventory, and the `no-restricted-syntax` D4 rules in `eslint.config.mjs`. **No depth is adopted anywhere in this program** — PP-6 says so in as many words, and the shadow gate stays. R126's carve-out for THE STUDIO contents block on the desk (*"the currently deployed studio at the bottom of the desk looks great, dont change the typography"*) also survives: the sheet is applied surface by surface as each is touched, and that block is not being touched.

*Entries add: R140 · last id = R140*

### R141 · R135 amended — the landmark ledger, "Sign out", the colophon, and the consequence sentence — 2026-09-08

**Ruled by Kody, 2026-09-08** (**PP-1**, **PP-3**, **PP-5**; deck sheets 10, 11 and 12, specimen on sheet 14). R135 (`:10701-10722`) deleted the client page's fourteen-control header outright and put multi-project switching in the mat. Four things are added on top of that ruling and one of its smaller rulings is restated with its consequence made explicit. **R135's refusal is not reopened.** Its clause at `:10717` — *"The header is removed for every client, solo or multi-project. No wordmark, no project switcher, no nav bar, no More ▾, no right-side utilities"* — stands exactly as written, and nothing below is a header returning under another name.

**One. The landmark ledger is page furniture, not a header.** A short ruled list of the page's own landmarks sits **under the doorplate**, inside the page, scrolling with it: it names what is on this one page and jumps to it. It has no wordmark, no project switcher, no utilities, no fixed position and no second destination — every entry is an anchor into the page a reader is already standing on. That is the distinction R135's refusal turns on: a header is a bar of destinations that leaves; a ledger is a table of contents that stays. It is furniture in the same sense the colophon is.

**Two. "Sign out" replaces "Leave the house."** The house metaphor earns the page its shape; it does not get to obscure the one control a person looks for by name when she wants to be gone. *Leave the house* is a sentence about a house. *Sign out* is the thing she is trying to do. `mat.tsx:166` and `components/threshold/__tests__/mat.test.tsx:194` change together.

**Three. The colophon is page furniture on client surfaces.** PP-1 rules that the studio is the author and Patina is the press: every client surface opens with the studio's letterhead and closes with a colophon reading *"Prepared by {studio} · Sent through Patina"*, and **no Patina wordmark stands above it**. `/pay/<token>` and the settling sheet already print that exact line in two places; it becomes one shared component so the two cannot drift. **This binds client surfaces only.** PP-9 carves the Desk out by name: the designer portal keeps its PATINA wordmark and its footer identity, because a designer's working tool is not a letter the studio sends.

**Four. A consequence sentence stands above every terminal act, in every state.** One sentence, `.t-body`, directly above the filled act R139 rules — what will happen, to whom, for how much, and whether it can be undone — present when the act is armed and present when it is not. An act that is filled charcoal and carries an amount has already told the reader it is heavy; the sentence tells her what the weight is.

**Five. `--color-error` stays forbidden, and the token goes.** R135 already ruled that a refusal is body ink under a hairline everywhere on the page and that `--color-error` is red and forbidden by name. The grep this ruling asked for, run against the worktree:
...
Two of those four lines are not consumers: `open-chapter.test.tsx:242` asserts the token's **absence** from rendered markup, and `AvatarUploadField.tsx:130` is the prose comment recording R135's own removal. The declaration at `globals.css:58` therefore has **exactly one consumer left in the app** — `.da-danger:hover { color: var(--color-error) }` at `:373-375`. The condition this ruling set is met: **the token is removed at `:58`, and the `.da-danger:hover` rule that is its only reader is removed with it** rather than left pointing at a name that no longer resolves. A danger act rests and hovers in the same scored grammar as every other act (I107, as amended by R139); a refusal is body ink under a hairline. `--color-terracotta-ink` is not needed and is not adopted here. The CSS edit belongs to the house-page lane; this entry only rules it.

*Entries add: R141 · last id = R141*

### R142 · The client imagery doctrine — the source hierarchy, and a labeled concept render may lead a room — 2026-09-08

**Ruled by Kody, 2026-09-08** (**PP-4** and **PP-7**; deck sheets 10 and 13, specimen on sheet 14). This **extends R107 · The Room View — the room as material, in Patina's hand** (`:3765`, 2026-07-16) — the Room View entry, not the other `### R107` at `:8044`, which is *The fidelity ladder* and is a schedule vocabulary, unrelated to imagery. Two entries carry the same id in this log; this one extends the first.

**The source hierarchy, enforced by caption.** An image on a client surface ranks: an **installed photograph** of the thing itself; then the **studio's own** board image or scan-derived drawing; then the **maker's product photograph**; then a **drawn silhouette** of the piece. Every plate carries its caption underneath in `.t-meta`, sentence case, and the caption says **what it is · whose it is · when** — *"Study · photographed by Local Dev Studio · 3 September 2026"*, *"Reading chair · drawing by Local Dev Studio · photograph from Harmon Bench Works to follow"*. The caption is the enforcement: a picture whose provenance cannot be stated in that form is a picture that does not go on the page.

**Concept renders — studio-uploaded, labeled on the image, and permitted to lead.** A concept render is admitted only when the studio uploads it for that project. It carries a label **on the image itself**, reading **"Concept · not installed"**, at **≥14px** — not a 12px caption 440px below it, which is what **VC-40** measured: *"Your home, taking shape."* at 40px above a room that is not the client's home, corrected in 12px muted ink far enough down the page that nobody reads the two together. When a room has no installed photograph, a labeled concept render **may be the first image in that room's band**. That is the whole of the permission.

**What is never permitted.** Stock photography. Gradients and procedural fills standing in for material — **VC-38** found the proposal's "material sheets" were `repeating-linear-gradient` stripes at 95px, reading as corrugated card, on the slide that argues *give the materials room to speak*; **VC-39** found the one enlarged product preview, captioned "Solid oak table", was a photograph of a green-velvet dining room. Generated rooms from any source but the studio's own upload. And the hash block: **VC-44** counted five of six FF&E thumbnails rendering as the no-image placeholder, which made that column the page's dominant left-edge texture. The answer to an absent photograph is a **drawn silhouette** of the piece — inline SVG on `--paper-doc`, stroked in `--ink-faint` at 1px, secondary hatch at `stroke-opacity: .5` — never a diagonal hash, never a blank fill, never a broken-image glyph. **VC-45**'s empty rooms — Hall and Stair, drawn as outlined ~370×25px rectangles holding a tiny room name — become a room name, a floor line and one sentence. Never a rectangle.

**The eleven-pixel rendered floor binds drawn geometry.** **C01** found the room-band footprint labels set as `fontSize={11}` in **SVG user units** inside a `viewBox` 1000 units wide: at a 390px viewport, with about 358px of content, they render at **3.9px**. `plan-key.tsx:24-35` had already solved exactly this with a documented eleven-pixel floor and a phone crop, and `room-band.tsx` (`:53`, `:70`, `:230`) never got it. The floor is on the **rendered** size, not the authored number, and it binds every drawing on a client surface: a label that renders below 11px is not small, it is unreadable, and an unreadable measurement is a lie about the room. This is the honesty R107 already asked of low-confidence walls, applied to the type on the drawing.

*Entries add: R142 · last id = R142*

### I153 · The house sheet has a home — `docs/design/house-sheet/SPEC.md` — 2026-09-08

`artifacts/portal-polish-review-2026-09-08/specimens/SPEC.md` — §A as amended by §F — is copied **byte-identical** to `docs/design/house-sheet/SPEC.md` and that copy is the canonical one. A contract both portals build against does not live in an artifacts folder named for the week it was reviewed; artifacts are the record of a session, `docs/design/` is where a rule lives. Cite the `docs/` path from here on. Under **PP-5** the sheet is **the type and rhythm contract for both portals, applied surface by surface as each is touched** — it is not a licence to restyle a surface nobody is working on.

What the sheet holds:

- **Seven type steps**, named as classes — `.t-d1` 34/1.15, `.t-d2` 26/1.20, `.t-d3` 20/1.30 (Playfair 500, sentence case, tracking 0); `.t-body` 16/1.55, `.t-body-sm` 14/1.50 (Inter 400); `.t-meta` 12/1.50 sentence case and `.t-head` 11/1.50 UPPER (DM Mono, .08em). Two additions that are not new steps: `.t-authorship` (`.t-d3` in Playfair italic — asides and authorship only, never a heading, never a control) and **`.t-money`**, DM Mono 15/1.5, `tabular-nums`, .02em, carrying every ledger figure (§A3, §F-B). No inline `font-size`.
- **The 24px module.** Every block-level gap is 24 / 48 / 72, or 12 as the half. Nothing else. Page measure 1100px centred; prose capped at 65ch.
- **Three radii and only three**: 2px for marks and small plates, 3px for image plates, stage plates and the terminal act, 50% for the 7px mark dot alone.
- **Three paper stocks**: `--paper #FAF7F2` the ground, `--paper-doc #FCFAF6` a document laid on it, `--rail #E8E3DB` the rail and the deeper sheet — with a dark set that keeps every hue and lifts lightness, and `body` painted explicitly, never transparent.
- **State pigments only**, each as a material value and its own paper ink: clay, golden, terracotta, sage, plus `--oak #8B7355` as the resting-rule pigment (R139) and the four-step ink ramp `--ink` / `--ink-muted` / `--ink-subtle` / `--ink-faint`. **No new hex literal outside the sheet.**
- **No shadows** (`box-shadow` appears in no file the sheet governs; the one `--elevation-sheet` token is R126's, at its three sites, and none of the three specimens spends it), **no truncation** (`text-overflow: ellipsis` never appears — wrap), and a region with nothing to say renders nothing.

The sheet also carries the exact CSS for R139's four action tiers (§A5), the consequence sentence (§A6), letterhead and colophon (§A8), plates, captions and empty states (§A10), reduced motion and forced colors (§A11), and §F's fifteen post-review amendments. Where §A–§E and §F disagree, §F wins.
...
### R149 · The Standing Head — six rulings and a direction — 2026-09-10
...
### I154 · The Band built — the head of every spread, per R149 — 2026-09-10
