...
**00471 hold CLEARED 2026-08-12 (same evening)**: applied to Strata after the Field release carrying `cbe88574` shipped; reader-loss query returned 0. The `useSiteRequestActionDetail` action-detail hook is desilenced — `meta: { errorSurface: 'silent' }` and the `TODO(wave1-00471-held)` comment removed, restoring normal error surfacing.

**FF&E GA landed 2026-08-13 — closes the "three functions ACTIVE with no source on main" flag above.** The unmerged FF&E lane flagged 08-12 (`project-ffe-document-extract`, `project-review-media`, `selection-review-send`, deployed 08-10 out-of-band) is now on `main`: `ffe-ga/integration` merged via `01fdee48`, fast-forwarding main to `2482f3e6`. Source↔prod parity restored — `po-send` (v38) and the 3 FF&E functions (v5 each) redeployed from main in the 07:50–06:56Z window 2026-08-13, so the previously-orphaned prod functions now have a matching git tip. A new migration, **00473_board_cover_ffe_review_derivative_source**, applied to Strata to fix a live prod bug: 00462's guard rejected FF&E `<project>/source/...` cover references. Designer portal (`7b795a4a`, 07:02Z) and client portal (`03e8011d`, 07:03Z) deployed with FF&E GA UI live for the first time. Follow-ups owed: Kody's signed-in walk of the FF&E surfaces (designer add-to-project sheet + release ceremony, client review editions); extension picks this up on the next `extension-v*` tag; iOS `ProjectsAPIClient` rides the next app build; review-docket items — zero-FF&E-line POs are uninspectable by design of 00446 (product call, not a bug), the empty-state CTA steers to the spec book rather than the add-to-project sheet, and minor nits are filed in the session log. Worktree/branch hygiene done same session: `ffe-ga/land` and `ffe-ga/integration` (plus their upstream tracking branches `ffe-ga/data`, `ffe-ga/designer`, `ffe-ga/client-import`) deleted locally and on `origin` after ancestor-of-main verification.

**Schedule Fidelity Wave 1 (Direction A — "the collapse", R107–R114, I126–I128) merged + deployed 2026-08-13, Kody-authorized full chain.** `feat/schedule-fidelity-wave1` merged to main `--no-ff` at **`361f1cf8`** (parent `47b473fd`); touches `packages/utils/src/schedule.ts` + new `schedule-fidelity.ts`, `packages/supabase/src/hooks/use-schedule.ts`, and the designer-portal desk-derivation/desk-schedule/section-derivation/section-stage-line document-lib files, plus one migration. Pre-push gates green: `@patina/utils` 18/18 suites (557 tests) and `@patina/designer-portal type-check` clean. Pushed `origin/main` `eed8da17..361f1cf8`. Worktree `.claude/worktrees/agent-ac51b6876c7341f30` removed; `feat/schedule-fidelity-wave1` and its scaffold branch `worktree-agent-ac51b6876c7341f30` deleted (both ancestor-of-main verified first). **00474_schedule_template_comember_visibility.sql** was the only pending migration and applied clean to Strata via `supabase db push`. Object-probed post-apply via a schema-only `supabase db dump`: both `public.apply_phase_template(uuid,text,uuid)` (3-arg) and `public.seed_project_schedule_from_template` now gate through `public._can_author_proposal(designer_id)` with no remaining bare `designer_id = v_actor` check; the 2-arg `apply_phase_template` bridge (thin SQL wrapper calling the 3-arg form) is unmodified. Designer portal redeployed via `./infra/deploy-portal.sh designer` (prod `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` exported from `wrangler.jsonc` literals ahead of the run, since the checkout's `apps/designer-portal/.env.local` is currently local-pointed and the script's preflight guard correctly refused it otherwise) — Version ID **`652103ea-7b7c-4c59-9539-8daff3457f03`**, `wrangler deployments list` bottom row `2026-08-13T18:19:12.630Z` confirms freshness; unauthenticated behavior probe of `/auth/signin` returns 200. **Owed: Kody's signed-in Band/Week-N schedule walk** — nothing beyond the unauth probe above was exercised this session. The `PATINA_ALLOW_LOCAL_PROD_DEPLOY=1` override in `.claude/settings.json` (flagged in the FF&E GA entry above as owed-for-revert) is still in place and was relied on again this ship.
