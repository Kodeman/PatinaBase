...
**Storage path on upload** (unchanged from 00077/00287): every artifact is PUT to `{folder}/{userId}/{roomId}/{filename}` in the private `room-scans` bucket (500 MB limit; MIME list from 00077) — a **per-kind folder**, NOT a single bundle prefix. 00077 storage RLS reads segment `[2]`=`userId`, `[3]`=`roomId` (1-based), so both ids MUST hold those positions for every object. No new bucket (R-e). Bundle budget: **300–600 MB/room** (SC-12). The upload **Content-Type** must be in the bucket's `allowed_mime_types` — non-allowed semantic types (`.ndjson`, `.tar`) transport as `application/octet-stream` while their manifest `mimeType` stays semantic (§11 B-17).

**Storage layout — kind → folder map (B-18).** The authoritative source is the iOS `ScanUploadDescriptor.all` table (CaptureKit); the item-9 worker's `keys.KIND_TO_FOLDER` and this table MUST stay in lockstep. Column-less kinds are resolved server-side by prefix-swap off the manifest's verified `{userId}/{roomId}` (see B-18).

| kind | folder | filename | room_scans column |
|---|---|---|---|
| `usdz` | `usdz` | `scan.usdz` | `model_url` |
| `capturedRoomJson` | `captured_room` | `captured_room.json` | `captured_room_json_url` |
| `mesh` | `mesh` | `mesh.ply` | `mesh_url` |
| `bundleManifest` | `manifests` | `manifest.json` | `bundle_manifest_url` |
| `depthIndex` | `depth` | `depth_index.ndjson` | — (prefix-swap) |
| `scorecard` | `scorecard` | `scorecard.json` | — (prefix-swap) |
| `anchors` | `anchors` | `anchors.json` | — (prefix-swap) |
| `keyframeIndex` | `keyframes` | `keyframe_index.ndjson` | — (prefix-swap) |
| `keyframeSummary` | `keyframes` | `keyframe_summary.json` | — (prefix-swap) |
| `depthArchive` | `depth` | `depth.tar` | `depth_archive_url` |
| `keyframesArchive` | `bundle` | `keyframes.tar` | `scan_bundle_url` |
...
| B-19 | **Photos are a SEPARATE LANE — `photos_metadata.json` is device-local, NEVER a bundle `artifacts[]` entry.** Posed photos ride `room_scan_images` (uploaded to the `photos/{userId}/{roomId}/` storage folder); the manifest's top-level `photos[]` metadata array is the wire contract, cross-checked against `room_scan_images` (confirm-scan-bundle) and against the device-local `photos_metadata.json` sidecar (validator §10.11, only when the sidecar is present in the bundle dir). The iOS `FieldManifestAssembler` therefore does **not** list a `photosManifest` artifact, and no `photos_manifest_url` column is written. Item-9 ingest, if it ever meets an artifact kind it cannot resolve to a storage key (a legacy bundle's stray `photosManifest`, or any future unknown kind), **skips** it and emits an `ingest.kind_skipped` telemetry event — the validator then returns the honest verdict against the manifest rather than the worker crashing. | The item-8 uploader (`ScanUploadDescriptor.all`) never uploaded `photos_metadata.json`, yet the assembler listed it in `artifacts[]` — so every photo-carrying bundle fatal-failed server ingest at attempt 2 on `MISSING_FILE` (found on the M3 real-room pull of scan `95266be1`). Photos were always a distinct lane (`room_scan_images`, 00082) from the checksummed geometry bundle; conflating the two put a device-local file into the server's required-artifact set. Removing the `artifacts[]` entry (device still writes the sidecar for §10.11) is the minimal three-way fix: iOS stops listing it, the spec pins the contract here, and the worker degrades gracefully + audibly on any unresolvable kind. |
| B-18 | **Storage layout is PER-KIND folders, not a single bundle prefix.** Every artifact is PUT to `{folder}/{userId}/{roomId}/{filename}` in the `room-scans` bucket, where `{folder}` is the artifact's own root (00077 RLS: `[2]`=userId, `[3]`=roomId; the manifest is at `manifests/{userId}/{roomId}/manifest.json`). The kind→folder map is pinned below (§4 "Storage layout"). The server derives a column-less artifact's key by **prefix-swap**: take the manifest's verified `{userId}/{roomId}` segments and build `{KIND_TO_FOLDER[kind]}/{userId}/{roomId}/{basename(relativePath)}`. The item-9 worker owner-anchors EVERY derived key (segment `[1]`==row `user_id`, `[2]`==row `room_id` when present) and rejects a foreign prefix with `OWNERSHIP_VIOLATION` — its service key bypasses storage RLS, so this check IS the RLS-equivalent. | The shipped iOS uploader (`RoomScanStoragePath` + `ScanUploadDescriptor.all`) writes per-kind folders; a "dirname-of-manifest" model would resolve the five column-less kinds (`depthIndex`, `scorecard`, `anchors`, `keyframeIndex`, `keyframeSummary`) into `manifests/…` and 404 on every real bundle. Pinning the map as a contract (spec + worker `KIND_TO_FOLDER` + iOS descriptor table, cross-cited) keeps device-write and server-read in lockstep. Owner-anchoring closes the service-key hole: without it a doctored `room_scans` URL column could point the worker at another user's object. |
```
