...
“What this spike does not prove” pass.
...
- [COLMAP 4.0](https://github.com/colmap/colmap/releases) integrated GLOMAP as
  `global_mapper`; the validated current release for this design is **4.0.2**.
...
The future box installer should pin and probe COLMAP 4.0.2 (or a separately
qualified later 4.x release). It should remove any assumption that a standalone
`glomap` binary is the supported runtime.

## Field v3 bundle seam

The production capture code is decisive:
...
The refine adapter must never apply another orientation transform. It may copy
the HEIC or decode the already-rotated raster into a COLMAP-supported format,
but the decoded raster must remain `width = nativeHeight` and
`height = nativeWidth`.
...
This is intentionally `H - cy`, not `H - 1 - cy`. The unit fixture uses an
off-centre principal point and projects a 3D ray through both bases so an
off-by-one or a dimension-only swap fails.
...
1. **Start one stage deadline.** Create `RefineDeadline` at handler entry. Its
   engine deadline is 3,000 seconds from that one monotonic start, leaving 600
   seconds inside the 3,600-second visibility lease. Every subprocess receives
   only `deadline - now`; never create a new 50-minute timeout per command.
2. **Materialize images without rotating them.** Verify each source checksum and
   the right-rotated dimensions from the index. If the qualified COLMAP build
   reads HEIC, copy/link it. Otherwise decode the physical raster to PNG with no
   EXIF/CI autorotation and verify the same encoded dimensions. The materialized
   name becomes the canonical COLMAP image name.
...
8. **Refine.** Run global `bundle_adjuster` over the triangulated model with the
   ...
   raw ARKit output when BA fails.
...
## Metric alignment and drift definition
...
The exact reported metric is:

```text
e_i                    = y_i - (s R x_i + t)
rmse_m                 = sqrt(mean(||e_i||^2))
trajectory_rms_radius_m= sqrt(mean(||y_i - mean(y)||^2))
sfm_residual_pct       = 100 * rmse_m / trajectory_rms_radius_m
mean_translation_drift_pct
                        = 100 * mean(||e_i||) / trajectory_rms_radius_m
```

The 0.2–0.5% P2 target applies to `sfm_residual_pct`. RMS trajectory radius is
the denominator because it is invariant to keyframe cadence; total path length
would change merely by recording more frames. Persist RMSE, RMS radius, mean
drift percentage, and maximum drift metres so the percentage is auditable.
...
adapter-v1.json
pairs-v1.txt
adapter-manifest-v1.json       # commit marker, always last
```
...
The local prototype implements this with a temporary file plus atomic hard link.
The storage implementation should use create-only upload (`upsert=false`) and,
on conflict, compare the existing object's checksum before treating it as an
idempotent replay.
...
4. Complete refine only after both successor enqueue attempts succeed. It does
   not enqueue `present`.
5. Present remains an independent data-gated join. Only Present may mark ready,
   after dense mesh, measure mesh, and splat URLs all exist.
...
branches pass their manifest key/checksum when they conflict-ignore enqueue the
same Present task. Only Present composes `room_files.present` once from the three
verified immutable manifests and changes the scalar lifecycle to `ready` with
`presented_at`. A fatal branch/Present failure may set `present_status='error'`;
ordinary branch progress is represented by events, not competing scalar writes.

## Executable proof

`services/scan-pipeline/tests/test_refine_adapter.py` covers:

- off-centre physical-right intrinsics and equivalent 3D ray projection;
- ARKit ↔ COLMAP pose/axis conversion and inverse;
- exact known Sim(3), residual, drift denominator, and degeneracy rejection;
- deterministic temporal + ARKit-spatial loop pairing;
- deterministic connected-coverage/verified-loop fatal classification;
- full-pose primary versus position-only fallback contract;
- versioned source/output checksums without mutating HEIC bytes;
- immutable idempotent and conflicting concurrent writers, manifest last; and
- one shared absolute stage deadline with a 600-second completion reserve.

## What this spike does not prove

The following are hard gates before deployment:

1. **COLMAP 4.0.2 box probe:** install/qualify the supported COLMAP build and
   execute the exact PyCOLMAP database/model calls above on a tiny fixture. Save
   `colmap -h`, Python API version, GPU SIFT result, IDs before/after camera
   rewrite, and the triangulated model as evidence.
2. **HEIC materialization probe:** prove whether that build reads Field HEIC.
   If not, bless a no-autorotate decoder and prove output pixel dimensions and a
   projected off-centre feature against the source raster.
3. **Real geometric proof:** run read-only/local scratch refinement on the
   ...
   verified loop edges, runtime/VRAM, and the exact metric. No DB/storage write.
...
5. **Artifact consumer contract:** Fuse and Splat must read the same aligned pose
   schema/checksums before the fork is enabled.
