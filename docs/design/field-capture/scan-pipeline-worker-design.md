...
- **The fork.** `refine`, on success, enqueues BOTH successors —
  ...
  refine.id`. Both stable successor payloads carry the same immutable refine
  manifest key + sha256. The two then run independently (concurrently on two GPU
  leases, serially on one — §10.9). A crash between enqueues re-runs `refine`;
  deterministic artifacts and idempotency keys converge.
...
- **The join payload is identical and order-independent.** BOTH terminal branches
  (mesh solve-upgrade and Splat) call the same constructor. Payload is stable IDs
  only: `{scan_id, room_file_id, room_file_version, user_id, refine_task_id}`;
  task key is `'{scan}:present:{v}'`; both use `p_on_conflict='ignore'` and the
  common `parent_task_id=refine_task_id`. Branch task IDs, manifest keys/checksums,
  timestamps, and results are forbidden from the enqueue. The durable Present
  row is identical whichever branch finishes first.
