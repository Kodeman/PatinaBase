...
### I14 · Prod ship (FULL) — Strata + Cloudflare — 2026-07-17

Strata: 13 migrations in one linear `db push` — BOH 00350–00354/00358/00360–00364 plus 00341 + 00370 (Field Capture P1, **schema only, no FC-P1 app code**), the latter two by Kody's explicit ruling. Verified: 18 BOH tables, `ledger_accounts` 15 rows, `fulfillment_config` 7 keys, `fulfillment_intake_order` RPC, `fulfillment_events` in `supabase_realtime`, 3 fulfillment crons (intake `*/1`, events-mirror `*/5`, stripe-recon `20 3`). 8 BOH edge functions deployed (unauthenticated probes 401/403/400/405, no 5xx); crons succeeding from the first minute (`fulfillment-intake` 9+ consecutive runs, `-events-mirror` green, FC-P1 `scan-pipeline-ingest-sweep` green). Admin `NEXT_PUBLIC_ENABLE_FULFILLMENT=true` (commit `383c46a1`). **Portals deployed**: admin worker version `1417ca42` + client `fd120102` (Cloudflare Workers via deploy-portal.sh); admin `/fulfillment` gates unauthenticated to signin (307), the flag confirmed baked into the worker env, ~80-request tail clean of errors. A latent `@patina/fulfillment` exports-map gap (config-form/vendor-form/transmission-log subpaths undeclared) that blocked the first admin build attempt was resolved out-of-band by the concurrent Field-Capture session refactoring consumers to barrel imports (shipped HEAD `1b4ce665`); no exports edit was needed. Secrets all pre-provisioned and verified by name on Strata (APNS_*, STRIPE_*, RESEND_API_KEY, POSTHOG_API_KEY — the phc_ project token, after a phx_ personal key was set by mistake and flagged for revocation). Backend live but dormant — prod has no seed data by design; `vendor_profiles` and orders start empty, the chart and config are in-migration. **Owed**: Kody's authenticated prod walk of the fulfillment zone (magic-link); the first live `orders@patina.cloud` PO send (closes O3); the first delivered push (closes O1); a live Stripe reconciliation exercise once real balance transactions exist; Leah's first deck session (opens the L-family).
