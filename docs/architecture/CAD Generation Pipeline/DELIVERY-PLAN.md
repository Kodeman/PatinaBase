...
**As shipped:** the routes add no authorization of their own. They verify the caller's Supabase JWT, then read inside one `SET LOCAL ROLE authenticated` transaction on the uncached `DB_FRESH` binding, so `room_files`' delegation to `room_scans` (00341) and `media_objects_select`'s delegation to the same (00489) are what decide visibility — the caller's own RLS, not a route-side predicate. There is no `scan_reader` role or `scan_media_read` view on this path: that view remains a *service* capability with no tenant predicate, and 00490 says a login role must not inherit it before one exists. `scan_reader` stays an unprovisioned service surface for a future non-interactive caller, not part of the interactive read path. Every negative (missing/invalid Room File, wrong kind, unconfirmed object, a Room File the caller cannot see) collapses to an identical 404 so a policy can never prove an object exists without binding the caller — the mood-board bug class this plan gates against. Missing/invalid JWT is 401. ACL conformance coverage and the negative-404 tests are required (`infra/edge-api-worker/test/scan-route.test.ts`).
