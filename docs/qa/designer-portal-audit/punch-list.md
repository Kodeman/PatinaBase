...
- **SMOKE-04 remainder + SMOKE-05** — IN PROGRESS: **3085 → 80** real type errors. Done so far: (1) **rooms/viewer r3f ~40** — fixed with a type-only `React.JSX` re-augmentation shim (`src/types/react-three-fiber.d.ts`) instead of a risky r3f v8→v9 upgrade. (2) **`@patina/api-client` 15** — was missing from the app tsconfig `include` (TS6307); added. (3) per-file bugs fixed: `lib/permissions.ts` (23 — was the wrong `UserRole` junction interface instead of `UserRoleName`; **security-relevant**), `use-search.ts` (10 — `CatalogSearchApiResponse` union collapsed to a permissive wire shape), **all email-builder props-forms** (~38 — block-prop types weren't top-level re-exported from `@patina/types`), **all 10 `api/projects` routes** (31 — dead `@/auth` import + untyped `context`), `'destructive'`→`'error'` variants (10), framer-motion `Variants` annotations (8). **Remaining ~80**: a scattered long tail — Badge variant-API mismatches, `response`-is-unknown casts, `ValidationVote` semantic mapping (`agree/disagree/skip` vs `confirm/adjust/flag`), misc per-file TS2322/2339. Plus migrate deprecated `next lint` → ESLint CLI (SMOKE-05). `tsc` won't exit 0 until the long tail is cleared.
- **SEC-06/07 Help content** — LARGELY DONE: (1) authored + published 5 starter `helpContent` section-intro docs to `kv3qrinl/production` (decisions/clients/financials/pipeline/products), all mapped to real SectionIntro consumers; decisions intro verified rendering live. (2) **Fixed the schema↔consumer mismatch**: `useHelpContent` now projects `coalesce(flat, nested...)` so both flat AND Studio-nested (`tooltipContent.body` etc.) content render — verified flat still works + a nested test doc projects correctly. So Studio authoring of the ambient/reactive layers (tooltips, field helpers, empty states, learn-more) now works. (3) **Aligned the reference layer too**: `ContextualHelpPanel` (was flat-only) and `HelpSearch` (was nested-only) now `coalesce(flat, nested)` — matching `RelatedArticles`'s normaliser — so Help Center articles render regardless of authoring shape; both GROQ queries verified executing (search now surfaces the flat section-intro docs it previously skipped). **Code alignment is complete across all help layers.** **Remaining (non-code):** deploy the `helpContent` schema to the dataset so docs are editable in the Studio UI; and author the broader content library (ongoing content task).
