...
- **SEC-06/07 Help content** — PARTIALLY DONE: authored + published 5 starter `helpContent` section-intro docs to `kv3qrinl/production` (decisions/clients/financials/pipeline/products) — all mapped to real SectionIntro consumers; verified the decisions intro renders the CMS copy live. **KEY BLOCKER (code, not content):** the help-system frontend reads **flat** fields (`data.body`, `data.heading`, article `title`/`body`) but the studio `helpContent` schema **nests** them (`tooltipContent.body`, `emptyStateContent.*`, `helpArticleContent.*`) — so anything authored via the Studio UI won't render. Fix = align the two (either flatten the schema in `studios/help-system/schemas/helpContent.ts`, or project nested→flat in the frontend GROQ/components). Until then, content must be authored flat (as the starter docs are). Also: the help-system schema isn't deployed to the dataset (only marketing types are) — Studio editing of these docs needs `helpContent` deployed.
