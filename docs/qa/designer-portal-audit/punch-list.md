...
- **SEC-06/07 Help content** — LARGELY DONE: (1) authored + published 5 starter `helpContent` section-intro docs to `kv3qrinl/production` (decisions/clients/financials/pipeline/products), all mapped to real SectionIntro consumers; decisions intro verified rendering live. (2) **Fixed the schema↔consumer mismatch**: `useHelpContent` now projects `coalesce(flat, nested...)` so both flat AND Studio-nested (`tooltipContent.body` etc.) content render — verified flat still works + a nested test doc projects correctly. So Studio authoring of the ambient/reactive layers (tooltips, field helpers, empty states, learn-more) now works. **Remaining:** the reference-layer (Help Center articles) queries in `ContextualHelpPanel`/`RelatedArticles`/`HelpSearch` still read flat `title`/`body` — apply the same nested→flat projection there for Studio-authored articles; deploy the `helpContent` schema to the dataset for Studio editing; and author the broader content library (ongoing content task).
