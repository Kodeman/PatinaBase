...
| S2 | Consent and suppression behavior is not a phone-global, sender-bound state machine. | **S2. Consent codes: same code space, kind='optin', one outstanding challenge per engagement version, bound to the normalized recipient. `YES NN` grants that engagement only; bare YES with more than one open challenge never grants and asks which. STOP suppresses the recipient phone on that sender number globally (carrier semantics; independent of party rows; survives adding a new party); START re-asks eligible pending challenges and never grants. Suppression wins over any party consent column.** |
...
| S9 | Later migrations may already have higher numbers when a ticket begins. | **S9. Migration numbers are allocated by the orchestrator on the ticket (00591–00599 for Phase 0). Keep the allocated number even if a higher number exists when you start; report a collision instead of renumbering.** |
...
| START from a number with no prior consent | Existing behavior can grant broadly or ambiguously. | Fixture proves START only re-asks eligible pending challenges. |
