...
The implementation preserves the original inbound delta on the independently accepted atomic, selection/callback and availability foundations. Context migration, pause and digest-ref hookup remain the subsequent inbound slice (SQ-25); `report_condition` is owned review, not an applied effect here.
