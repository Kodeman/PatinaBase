...
**Where it lives:** All three surfaces are native/browser clients of one Supabase project (self-hosted at api.patina.cloud), with Supabase Auth as the single session source of truth. None of the three route through the retained NestJS services (`orders`/`media`/`projects`) for capture or sync — everything is direct-to-Supabase (Storage + PostgREST + RPC + edge functions). The Patina companion app is a mature, prod-wired app; the Field Capture app and Chrome extension are the newer, designer-facing investment area, reflecting a pivot in the native roadmap from consumer-first to designer-first tooling.

## 3. As-Built Architecture

### 1. Patina companion app (`apps/mobile/Patina/Patina/`)

- **Backend targeting**: `Services/API/APIConfiguration.swift` — `DeploymentTarget.current` defaults to `.selfHosted` → `https://api.patina.cloud` (Kong), portal `app.patina.cloud`, storage `storage.patina.cloud`. `Endpoint` enum maps GoTrue auth, PostgREST tables, storage, and the three companion edge functions.
...
- **Config**: `App/Configuration/AppConfiguration.swift` → `https://api.patina.cloud`, buckets `capture-media` + `product-images`, app-group `group.cloud.patina.capture`, URL scheme `capture://`.
...
- ⚠ **Stale Supabase Cloud config in `APIConfiguration.swift`.** Hardcodes a stale Supabase Cloud realtime ref (`wss://bkvcixdmuyejfzcijpdg.supabase.co/...`) in the `.cloud` branch; the app runs `.selfHosted` by default so this is dead/misleading config.
