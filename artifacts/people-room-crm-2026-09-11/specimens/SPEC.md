...
| 7 | Region "Jobs": "Okonkwo residence · Dana Kowalski · On the job" with the stage word, and a read-only line "Draw 1 waiver ledger, in the money book" as an inline act. **Specimen only** (amended 2026-09-13, CR9-3): the SHIPPED card has no money book to open — no such surface exists in this build, and R-B already put the money papers in a later room — and `project_parties` carries no draw or waiver column to number a draw from, so the card prints the read-only sentence "Waiver ledger and draw state, in the money book." with no control. The inline door, and the draw number in front of it, are **W3 money-book work** |
...
| 2 | Kind switch, `role="group"`, eight words: a client, a household member, a maker, a GC, a sub, an installer, a receiver, someone else. "a sub" pressed. Rendered as bordered chips per §2.3 (`.pick` at 1440, `.narrow` at 390; pressed = `--rail` ground, `--ink-faint` border, `--ink` text). |
