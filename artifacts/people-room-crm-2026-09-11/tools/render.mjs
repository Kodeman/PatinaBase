...
(async () => {
  ...
  const rootDir = resolvePath(scriptDir, '../../..');
  const playwrightModulePath = '/Users/kody/Code/patina-merged/node_modules/.pnpm/playwright@1.58.2/node_modules/playwright/index.mjs';
  ...
  try {
    ...
    // Write console.json. Merge with any run that wrote the same filename first
    // (the 1440 and 390 commands share one baseName), so the report holds every
    // capture of the pair rather than only the last run's half.
    const consoleJsonPath = `${options.out}/${baseName}-console.json`;
    let merged = captures;
    if (existsSync(consoleJsonPath)) {
      try {
        const prior = JSON.parse(readFileSync(consoleJsonPath, 'utf8'));
        if (prior && Array.isArray(prior.captures)) {
          const fresh = new Set(captures.map(c => c.file));
          merged = prior.captures.filter(c => !fresh.has(c.file)).concat(captures);
        }
      } catch (e) {
        // Unreadable prior log: this run's captures stand alone.
      }
    }
    writeFileSync(consoleJsonPath, JSON.stringify({ captures: merged }, null, 2));
    console.log(`\nConsole log: ${baseName}-console.json (${merged.length} captures)`);
