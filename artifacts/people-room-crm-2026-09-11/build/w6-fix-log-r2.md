...
**Fifth amendment (this commit, closing the ef2712454 re-review’s F-A1 / F-A2 / F-A3 / F-A4 / F-A5).** Four amendments had patched four separate predicates for one question — may this caller record an authority on this job — and each re-review found the next seam between them. They are one derived value now, `authorityStanding` (`'unread' | 'none' | 'member' | 'admin'`, add-person-sheet.tsx:601-611), and every reader reads it: the band’s render (`!== 'none'`, :1772-1779), the disabled scopes (:1849-1852) and the owner/admin notice (:1863) at `'member'` only, the scope snap-back at `'member'` only (:674-681), the act’s hold (:632-640), `submitParty`’s three guards (:901-940) and the grant write (`if (grantRequested)`, :1137), with `isOrgAdmin` now just `authorityStanding === 'admin'` (:659). What that closes. A phrase typed before the book resolved NULL stayed in state after the band unmounted and the third guard then refused the WHOLE add — the seat included — with no field left to clear it in; the sheet now clears the grant as the band goes (F-A1). An unresolved BOOK under a RESOLVED list read as 'no standing', so the notice asserted a refusal, money and draw_certify rendered disabled and the snap-back took back a scope the studio had chosen — an unread book is 'unread' whatever the list says (F-A2). A book kept by a studio the caller holds no ACTIVE membership in (`useOrganizations` returns active memberships only, use-organizations.ts:167-169; all four 00624 authority policies gate on `is_active_studio_member(project_party_recorded_studio())`) offered every scope and met the refusal after four writes — that standing is 'none', with no band and a sentence in its place (F-A3). The third guard lacked the `authorityOpen` conjunct the writer carried; both now read the one `grantRequested` object (F-A4). And the band’s absence on a job with nowhere to record an authority was silent — one sentence stands where the band was (F-A5). **Two sentences are owed Kody’s wording ruling:** “This job isn’t attached to a studio yet, so there is nowhere to record the authority.” (carried over from the fourth amendment, still unruled) and the new “You’re not on the studio that keeps this job’s book, so there is nowhere to record the authority.” — composed in the sheet’s voice from the first, not ruled. One deviation from the brief’s table, flagged: the NULL-book row is evaluated BEFORE the either-query-undefined row, because a resolved NULL book answers the question on its own and the brief’s literal order re-opened the band on a no-studio job whenever the membership list was unreadable — the fourth amendment’s own case (“offers no authority band at all where the job records no studio”, which sets `orgsState.isError`) fails on that order. Tests: the F-1 case that expected the whole add refused on a grant carried onto a no-studio job was repaired to the behaviour the clearing effect makes true (the grant is cleared, the sentence stands, the seat still writes); three cases added — no standing asserted on an unreadable book whatever the list says (F-A2), no band and a cleared grant where the caller is not on the studio that keeps the book (F-A3), and the snap-back plus notice on a standing that read back as a member’s. The orgs mock now carries a settable list, so a case can return a resolved list WITHOUT the recorded studio and with roles. Gates: `pnpm --dir WT/apps/designer-portal exec jest src/components/document/people src/components/document/roster` — 43 suites, 714 tests, all passing; `pnpm --dir WT --filter @patina/designer-portal type-check` clean. No server, database or Playwright command was run for this pass.

**Sixth amendment (this commit, closing the d062a2642 re-review's F-B1 / F-B2 / F-B3).** The fifth amendment's one standing value was read in one place it could not answer: BEFORE A JOB IS CHOSEN. With `projectId` empty, `useProjectRecordedStudio(null)` is disabled (`enabled: Boolean(projectId)`, use-coordination.ts:2332-2346), so `data` is `undefined` with `isLoading` false — 'unread' standing, for the one reason no press can resolve. Three consequences, all fixed together (F-B1, major): the band was offered on a sheet with no job, so a phrase typed into it held the act on “Couldn’t read your standing…” (`recordedStudioLoading` is false for a disabled query, so even the sentence was the wrong one); the held press ran `onHeldActivate`, never `onClick`, so the refusal that is actually true there — “Field crew work a project — pick which one they’re on.” (add-person-sheet.tsx:967) — was unreachable; and `refetchRecordedStudio()` on the DISABLED query still runs its `queryFn` (TanStack v5's `executeFetch` ignores `enabled`), which returns `null` for a null project, flipping the standing to 'none' and letting the clearing effect destroy the typed input. The band is now rendered only with `!!projectId` (`:1802`) — and with no job NOTHING stands in its place, since neither 'none' sentence is true of a job nobody has chosen and the project select above is the thing to use; `!!projectId` also joins the standing branch of `heldReason` (`:648`) and `submitParty`'s second guard (`:936`), so the press falls through to the pick-a-project refusal; and `onHeldActivate` refetches the recorded-studio query only with a project set (`:2090-2098`). F-B2 (minor, one word): `is_active_studio_member` requires `status = 'active' AND role <> 'guest'` (00417:40-55 — the guest exclusion is deliberate, a guest seat is a courtesy login that must not open the studio's book), while `useOrganizations` filters on status alone (use-organizations.ts:167-169), so an active GUEST reads back in the list; list-presence was never the predicate. The derivation is now `if (!org || org.membership?.role === "guest") return "none";` (`:612`), and both the docstring (`:590-602`) and the R-CD row's claim that list-presence equals `is_active_studio_member` are corrected. F-B3 (minor): the `|| recordedStudioLoading` conjunct in the standing sentence choice was dead wherever it could be reached — the recorded-studio branch returns first on every unresolved book — and is dropped from both the hold (`:651`) and the guard (`:938`). **Tests.** Two cases added to the “the recorded studio, while it is still resolving” describe: no band and no stand-in sentence before a job is chosen, with the press yielding the pick-a-project refusal and no `refetch`; and a guest on the studio that keeps the book read as 'none' — the not-a-member sentence in the band's place, the seat still written, `setAuthority` never called. Both were confirmed to FAIL against the pre-fix source (the render gate and the guest check reverted in place: “2 failed, 42 passed”) before being confirmed green. One existing case was repaired: “R-J — says plainly that nothing defaulted, and offers the act (CR-16)” asserted the band on a sheet whose describe-level `beforeEach` picks no project; it now picks one first. The recorded-studio mock also honours the argument the sheet passes (`data: undefined` when the project is null), which is what the real disabled query does and what makes the F-B1 case stand where the studio does. **On the fifth amendment's snap-back case** (“snaps an admin-only scope back once the standing reads back as a member’s”): it is a COVERAGE ADDITION, not a regression proof — it passes against the fourth amendment's predicates as well as the fifth's, so it documents the snap-back rule rather than demonstrating the seam the fifth amendment closed. **Gates.** `pnpm --dir WT/apps/designer-portal exec jest src/components/document/people src/components/document/roster` — 43 suites, 716 tests, all passing; `pnpm --dir WT --filter @patina/designer-portal type-check` clean. No server, database or Playwright command was run for this pass.

**Seventh amendment (this commit, closing the 7199eab6a re-review's residual R1 / R3 / F-2).** The sixth re-review returned CLEAN — the cluster's substance holds — and listed R2 and R4–R9 as owed minors, still owed after this commit. Three were taken. **R1 (the clearing effect missed the band's second unmount path).** The sixth amendment's own `!!projectId` render gate (`add-person-sheet.tsx:1832`) gave the band a way off the page that no standing change accompanies: clearing the project select back to “Which project…” unmounts the band while `authorityStanding` still reads whatever the last job said, so `authorityOpen`, `authorityPhrase` and `authorityThreshold` were stranded invisibly and reattached to the NEXT job picked — one client's authority carried onto another's work, on a field nobody could see. The effect's condition is now `if (authorityStanding !== "none" && projectId) return;` (`:715`), `projectId` joins its dep array (`:742`), and its docstring says both paths clear alike. **R3 (the money / draw_certify refusal was the only one in the cluster still inside the `try`).** `submitParty`'s first three refusals stand before `setError(null)` and every write; PR-n's admin-only gate did not — it `throw`s at `:1181`, reached only after the seat, the card, the channels, the rule and the affiliation are written, and caught below as the generic “Could not add them just now. Try again.”, with `chainRef` resuming into the same refused grant on every retry. A fourth guard now stands with its siblings (`:1000-1008`): `grantRequested && isAdminOnlyAuthorityScope(authorityScope) && authorityStanding !== "admin"` sets the same sentence and returns. **The in-`try` copy is KEPT**, as defence in depth for any future caller that reaches the chain another way. **The hoisted guard is not reachable from this sheet's UI, and no test was written for it.** With `grantRequested` true, a press can only arrive at it holding an admin-only scope on a non-'admin' standing, and every route there is closed earlier: at 'none' the band is gone and the clearing effect has emptied the scope (and guard 3 refuses regardless); at 'unread' guard 2 refuses first; at 'member' the scope select renders the two admin-only options `disabled` (`:1912-1915`) AND the snap-back effect (`:689-696`, keyed on `authorityStanding`, `authorityScope` and `defaultAuthorityScope`) resets a chosen one to the kind's default on the commit that follows the change — including the unread → member transition, where the option was not disabled when it was picked. The effect runs before any press can be dispatched, so no UI sequence delivers a press in that state; a test would have to drive state the sheet does not expose, which is a test of the test harness, not of the sheet. **F-2 (test narrative, `add-person-sheet-kinds.test.tsx`).** The guest case's closing comment claimed “The seat itself is unaffected — it is the GRANT a guest cannot record.” That is false in production: `project_parties_studio_insert` (00584:884-893) gates on `is_studio_comember`, which admits no guest on either side of the shared membership (00315:44-48), so a guest on the studio that keeps the book is refused the SEAT as well. The comment now says what the case actually asserts — that the sheet drops the grant and writes the seat as it would for anyone, that the DB refuses that seat, and that the assertion is about the sheet only. **Tests.** One case added to the “the recorded studio, while it is still resolving” describe: “clears a grant stranded by clearing the project select” — pick a job, open the band, type a phrase and a figure, clear the project select (band and both fields gone), pick the same job again, and the band is back with `aria-expanded="false"` and both fields empty. It was confirmed to FAIL against the pre-fix effect condition (reverted in place: `aria-expanded` received `"true"`) before being confirmed green. No test for R3, per the reachability finding above. **Gates.** `pnpm --dir WT/apps/designer-portal exec jest src/components/document/people src/components/document/roster` — 43 suites, 717 tests, all passing; `pnpm --dir WT --filter @patina/designer-portal type-check` clean. No server, database or Playwright command was run for this pass.

Eighth amendment (this commit): text corrections, guard 4 project conjunct, sentence hoisted; the A→B carry is owed a ruling.
