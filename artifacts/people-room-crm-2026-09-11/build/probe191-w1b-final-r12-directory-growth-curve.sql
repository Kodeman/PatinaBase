\pset pager off
-- probe191 (r12) — where does people_directory's residual cost cross the
-- 8s statement_timeout `authenticated` carries in its own rolconfig?
BEGIN;
...
BEGIN
  ...
  FOREACH k IN ARRAY ARRAY[0,200,200,200,200,400,400] LOOP
    IF k > 0 THEN
      INSERT INTO public.studio_contacts (organization_id, entity_kind, contact_kind, full_name, created_by)
      SELECT 'b0000000-0000-0000-0000-000000000001','person','sub','Scale P '||k||'-'||g||'-'||clock_timestamp()::text,
    ...
    BEGIN
      ...
      t1:=clock_timestamp();
      RAISE NOTICE 'cards=% seats=% -> SELECT * : % rows in %', cards, seats, n, t1-t0;
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS msg = MESSAGE_TEXT;
      RAISE NOTICE 'cards=% seats=% -> SELECT * RAISED % (%)', cards, seats, SQLSTATE, msg;
    END;
