...
| W6 | Integration + Chrome QA on a local prod build | **DONE** — rebase complete onto `origin/main` `c879118ec` (166 ahead), final HEAD `30e06ab9f`; pre-deploy minors landed 2026-09-16 (patina-merged-73) on top. Gates green except origin/main-proven pre-existing reds (`schedule-region-head.test.tsx`, `fulfillment-po` deno TS2345, `stripe-rail.test.ts` seed, `field-coordination`/`library-configuration`/`action-visibility` specs, ~30 `lens-*` infra-gap). Findings: F1/F2/F4/F4-new **FIXED** and re-verified; F5 (`pay-link.spec.ts:571`) **FIXED at `0f29187f3`** (`w6-fix-log-r2.md` Item B, stale fixture); `pay-link.spec.ts:470` (return-origin stamp missing, flagged by that same pass) and the PWA `NetworkOnly` regex missing `paperwork` (`w6-qa.md` §4's own `F4`, distinct numbering from this table's F1–F5/F4-new) **FIXED in the pre-deploy-minors commit** — `tests/pay-link.spec.ts` 7/7 chromium, `public/sw.js` regex verified after a real `pnpm build`. `e2e/document/hours.spec.ts:60/72` **reclassified pre-existing, environment-flaky, NOT ours** (`w6-fix-log-r2.md` "W6 follow-up triage" Item A — product code byte-identical to `c879118ec`; root-caused to a `router.replace` RSC fetch `net::ERR_ABORTED` in headless Chromium, not an app defect). **One open item remains: F3** (console fetch races on `/desk`/`/people`, minor) — to be re-checked against a Strata-backed build or prod. **Full e2e re-run completed 2026-09-16** against local-prod builds at HEAD `91205b61b`: designer 277 passed / 67 failed / 34 skipped / 16 did not run (394 total); client 53 passed / 4 failed (57 total). Of the 71 total reds, **6 are OURS** (5 designer `e2e/people/**` test-authoring gaps — dynamic-label races, an `Authority`-field visibility gap, a `role="alert"` strict-mode collision vs Next's own route-announcer, and one genuine not-yet-fixed defect at `person-card.spec.ts:74` where a "do not contact, routed to" Directory row doesn't render its mailto link; plus 1 client `design-build-door.spec.ts:490` gap in the project-less deposit-invoice `letterbox.tsx` this program built) — all recommended as follow-up tickets, none fixed here per this task's no-product-fixes scope. The remaining 65 reds are pre-existing (documented `studio_id_not_designer_studio` trigger-tightening reds, ~28 unrelated legacy/scaffold test-debt, a command-palette contention cluster, and 3 same-session DB-pollution artifacts from repeated non-reset runs). Full classification table, failure text, and root-cause evidence in `build/integration-report.md` §10. Deploy inventory, migration `--include-all` requirement, and the 34-function edge-deploy set (6 needing `--no-verify-jwt`) also recorded there. **Final clean single-pass re-run 2026-09-16 (HEAD `a32d557bf`, `--workers=1`, DB reset fresh before each suite, no services running):** designer 279 passed / 64 failed / 51 skipped (394 total); client 54 passed / 3 failed (57 total). Of the 67 total reds, **9 are OURS**, all designer, all exercising `desk/page.tsx` or `doc/[id]/page.tsx` (both touched by this program's `call-sheet` flag retirement) or new `e2e/people/**` spec files this program wrote — `desk-walkthrough.spec.ts:152`, `help-panel.spec.ts:73`, `margin-handoffs.spec.ts:154`, `wave2-screenshots.spec.ts:24`/`37`, `wp3-screenshots.spec.ts:153`, `add-client-letter.spec.ts:47`/`123`, `bring-forward.spec.ts:264`. The remaining 58 are pre-existing (the `studio_id_not_designer_studio` trigger cluster, 20 legacy `catalog/**` reds against a relocated `/catalog` route, 21 legacy `crm/**` reds against a dead `/crm/kanban` + `/api/test/cleanup`, and 6 other diff-stat-empty reds including the client's `threshold.spec.ts:354`, now confirmed to reproduce even on a clean single pass — superseding the earlier same-session-pollution theory). Full table in `build/integration-report.md`'s "Final clean full run on a32d557bf" section. |
| W7 | Single deploy chain — starts with the email-deliverability checklist (`artifacts/people-room-crm-2026-09-11/build/email-deliverability-checklist.md`) | **DONE** — 21 migrations (`00592`–`00594`, `00621`–`00638`), 34/34 edge functions (`paperwork-upload` new), designer portal `466fc671-…`, client portal `6f41b63e-…`, 18 Sanity help docs (17 created + 1 replaced in place) all live and independently re-verified (`verify-report.md`); TestFlight build 5 VALID, not yet tester-assigned. Program closed out in `build/ship/report.md`. |
