...
BEGIN
  ...
  RAISE NOTICE 'D-a BEFORE merge: seat=% job=% firm=% last_touch=%', r.person_id, r.job, r.firm, r.last_touch_at;
  ...
  SELECT person_id, meta->>'project_name' AS job, last_touch_at, meta->>'company_name' AS firm
    INTO r FROM public.people_directory WHERE display_name='Marta Uncarded';
  RAISE NOTICE 'D-b AFTER merge:  seat=% job=% firm=% last_touch=%', r.person_id, r.job, r.firm, r.last_touch_at;
  EXECUTE 'RESET ROLE';
END $$;
ROLLBACK;
