...
## `D→A-8` → **L1-A** · two new `pearl` strokes on the Welcome screen
...
| `AuthScreenView.swift` | `pearl` → `Border.strong` (and `D→A-8`'s two new ones) |
...
## From L1-C — fix round (2026-09-02) · `StyleContinueButton` gained a defaulted `ground:` parameter

Full text: `l1c-notes-out.md` §9. Recorded here because
`Features/StyleConversation/Shared/Components/StyleContinueButton.swift` is inside **L1-A's** glob
(`Features/StyleConversation/**`, PROGRAM.md §3), L1-C edited it for `GAP4-16`, and L1-C's first-pass
notes wrongly said the file had "no lane". Nothing is asked of L1-A; this is so the rebase is not a
surprise.
...
**Defaulted**, so the four quiz call sites are byte-identical and need no edit. The only caller
passing it is `RevealView.swift:54-59` (L1-D's file by name). If L1-A touches this file for `P-34` or
`C3-03`, keep the `Ground` enum and the `ground` parameter.

One consequence: L1-D took `GAP4-16`'s first option as well (`RevealView`'s body wrapped in
`.environment(\.colorScheme, .dark)`), and the two compose — D→C-9 says "take both"; if only one can
survive, take L1-D's, and `ground: .charcoal` reduces to a no-op rather than a regression.

---

## From L1-E (Copy) — round 3, 2026-09-02

Full text and reasoning: `build/waves/w1/l1e-notes-out.md` and `build/waves/w1/l1-e-copy-deck.md` (revision 3).
