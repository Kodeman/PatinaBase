...
## From L1-C — fix round (2026-09-02) · two `C3` sites survive the `C6-18` chip rewrite

Full text: `l1c-notes-out.md` §10. `Features/Rooms/Components/RoomTypePillRow.swift` is **L1-C's** by
name (PROGRAM.md §3) and L1-C merges **first**, so L1-D cannot reach into it — but after this lane's
`C6-18` rewrite (44 pt floor, `.isSelected`, a wrapped arm before the scroll) the file is still a
`C3-05` and a `C3-01` site, and both are in L1-D's own ledgers.

| site, in `private func chip(raw:label:)` | today | what the sweep wants |
|---|---|---|
| the selected chip's fill | `PatinaColors.clay` under a `PatinaColors.offWhite` label — **2.18:1** | fill `PatinaColors.Interactive.active`, label `PatinaColors.Text.inverse` |
| the unselected chip's stroke | `PatinaColors.pearl`, `lineWidth: 1.5` | `PatinaColors.Border.strong` — this is an unselected chip edge, the rule a tester is meant to see |

`D→C-7` already names `RoomTypePillRow.swift:32, 37` for `C3-05` and `D→C-6` names `:41` for
`Border.strong`. **The line numbers have moved** (the wrapped arm added lines above them); the sites
have not. L1-C's `.white` → `.offWhite` edit was token hygiene and does not move either ratio.

L1-C could not apply these itself: `Interactive.active`, `Text.inverse`'s new light side and
