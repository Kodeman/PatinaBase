...
SELECT res->>'decisionId' AS webm1_id, res->>'artifactHash' AS webm1_hash,
       res->>'authorityRevision' AS webm1_rev
FROM (SELECT public.create_project_approval_decision(:PROJECT, jsonb_build_object(
  'title', 'Fixture WEBM1 - Motion leg',
  'question', 'Approve the mudroom bench detail?',
...
SET LOCAL ROLE authenticated;
SELECT public.confirm_project_decision_review(:'webm1_id', jsonb_build_object(
  'authorityRevision', (:'webm1_rev')::int, 'artifactHash', :'webm1_hash',
  'reviewMethod', 'portal_clickthrough'), 'web-walk-r1:webm1-review') AS _;

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :DESIGNER, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT (public.publish_client_decision(:'webm1_id')).updated_at AS webm1_pub \gset

RESET ROLE;
COMMIT;

\echo G9 seeded:
SELECT :'webm1_id' AS webm1_id;
