...
SELECT res->>'decisionId' AS webh_id, res->>'artifactHash' AS webh_hash,
       res->>'authorityRevision' AS webh_rev
FROM (SELECT public.create_project_approval_decision(:PROJECT, jsonb_build_object(
  'title', 'Fixture WEBH - Web walk leg',
  'question', 'Approve the kitchen sconce placement?',
  'context', 'Web walk row for the WEBH leg.',
...
), 'web-walk-r1:webh-create',
  'The sconces sit above the counter, not the sink.') AS res) t \gset

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :CLIENT, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT public.confirm_project_decision_review(:'webh_id', jsonb_build_object(
  'authorityRevision', (:'webh_rev')::int, 'artifactHash', :'webh_hash',
  'reviewMethod', 'portal_clickthrough'), 'web-walk-r1:webh-review') AS _;

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :DESIGNER, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT (public.publish_client_decision(:'webh_id')).updated_at AS webh_pub \gset

RESET ROLE;
COMMIT;

\echo G9 seeded:
SELECT :'webh_id' AS webh_id;
