...
SELECT res->>'decisionId' AS webm2_id, res->>'artifactHash' AS webm2_hash,
       res->>'authorityRevision' AS webm2_rev
FROM (SELECT public.create_project_approval_decision(:PROJECT, jsonb_build_object(
  'title', 'Fixture WEBM2 - Motion leg',
  'question', 'Approve the pantry shelving run?',
...
SET LOCAL ROLE authenticated;
SELECT public.confirm_project_decision_review(:'webm2_id', jsonb_build_object(
  'authorityRevision', (:'webm2_rev')::int, 'artifactHash', :'webm2_hash',
  'reviewMethod', 'portal_clickthrough'), 'web-walk-r1:webm2-review') AS _;

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :DESIGNER, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT (public.publish_client_decision(:'webm2_id')).updated_at AS webm2_pub \gset

RESET ROLE;
COMMIT;

\echo G9 seeded:
SELECT :'webm2_id' AS webm2_id;
