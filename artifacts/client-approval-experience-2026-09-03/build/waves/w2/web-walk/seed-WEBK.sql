...
SELECT res->>'decisionId' AS webk_id, res->>'artifactHash' AS webk_hash,
       res->>'authorityRevision' AS webk_rev
FROM (SELECT public.create_project_approval_decision(:PROJECT, jsonb_build_object(
  'title', 'Fixture WEBK - Web walk leg',
  'question', 'Approve the stair rail profile?',
  'context', 'Web walk row for the WEBK leg.',
...
), 'web-walk-r1:webk-create',
  'The profile carries the same radius as the newel.') AS res) t \gset

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :CLIENT, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT public.confirm_project_decision_review(:'webk_id', jsonb_build_object(
  'authorityRevision', (:'webk_rev')::int, 'artifactHash', :'webk_hash',
  'reviewMethod', 'portal_clickthrough'), 'web-walk-r1:webk-review') AS _;

RESET ROLE;
SELECT set_config('request.jwt.claims',
  json_build_object('sub', :DESIGNER, 'role','authenticated')::text, true) AS _;
SET LOCAL ROLE authenticated;
SELECT (public.publish_client_decision(:'webk_id')).updated_at AS webk_pub \gset

RESET ROLE;
COMMIT;

\echo G9 seeded:
SELECT :'webk_id' AS webk_id;
