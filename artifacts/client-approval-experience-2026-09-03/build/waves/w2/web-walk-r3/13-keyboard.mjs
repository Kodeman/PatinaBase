...
const ask = askBy(page, 'kitchen island length');
...
await page.screenshot({ path: `${SH}/13-keyboard-hold-midfill.png`, clip: { x: box.x - 8, y: box.y - 8, width: box.width + 16, height: box.height + 16 } });
await page.waitForTimeout(700);
await page.keyboard.up('Enter');
await page.waitForTimeout(2500);
console.log('STAMP:', t(await ask.locator('[data-testid="approval-stamp"]').first().textContent().catch(() => '(absent)')));
await ask.screenshot({ path: `${SH}/13b-keyboard-approved.png` });
await browser.close();
