/* House-page renders for the Wave 2c ship report. Signs in as the seeded solo
...
const OUT = '/Users/kody/Code/patina-merged/artifacts/portal-polish-build-2026-09-08/waves/w2c/renders';
...
const viewports = [
  { name: 'house-w2c-1440x900', width: 1440, height: 900, scale: 1 },
  { name: 'house-w2c-390x844@2x', width: 390, height: 844, scale: 2 },
];
...
for (const vp of viewports) {
  ...
  page.on('pageerror', (e) => consoleErrors.push({ text: `pageerror: ${e.message}`, url: '', line: 0 }));
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      ...
      planKeySentence:
        document.querySelector('[data-testid="plan-key-sentence"]')?.textContent?.trim() ?? null,
      planKeyList: [...document.querySelectorAll('[data-testid="plan-key-list"] > *')].map((n) =>
        n.textContent?.trim(),
      ),
      // Every money token the page actually SHOWS, taken from visible text
      // nodes only (script/style excluded), so the RSC flight payload's `$1`
      // placeholders cannot be mistaken for a figure. A token without cents
      // is a §F-B divergence; the list is the evidence either way.
      moneyTokens: (() => {
        ...
        for (let n = walker.nextNode(); n; n = walker.nextNode()) {
          for (const m of (n.textContent ?? '').matchAll(/[$£€][\d,]+(?:\.\d{2})?/g)) {
            found.add(m[0]);
          }
        }
        return [...found];
  ...
  report.push({ viewport: vp.name, file, consoleErrors, requestFailures, ...metrics });
  await context.close();
}

await browser.close();
writeFileSync(`${OUT}/render-report.json`, JSON.stringify(report, null, 2));
console.log(
  JSON.stringify(
    report.map(({ moneyTokens, ...r }) => ({ ...r, moneyTokens })),
    null,
    2,
  ),
);
