...
## W7-R1 · Kody's three prod rulings, specced · DESIGN LEAD · 2026-08-31

USER RULINGS (2026-08-31, verbatim): *"On desktop this collection of strata symbols at the top is useless. Have it be a single strata mark that is filled in to represent current progress on the document."* · *"Then on the lens between sections there should not be giant gaps, the only gap should be after 'the record'."* · *"filled with this job items should have an icon for each room"* [the FILED WITH THIS JOB door rows]. His word supersedes OD-14's extent-proportional segments and the seven-mark arc (RF-02's arc row). Specs below are build-verbatim; mechanics beyond them are the ARCHITECT's.

**§1 · The progress mark** (`doc-spine.tsx` — the arc `<ul>` and its seven jump cells are deleted; the wrapping arithmetic comment with them):
- One **`<StrataMark size="md" fill={fillStateAtSection(activeKey)} breathing label={`${stageWord} — ${stageIndex.toLowerCase()}`} ground="light" />`** in the head where the arc row stood. The component already IS the brand's progress device (strata-mark.tsx, R15 fill-state / R35 progress): three descending lines, each a left-clipped fill over a ghost track in the movement hues (Mocha / Clay / Dusty Blue at 0.55) — use it as shipped; no bottom-up flood is invented and no new rendering is written. `md` = 88px wide, 3px bars, 17px tall — one size at both desktop tiers (fits 1180's 112px measure and 1440's 164px).
- **Fill = current progress**: the shipped `fillStateAtSection` staircase evaluated at the ACTIVE section (the engagement's own [shaping, commitment, delivery] as of now) — `1 OF 6` becomes the fill the lines carry.
- **`3 OF 5` stays**: Kody called the ROW useless, not the count — the stage phrase block (`PROCUREMENT & ORDERS` over `3 OF 5`) is untouched, including its L-6 yield.
- **No press, no tooltip**: the ladder is the navigation and the seven jumps die with the row; the mark is `role="img"` with the aria-label above (`Procurement & Orders — 3 of 5`); no `title`, no hover affordance. Keyboard reach to sections stays on the ladder rows.
- **Breathing stays** (the one ambient move, now on the one mark); reduce form: `doc-breath` 0s and the fill's `transition` inert — no animation, the filled mark simply stands.
- Pre-work spreads (one-line head, W5-R4/F2): the mark prints with the same fill and the spread's stage word; nothing else changes.
- Head reserve: the arc's 44/48px row goes; the head's measured reserve shrinks accordingly (the walker records the new 1440/1280 head heights; no gate moves — SC1 only improves).

**§2 · The ladder track — stops pack, one gap, after `The record`** (`lens-ladder.tsx`):
- Every segment row: **`flexBasis: 'auto', flexGrow: 0, flexShrink: 0`**, `min-height: 36px` (`LADDER_SEGMENT_MIN_PX` — the 44px target is met by the row's `py-1` + line heights; rooms rungs and `+N more` unchanged). `--seg-floor`/`extent` growth is retired (OD-14 superseded); the derivation keeps computing extents for the reading window only.
- The track (`[data-lens-track]`): **`flexBasis: 'auto', flexGrow: 0, flexShrink: 1`**, keeps `min-h-0 overflow-y-auto` — it takes its rows' height and scrolls only when the column genuinely cannot hold them.
- The doors block: **`mt-auto`** replaces `mt-3` — the ONE flexible gap in the nav column sits between `The record` and the doors' `border-t`. The nav stays `flex min-h-0 flex-1 flex-col`, so the doors bottom-anchor and the stops top-anchor.
- **Short viewports**: `mt-auto` collapses toward the doors' own `pt-3`/`border-t` (margin-auto → 0 with no free space); floor `12px` (`[margin-top:max(12px,auto)]` is not CSS — the architect wires the equivalent: the doors block also carries `mt-[12px]` so the collapsed state keeps one breath). When the rows themselves overflow, the track scrolls as shipped and the doors stay whole.
- **The reading window bracket stays printed**: with packed rows it spans from the top of the first row whose stop intersects the frame to the bottom of the last such row (min 27px); it no longer maps to data-extent shares. Mechanics (the rAF write) the ARCHITECT's.
- Acceptance on `…d5` at 1440×900: the gap between consecutive stop rows ≤ the row's own `py-1` rhythm (no inter-stop gap > 16px); exactly ONE gap > 24px in the nav, and it sits between `The record`'s row and `FILED WITH THIS JOB`; at 900×620 (short) the doors still print whole with ≥12px above them.

**§3 · Door icons** (`lens-ladder.tsx` doors + the 390 sections sheet's door rows — SAME icons, same rows):
- `lucide-react` (already a portal dependency): **Plan room → `Compass` · Spec book → `BookOpen` · Boards → `LayoutGrid` · Call sheet → `Users`**; the conditional rows keep the set whole — `The client's copy → FileText`, `Put down the room → Undo2`.
- Size **14px** (`size={14}`), `strokeWidth={1.5}`, **`currentColor`** (inherits the row's charcoal and its clay-ink hover), `aria-hidden` (the word is the label), `shrink-0`, gap **8px** (`gap-[8px]` on the row, replacing bare text) — the row stays `min-h-11 text-[13px]`, no width change.
- The sections sheet at 390 prints the same icon on each door row at the same size; the Margin door and other list rows are NOT doors and get none.

### W7 sign-off · `document-lens/w7-adjust` @ `c616045b7` · DESIGN LEAD · 2026-08-31 — **SIGNED**

From the lane's shots (`build/w7-shots/`): **§1 seen** — `head-mark.png` / `rail-after.png`: one StrataMark above the count, the three-line staircase legible (line 1 full mocha, line 2 partial clay, line 3 faint dusty-blue), `3 OF 5` printed, no arc row, the mark reads as its own device; the stage phrase absent at s0 is L-6's yield working, not a miss; head reserve 107/93 accepted. **§2 seen** — stops packed with 0px inter-stop gaps, each value under its name, exactly one ~88.5px gap before `FILED WITH THIS JOB`, doors bottom-anchored above the border. **§3 seen** — Compass · BookOpen · LayoutGrid · Users at 14px/1.5 before the labels in the rail AND on the 390 sections sheet (`sheet-doors.png`; the sheet also prints the packed stops with their values — good). §4 trusted to the e2e as stated.

**The two judgement calls — both accepted.** (1) The drawer reserve measured, not added: 27px clear at 1180×620 (doors end 533, drawer 560) — nothing overprints, and a blind reserve would spend rail height on air; if a future organ moves the drawer, the walk's number catches it. (1b · W7-C14, ruled after) **(b)** — the ghost track steps one register on the rail stock: `StrataMark` gains `ground="rail"` with track `rgba(44,41,38,0.22)` (from 0.12; still quieter than any text) so the unfilled remainder is legible at arm's length and the mark reads as an instrument, not the device at rest; fills and hues unchanged; `doc-spine.tsx` passes it. **Re-shot verified 2026-08-31 — SIGNED:** at 0.22 the unfilled remainder prints on each line and the mark reads as a fraction at arm's length on the rail stock; W7-C14 closed. (2) The short-viewport restatement — my "900×620" was mis-addressed (no rail exists below 1180); **absent-below-1180 + doors-whole-at-1180×620** is the correct form of the ruling and W7-R1 §2's acceptance line is amended to it.
