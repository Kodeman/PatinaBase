...
**B-8 amends C18 (first-launch tour copy is canonical and delivered).** *What:* the ratified three-step tour is **rewritten, not re-anchored**. Step 1's copy — *"This is your Daily Room — picks and stories chosen for your space"* — becomes *"This is Today — what moved in your house, and what is waiting on you."* Step 3's anchor `.profileMonogram` no longer exists (M1: the monogram is gone), so step 3 re-points at the **Studio** tab with new copy — *"Your studio — projects, proposals, invoices and files"* — and `FirstLaunchTourTests`' pinned strings are updated with it. *Why:* B-1, B-3 and B-4 make two of the three ratified sentences false or dangling; naming that as "re-anchoring" would hide a rewrite of tested, ratified copy. *Cost:* three strings, one anchor, one test file. *Rollback:* ~~the tour is gated by the same `house-first` flag as the root it describes~~ — **amended W3-fix (R4)**: the rewrite applies on **both** roots, so the tour is NOT flag-gated. Rolling `house-first` off restores the W2 root but leaves the flag-off tour rewritten: one step list, step 2 on the Record card, `.addToRoom` retired. Reverting the tour means reverting R4's commit, not flipping the flag; and because both roots now read the same three Sanity surface keys, publishing `waves/w3/n3-sanity-copy.md` changes the flag-off tour's copy too (it cannot be staged behind the flag).
