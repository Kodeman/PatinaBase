...
await shot(page, 'c00b-client-solo-signin');
...
await page.locator('input[type="email"]').first().fill('client-solo@patina.dev');
await page.locator('input[type="password"]').first().fill('password123');
await page.getByRole('button', { name: /^sign in$/i }).click();
await page.waitForTimeout(12000);
console.log('URL after', page.url());
await shot(page, 'c01b-client-solo-landed');
await c.storageState({ path: `${HERE}/r2-state-client-solo.json` });
await b.close();
