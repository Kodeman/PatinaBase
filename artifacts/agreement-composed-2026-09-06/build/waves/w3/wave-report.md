...
## Walk fixes — round 2 (R48–R52 and the minors)

Date 2026-09-08. Round 2 of the web walk (`walk-web-r2.md`) returned one
blocker, three majors and a tail of minors and nits; the rulings that answer
them are **R48–R52** plus the "Minors fixed with this round" line in
`rulings-2026-09-06.md`. Three commits, explicit pathspecs, no push, no
production mutation of any kind, no `supabase/functions/` file touched.

Base for this pass: `82aee5a76` (the round-2 walk report; one commit past the
round's stated `77f90a8b7`). Head after the three fix commits: **`50d11f528`**;
this report is the fourth commit on top of it.

| Finding | Ruling | Commit | What landed |
|---|---|---|---|
| **W3R2-01** (blocker) | R48 | `b6f983541` + `c40986b74` | **The price is never hidden**, said at four layers. (a) `agreement-composer.tsx` withholds `onToggleClientVisible` entirely for `schedule/pricing_basis` and `schedule/draws` — the act does not exist on the two parts that state the money. (b) `readiness.ts` gains `HIDDEN_TURNKEY_MONEY_BLOCKER`, filed on either part when it is hidden, keyed off the document's **kind** rather than the flag so the flag-off room is the stricter side. (c) `upsert_agreement_parts` (00578) refuses the save in the room's own sentence — *your client signs the price and the draws, so "{title}" cannot be hidden from her*. (d) `send_commercial_document`'s design-build arm asks the same question of the rows as they stand — the walk's proposal was hidden before any of this existed — and then asks `_agreement_fee_unnamed`, which reads `pricing_basis` through the same `client_visible` filter as every other fee, so a turnkey paper that names no price she can read cannot be sent. SQL: `design_build_test.sql` **T25**, four arms (hidden basis at save, hidden draws at save, a whole agreement that still sends, and the walk's own already-hidden shape refused at the door). Jest: 5 cases in `client-visibility.test.tsx`, including the flag-off one. |
| **W3R2-02** (major) | R50 | `b6f983541` + `50d11f528` | **The receipt survives a reload.** The post-signature region was in-memory `sealedDoors` + the sign route's response, so it died with the page. Now: `get_client_commercial_document_bundle` projects `designBuild.depositOffer` — the deposit draw's own live invoice, its net still owed, its label and its active `invoice_links` token — and answers null the moment that invoice is paid or voided. `door-gate.tsx` reads the client signature off the bundle (`standingSignedAt`) and the offer off `designBuild.depositOffer` (`standingOffer`); in-session values still win. `threshold.tsx` mints a door for every `client_signed` origin paper on the same houseless rule `pending` and the receipt row take, carrying `signedAlready` so the door never asks for a name she has already given while its own bundle loads. SQL: **T23** (the offer is re-derived; a paid deposit is not an offer). Jest: 4 cases in `door-gate.test.tsx` ("a paper she signed on an earlier visit" — no fetch made at all), 1 rewritten in `threshold.test.tsx`. |
| **W3R2-03** (major) | R51 | `c40986b74` | **The studio previews the paper.** `redactPartsForClient` (`lib/document/design-build.ts`) is the TypeScript twin of `_agreement_redact_client_payload`: it drops the parts the studio hid, and under anything but `open_book` strips `costLines`, `costBasisCents`, `feeBps` and `subMarkupBps` from the pricing basis and puts the two derived keys in their place — `contractSumCents`, no longer derivable once the cost lines are gone, and `scheduleOfValues`, resolved in the mode the sub-disclosure clause elected. `service-agreement-preview.tsx` renders through it. `DesignBuildPricingBasisPayload` gains the optional `contractSumCents` the projection carries, and `contractSumCents()` prefers it — the same read the client's own body already makes. The studio's cost view is unchanged: it lives in the pricing-basis editor, which reads the authored row. Jest: 4 cases in `agreement-parts-body.test.tsx` (closed book drops both rows and keeps the price; the schedule of values stands; an open book withholds nothing; a hidden part is never drawn). |
| **W3R2-04** (major) | R49 | `c40986b74` | **No double count is named where she can read it.** The rule was filed on the supervision clause alone, and part-scoped sentences render only inside the open editor — so the rail moved by one and said nothing. It is now filed three ways: against no part (the readiness rail's own line), against the pricing basis, and against **every clause carrying a supervision fee** — read over the payloads, not the seeded `patina.supervision_fee` key, so a clause renamed out of the Library still counts, exactly as `_validate_no_double_count` reads it. The composer de-duplicates messages before the rail and the send sheet, so one rule prints once in each. Send: `readiness.ready` is false while it stands, and the sheet's **Send agreement** is disabled on `!readiness.ready` — unchanged, and now actually reachable, because the sentence that holds it is finally visible. Jest: 2 cases in `no-double-count.test.tsx`. |
| **W3R2-05** (minor) | — | `c40986b74` | The send sheet branches on `document.kind`: title *Send design-build agreement*, eyebrow `DESIGN_BUILD_PAPER_COPY.documentLabel`, and "receives the price, the schedule of values, the draw schedule, the allowances, who is doing the work, and the terms" in place of the services list. The FURNISHINGS DEPOSIT block is not rendered at all — no furnishings pass through a design-build agreement. Jest: 1 case. |
| **W3R2-06** (minor, part) | — | `c40986b74` | `DESIGN_BUILD_COPY` gains `roomTitle` / `roomEyebrow` / `roomSubtitle`, and the Contract Room reads them on a `design_build` document — keyed off the KIND, so a frozen turnkey draft reads correctly too. **The rename affordance was NOT built** — see "Not fixed in this pass" below. |
| **W3R2-09** (minor) | — | `50d11f528` | `tradeAgreementSendFailureMessage` no longer returns the provider's `detail` for anything but `no_recipient`. A `send_failed` maps to *The agreement is recorded. The message could not be sent. Try again.* — true on both halves, since the row reaches `sent` before the letter is attempted — and `RESEND_API_KEY environment variable is required` stays in the function's log. Vitest: 1 case pinning both the sentence and the absence of the variable name. |
| **W3R2-11** (minor) | R47 | `b6f983541` + `50d11f528` | `rpc_start_agreement_thread(p_proposal_id)` (00578) opens or finds the direct thread between the homeowner and **that agreement's own designer**, keyed on `comms_threads.proposal_id`. It resolves the designer off the paper, so the browser names no counterpart, and admits only the client the paper was sent to, only once it was sent. `door-acts.tsx` calls it in place of `useStartDirectThread` for a houseless door. SQL: **T22** (keyed, idempotent, two participants, `project_id` still NULL — "never another project's thread"). |
| **W3R2-12** (minor) | R52 | `b6f983541` | **The origin deposit joins the house it opened.** `_countersign_design_services_agreement_impl` adopts the design-build draw ledger's invoices into the project it has just created, bracketed by `set_config('app.proposal_activation_id', …)` and its restore. `set_invoice_studio_id` is grafted from its head (00571:77-728) and widened by exactly one predicate: `project_id` may move from NULL to a project while that GUC names the design-build agreement whose draw ledger owns the invoice and whose client and designer match the row — the same GUC 00511 admits `proposals.project_id` under. Everything else stays immutable, and a project-bound invoice can still never be reparented (`OLD.project_id IS NULL` is half the predicate). SQL: **T24**, including the negative (an adopted invoice cannot be un-adopted). Two hashes re-pinned in `public_sd_hardening_contract_test.sql` with their reasoning inline. |
| **W3R2-14** (nit) | — | `c40986b74` | `LienWaiverAttachments` renders `null` with no draws. `DrawLedger` stands directly above it in the rail and already says the sentence. |
| **W3R2-15** (nit) | — | `c40986b74` | `AgreementTemplatePartEntry` gains `enabled?: boolean`, and the template picker's summary line filters on `enabled !== false` — the same predicate `materialize_agreement_template` applies. Flow-down no longer appears over a template that lays out ten parts without it. |

### Not fixed in this pass

- **W3R2-07** (minor) — the contradictory chips on a client-signed document. The two sentences the walk read ("V1 · AWAITING SIGNATURE", "Sent Sep 8 · not opened") are **not** in `service-agreement-instruments.tsx`, which prints `commercialStatusView`'s CLIENT SIGNED / AWAITING COUNTERSIGN correctly; they come from `proposal-watch.tsx`'s `statusLine` and `proposal-version-history.tsx`'s label, both of which read `proposals.status` — still `sent` until countersign, by design since 00331. Suppressing them means teaching the proposal-watch model that a client signature exists, which is a read the model does not make today. Left for a ruling on where that fact should enter the model.
- **W3R2-16** (nit) — a revoked-but-unspent `/trade/<token>` renders the 404 page with HTTP 200. The route already calls `notFound()` on every miss, and the walk's own note says a nonsense token behaves identically — so this is the client portal's general not-found status, not something the trade route can set from a page component (App Router pages have no status API beyond `notFound()`). Not reproduced in this pass; no portal was booted. Needs a diagnosis of why `notFound()` is answering 200 in this deployment before a fix can be aimed at anything.
- **W3R2-06's rename affordance** — the chrome landed; letting the designer rename the agreement in the room did not. `proposals.title` has no rename RPC and the composer has no field for one; adding both, plus the guard work to keep a sent paper's title frozen, is a build, not a copy fix. The stale title reaching the keepsake footer is a title the drafting flow wrote, and it stays until that act exists.

### Gates — every command run in the integration worktree, output read

| # | Gate | Result |
|---|---|---|
| 1 | `supabase db reset --workdir …/agent-agr-w3-integration` | **clean**. Ledger head probed after: `00579, 00578, 00577`. `rpc_start_agreement_thread` present with `proconfig {search_path=public}`; `set_invoice_studio_id` carries `v_adopts_origin_deposit` (probed on `pg_proc.prosrc`, not inferred). |
| 2 | `python3 scripts/generate-legacy-grants.py` | "baseline + **2570** replayed statements" (2568 before — the two new GRANTs); regenerated **before** the reset and replayed as its first seed. |
| 3 | `psql -v ON_ERROR_STOP=1 -f supabase/tests/commercial/design_build_test.sql` | exit 0 · **22 PASS** (20 + T22/T23/T24 and T25) |
| 4 | The other Wave 3 SQL suites, one by one | `trade_agreement_test` exit 0 · 8 PASS · `agreement_parts_test` exit 0 · 30 PASS · `agreement_library_test` exit 0 · 14 PASS · `agreement_fee_schedules_test` exit 0 · 11 PASS · `agreement_parts_projection_test` exit 0 · 5 PASS · `multi_studio_signature_test` exit 0 · 7 PASS · `billing/studio_invoice_test` exit 0 |
| 5 | `psql … edge_api/public_sd_hardening_contract_test.sql` | exit 0 — after re-pinning `set_invoice_studio_id` (`3a556842…` → `0d8fea5c…`) and `_countersign_design_services_agreement_impl` (`d3a2cade…` → `3ca87ae1…`). Both drifts were read from the suite's own assertion, then the new hash computed with the suite's own `extensions.digest(convert_to(prosrc,'UTF8'),'sha256')`. |
| 6 | `psql … edge_api/public_rpc_authorization_contract_test.sql` | exit 0 |
| 7 | `SUPABASE_DB_URL=… pnpm db:generate` + `git diff --exit-code packages/supabase/src/database.types.ts` | file 1.17 MB (not truncated); one function added (`rpc_start_agreement_thread`); after commit, a second regen left the tree **clean — in sync** |
| 8 | `pnpm exec turbo build --filter=@patina/types --force` | 1 successful, 1 total |
| 9 | `pnpm --filter @patina/designer-portal type-check` | clean |
| 10 | `pnpm --filter @patina/designer-portal test` (FULL) | **544 suites, 6691 tests, 12 snapshots — all passed** (was 6680; +11) |
| 11 | `pnpm --filter @patina/client-portal type-check` | clean |
| 12 | `pnpm --filter @patina/client-portal test` (FULL, coverage floor enforced) | **134 suites, 2245 tests — all passed** (was 2241; +4) |
| 13 | `pnpm --filter @patina/supabase type-check` · `test` | clean · **93 files, 1146 passed, 12 skipped** (was 1145; +1) |

No deno test was run and none was owed: **no file under `supabase/functions/` was touched**, so no importer of any `_shared` module needs redeploying. `platform_acl_compatibility_test.sql` fails on this stack and is a **documented known failure** (`supabase/tests/KNOWN_FAILURES.md:51`), unchanged by this pass.

### What did NOT change

- No production mutation of any kind. Nothing pushed. No Stripe call, key or webhook touched.
- No edge-function source, no `_shared` file, no `config.toml`.
- No `.env` file, no `.claude/`, no hook, no setting.
- No worktree created or removed.
- Prettier reports formatting drift on the files this pass touched; it is advisory locally and pre-existing on this branch (it flags a one-line addition to `packages/supabase/src/hooks/index.ts` as readily as a new component). Nothing was reformatted, so no diff is noise.

### Advisories from this pass — no ruling, not blocking

- **`_agreement_design_build_part` still reads the authored row.** R48 is enforced by two explicit refusals around it rather than by narrowing the reader, because the same function answers for `draws`, `allowances` and the disclosure resolution, and a `client_visible` filter there would change what "the pricing basis" means to four callers at once. The refusals are asked first, so nothing reaches the reader in a state the projection would drop — but the reader itself is still the authored row, and that is a seam worth naming.
- **A signed origin door now stands on every house until countersign.** That is what R50 asks for, and it means a household with three houses sees the same signed paper, with the same receipt and the same deposit offer, three times — exactly as the unsigned one already did under R30. If that reads as three papers rather than one, the fix is the same one R30's line makes for the unsigned case, extended to the receipt.
- **The deposit offer's `payToken` now crosses the client bundle.** It is the same token the sign route already handed this same client, and the bundle is readable only by that client or a studio co-member — but it is a bearer credential in a read a page makes on every load, which is a wider surface than a one-time response. Worth a look if `/pay` ever gains an act beyond paying.
- **A second direct thread can now exist between a homeowner and her designer.** `rpc_start_agreement_thread` keys on the paper, and `rpc_start_direct_thread` (unchanged, used elsewhere) finds the OLDEST direct thread between the two by participants alone — so a household that has both will see the agreement's thread beside the plain one in her letters. R47 asks for the keying; whether the inbox should distinguish them is not ruled.
