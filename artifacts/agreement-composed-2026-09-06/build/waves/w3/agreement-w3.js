...
GATES TO PASTE: bare 'cd ${lane.wt}' then 'pnpm --filter @patina/designer-portal type-check', 'pnpm --filter @patina/designer-portal lint', 'pnpm --filter @patina/designer-portal test -- <touched test files>' AND the full 'pnpm --filter @patina/designer-portal test' once at the end (paste suite/test counts). Commit per item (feat(document): …).`
  if (lane.key === 'client') return `You are the CLIENT PORTAL lane for Wave 3. Deliver EVERY item in the build sheet's client lane section: the sign route learns 'design_build' explicitly in its routing (the sheet's fail-open finding: the allowlist auto-admits the kind from @patina/types while the furnishings → trade_scope → else routing does not — add the branch and a test that asserts WHICH RPC ran), commercial-document-shell renders the design-build body (schedule of values, draws, attachments as leaves), consent-copy learns the kind, door-gate + the new DepositOffer (after the signature commits, a separate failable call mints the deposit link to the shipped /pay/[token] surface; a failure never touches the signature), jest with the coverage floor, and the e2e touchpoint the sheet names.
CARRIED FROM R30 (also required; see build/hotfix/r30-review-r4.md N1/N2): the origin door (household with no project) offers "Ask a question" like a project-bound door does — DoorActs must not withhold it for lack of a projectId (route the question to the studio on the agreement); and a failed list_client_proposals read renders an honest "could not load" state with a retry, never ProjectsEmptyState over a pending agreement. Jest for both.${common}
...
phase('Integrate')
let integ = await agent(integrationBrief(lanes), { label: 'S9 integration', phase: 'Integrate', schema: INTEGRATION_SCHEMA, model: 'opus' })
if (!integ) {
  log('integration returned nothing — dispatching the recovery steward')
  integ = await agent(`You are the RECOVERY integration steward for Wave 3. A previous steward created ${WT}/agent-agr-w3-integration on branch agreement/w3-integration and got part-way through the integration brief below before ending without a report. Do NOT recreate the worktree. First establish state economically: 'git -C ${WT}/agent-agr-w3-integration log --oneline -12', 'git -C … log --merges --oneline origin/main..HEAD' (which lanes are merged), 'git -C … status --short --untracked-files=no', 'ls supabase/migrations | tail -4' there, 'psql postgresql://postgres:postgres@127.0.0.1:54322/postgres -Atc "select version from supabase_migrations.schema_migrations order by version desc limit 3"', and whether ${W1}/wave-report.md or ${W1}/walk-env.md already exist. Then perform ONLY the steps not yet done, in the brief's order, and skip nothing that is undone. Keep tool calls economical — paste gate tails, not full logs — and make the StructuredOutput call the moment the gates are done; a prior steward ended without it.
----- ORIGINAL INTEGRATION BRIEF -----
${integrationBrief(lanes)}
----- END -----`, { label: 'S9r recovery integration', phase: 'Integrate', schema: INTEGRATION_SCHEMA, model: 'opus' })
  if (!integ) { log('recovery integration returned nothing'); return { steward, lanes: laneSummary() } }
}
