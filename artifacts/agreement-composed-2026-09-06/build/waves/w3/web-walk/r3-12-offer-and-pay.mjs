...
const TOKEN = '3eb55766d791d82d5810c7d9c45f391499c157ddc39b5df96e4e75a6bc9e1165';
...
await shot(page, 'r3-12a-papers-in-full');
...
for (const label of ['Marrow & Vale Residence', 'Aspen Loft Refresh']) {
  ...
  if (await link.count()) {
    ...
    await shot(page, `r3-12b-${label.replace(/[^a-z]/gi, '-').toLowerCase()}`);
  }
}
...
await shot(pay, 'r3-12c-pay-page');
...
if (await btn.count()) {
  ...
  await shot(pay, 'r3-12d-pay-clicked');
