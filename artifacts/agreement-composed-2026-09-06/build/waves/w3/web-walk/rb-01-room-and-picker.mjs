...
await page
  .locator('[role="dialog"]')
  .last()
  .getByText('client@patina.dev', { exact: false })
  .first()
  .click();
await page.waitForTimeout(14000);
console.log('room url:', page.url());
fs.writeFileSync(`${HERE}/rb-room-url.txt`, page.url());
await shot(page, 'r2-01a-contract-room');
...
await shot(page, 'r2-01b-template-picker-locked');
const dlg = await page.evaluate(() => {
  ...
  return {
    text: d.innerText,
    rows,
    links: Array.from(d.querySelectorAll('a')).map((a) => a.innerText + ' -> ' + a.getAttribute('href')),
  };
});
