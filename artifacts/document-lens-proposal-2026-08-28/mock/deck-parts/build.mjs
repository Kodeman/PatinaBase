...
//   SMOKE=1 node mock/deck-parts/build.mjs   (Phase 0 gate: missing lens css
//     and missing shots are tolerated instead of fatal, for building a
//     placeholder deck before the mechanics parts exist)
//
// What it does, in order:
//   1. concatenates PARTS (fixed order; a missing part is fatal)
//   2. injects fonts-data-uri.css / kit.css / lens.css at the
//      /* @@FONTS@@ */ /* @@KIT@@ */ /* @@LENS@@ */ markers
//      (missing lens.css is fatal, except SMOKE=1)
...
const ROOT = path.resolve(HERE, "..", "..");            // artifacts/document-lens-proposal-2026-08-28
...
/* ── 2. CSS: fonts (base64), kit, lens stylesheet ─────────────────────────── */
...
// Single lens marker: mock/lens.css carries this program's .lens-* mechanics
// css. A missing or empty file is tolerated only under SMOKE=1 (Phase 0 —
// lens.css is filled in W4/M0).
const lensPath = path.join(MOCK, "lens.css");
let lensCss = "";
if (!fs.existsSync(lensPath) || read(lensPath).trim() === "") {
  if (!SMOKE) errors.push("missing or empty mock/lens.css");
} else {
  lensCss = stripCssComments(read(lensPath));
}

// Product crops (see step 5 below) are emitted as :root{--crop-*} rules, in a
// generated block appended after @@LENS@@.
const cropManifest = [];
let cropCss = "";
...
let fragmentCount = 0;
const fragmentFilesUsed = new Set();
...
// Extra (non-fatal) gate: warn about any mock/fragments/*.html whose mtime is
// older than mock/final/index.html — a sign the fragment was cut before the
// final mockup's last edit and may be stale. Only meaningful once
// mock/final/index.html exists (the fragment cutter runs later in the
// program), so this is silent until then.
{
  const finalIndex = path.join(MOCK, "final", "index.html");
  if (fs.existsSync(finalIndex) && fs.existsSync(FRAGMENTS)) {
    const finalMtime = fs.statSync(finalIndex).mtimeMs;
    const stale = fs.readdirSync(FRAGMENTS)
      .filter((f) => f.endsWith(".html"))
      .filter((f) => f !== "_smoke.html")   // scaffold file, not a real fragment
      .filter((f) => fs.statSync(path.join(FRAGMENTS, f)).mtimeMs < finalMtime);
    if (stale.length) {
      warnings.push(
        "fragment(s) older than mock/final/index.html (may predate the final mockup): " + stale.join(", ")
      );
    }
  }
}
...
/* ── 7. Hard-fail gates ────────────────────────────────────────────────── */
// D4 as amended by ruling R126: ONE token, --elevation-sheet, spent at
// exactly one declaration site, `.doc-elevated { box-shadow:
// var(--elevation-sheet); }` (the margin chip, the ledger sheet, the studio
// drawer share the class). That single declaration is sanctioned; any other
// box-shadow value, a second declaration (even a second copy of the
// sanctioned one), or any drop-shadow( still fails the gate outright.
const SANCTIONED_SELECTOR = ".doc-elevated";
const SANCTIONED_VALUE = "var(--elevation-sheet)";
...
const shadowDeclarations = [];
{
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    if (/^none$/i.test(value)) continue; // a removal, not a shadow
    shadowDeclarations.push(value);
  }
}
const dropShadowCount = (html.match(/drop-shadow\(/gi) || []).length;

const sanctionedRule = html.match(
  /([^{}]*)\{[^{}]*box-shadow\s*:\s*var\(\s*--elevation-sheet\s*\)\s*;?[^{}]*\}/i
);
const sanctioned =
  shadowDeclarations.length === 1 &&
  dropShadowCount === 0 &&
  norm(shadowDeclarations[0]) === norm(SANCTIONED_VALUE) &&
  !!sanctionedRule &&
  sanctionedRule[1].trim() === SANCTIONED_SELECTOR;

const shadowMatches = sanctioned ? 0 : shadowDeclarations.length + dropShadowCount;
const shadowSummary = sanctioned
  ? `1 (sanctioned: ${SANCTIONED_SELECTOR} ${SANCTIONED_VALUE})`
  : String(shadowMatches);

const leftovers = html.match(
  /<!--\s*(FRAGMENT|IMG|include:|shot:|MISSING)[^>]*-->|\/\*\s*@@(FONTS|KIT|LENS)@@\s*\*\//g
) || [];
...
// One allowed non-font https host, scoped by convention to whichever part
// cites the assembled direction's own clickable mockup (an Artifact published
// at claude.ai). Every other host stays rejected.
...
console.log(
  ...
  `box-shadow ${shadowSummary}, markers ${leftovers.length}, non-ascii ${nonAscii}, SIZE ${mb(bytes)}`
);
console.log("");
