// Rendered QA for The Smart Lens deck.
...
// The @playwright/test resolution below is hardcoded to the designer-portal
// workspace's node_modules, exactly as LIFE's qa-run.cjs has it (this folder
// carries no node_modules of its own) — ported verbatim, not re-derived.
const { chromium } = require('/Users/kody/Code/patina-merged/apps/designer-portal/node_modules/@playwright/test');
const fs = require('fs');
const path = require('path');

const ROOT = '/Users/kody/Code/patina-merged/artifacts/document-lens-proposal-2026-08-28';
...
const SECTIONS = [
  'cover', 'ask', 'today', 'found', 'thesis', 'spine', 'header', 'body',
  'motion', 'mobile', 'walkthrough', 'build', 'roads', 'limits', 'colophon',
];
...
const PROBE = function () {
  ...
  // except the one R126 exception -- an element wearing .doc-elevated whose
  // computed boxShadow is exactly the --elevation-sheet token's resolved
  // value. Comments and CSS source can mention "box-shadow" -- this checks
  // what the browser actually painted, which is the only thing that matters
  // for D4.
  const SANCTIONED_BOX_SHADOW = 'rgba(44, 41, 38, 0.08) 0px 1px 2px 0px';
  const shadowed = [];
  let shadowSanctioned = 0;
  document.querySelectorAll('*').forEach((el) => {
    const bs = getComputedStyle(el).boxShadow;
    if (bs && bs !== 'none') {
      if (el.classList.contains('doc-elevated') && bs === SANCTIONED_BOX_SHADOW) {
        shadowSanctioned += 1;
        return;
      }
...
(async () => {
  ...
  for (const vp of VIEWPORTS) {
    for (const theme of THEMES) {
      ...
      await page.evaluate(() => {
        try { window.localStorage.removeItem('dk-document-lens-theme'); } catch (e) {}
      });
      ...
      const asReadChecks = [];
      for (const id of ['ask', 'today', 'spine', 'walkthrough', 'build']) {
  ...
  for (const [tag, p] of Object.entries(results.passes)) {
    line(`${tag}: scrollW ${p.scrollWidthDoc}/${p.clientWidth} bodyScrollW ${p.scrollWidthBody} | bodyBg ${p.bodyBg} | overflowers ${p.overflowers.length} | mocks ${p.mocks.length} (fitOverflow ${p.mocks.filter((m) => m.fitsParent === false).length}, pastVP ${p.mocks.filter((m) => m.pastViewport).length}) | shadowed ${p.shadowed.length} (sanctioned ${p.shadowSanctioned}) | idx rows ${p.index && p.index.rows}/${p.index && p.index.sectionsWithTitle} pos ${p.index && p.index.position} | fonts ${p.fonts.length}`);
