...
## 9. `lens.css` — the `.lens-*` namespace (W4, M0)

Companion to `kit.css`. Written for "The Smart Lens" program (2026-08-28) and
holds every mechanic named in `source/proposal.md` section 3 (the lens
mechanics table) plus the two condensation/density concepts SPEC C.5 makes
DOM attributes. `lens.css` never restyles the R126 register — every colour
it uses is either an existing kit/tokens value or one of the six new tokens
this program is allowed to add (`--lens-h-open`, `--lens-h-closed`,
`--doc-region-gap`, `--density-ink-full`, `--density-ink-reading`,
`--density-ink-condensed`), plus the two R126 tokens `kit.css` lacked
(`--doc-rail-stock`, `--elevation-sheet`) that the brief named explicitly.

**Reduced motion, one mechanism.** Rather than writing every animated
class's reduced-motion form a second time, `lens.css` sets `--motion-scale`
to `0` from both SPEC C.6 triggers — `@media (prefers-reduced-motion:
reduce) { :root { --motion-scale: 0 } }` and `[data-motion="reduced"] {
--motion-scale: 0 }` — and every class's own duration is written once as
`calc(<base>ms * var(--motion-scale, 1))`. Both setters assign the same
literal `0`, so there is nothing for the two triggers to disagree about;
under either one every transition/animation in this file resolves to `0ms`,
which is the reduced-motion form the section-3 table's own "Reduced-motion
equivalent" column describes for every row that carries CSS motion.

### Condensation — the lens line

| Class | Selector | For | from -> to | Reduced form | Section-3 row |
|---|---|---|---|---|---|
| `.lens-line` | `.lens-line` | the header organ's reserved height; carries `--lens-height` (SPEC C.5) | `--lens-height: var(--lens-h-closed)` <-> `var(--lens-h-open)` | duration resolves to 0ms via `--motion-scale` | H5 / "the header" |
| `.lens-line[data-lens-open="true"]` | attribute variant | the open (scroll-0) state | height -> `var(--lens-h-open)` (319px, SC1) | same | H3 |
| `.lens-line[data-lens-open="false"]` | attribute variant | the condensed state | height -> `var(--lens-h-closed)` (56px, under SC3's 64px ceiling) | same | H5 |

### Density — SPEC C.5's `data-density` values

| Class | For | Ink | Contrast on `--doc-paper` |
|---|---|---|---|
| `.lens-density-full` | a region at `data-density="full"` | `--density-ink-full` | 9.22:1 |
| `.lens-density-reading` | `data-density="reading"` | `--density-ink-reading` | 7.73:1 |
| `.lens-density-condensed` | `data-density="condensed"` | `--density-ink-condensed` | 6.51:1 |

### One class per section-3 mechanic

| Class | Row | from -> to (row's own value) | Reduced form |
|---|---|---|---|
| `.lens-sentence-outgoing` / `.lens-sentence-incoming` | L-1, the sentence turns | opacity `1 -> 0` / `0 -> 1` | prints instantly in place, 0ms |
| `.lens-reading-window` | L-2, the reading window travels | `top 202px -> 512px`, `height 74px -> 158px` at 1440 | no CSS transition to begin with (position-linked, JS-stepped per the SPEC's own amendment) |
| `.lens-segment-value` | L-3, a segment changes register | opacity `1 -> 0` (its value line yields while its head is in frame) | absent/present at settle, 0ms |
| `.lens-region-reserve` | L-4, a region ahead opens | `block-size` reserve -> `auto`, 0ms by the row's own rule | identical (no transition to reduce) |
| `.lens-region-passed` | L-5, a region behind is never taken back | `content-visibility: visible -> auto` | identical (no visual change of any kind) |
| `.lens-rail-head-line` | L-6, the rail head yields at s0 | opacity `1 -> 0` | absent/present at settle, 0ms |
| `.lens-seam` / `.lens-seam-arrow` | L-7, she folds a region | `--rule-strong -> --rule-mid`; fold-in + arrow-flip | paints already-settled/already-flipped on the first frame, 0ms |
| `.lens-row-editing` | L-8, the pen goes down | `--rule-hair @10% -> 1.5px var(--color-clay-ink)`; ground `transparent -> rgba(196,165,123,.12)` | both present and static (this is the shipped R126 contract; 0ms makes it so) |
| `.lens-settled` | L-9, the lens settles | arithmetic gate; moves nothing | identical (nothing to reduce) |
| `.lens-jump-target` | L-10, the press lands | `scroll-margin-top: var(--doc-landing-clear)` | identical landing offset in both registers |
| `.lens-sheet-panel` | L-11, the standing sheet opens | `translateY 8px -> 0`, `opacity 0 -> 1` | paints in place at full ink, 0ms |

### The navigator and reading line in the rail

| Class | For |
|---|---|
| `.lens-nav-segment[data-reading-index="true"]` | the rail segment holding the frame's current stop (SPEC C.5 `data-reading-index`) prints at full ink instead of `--text-muted` |
| `.lens-nav-room-rung[data-room-visible="true"]` | a room sub-rung's `0 -> 28px` reveal, while the reading window brackets `Pieces` or a room is in hand (override 2) |

### `--doc-rail-stock` and the contrast gate

`--doc-rail-stock` carries a `/* contrast: ignore */` comment. It is a
ground/background token, not text, and — being `lens.css`'s only other
ground-shaped custom property — would otherwise become the *sole* ground
`research/contrast-check.mjs` tests every kit `-ink` token against,
crowding the kit's own `--doc-paper`/`--doc-sheet-*` grounds out of the run
entirely. Ignoring it restores the kit's full ground list as the check's
target, exactly as `mock/final/tokens.css`'s own `--rail-stock` already does
for the same reason (see its comment at that file's line 14).

Running `node research/contrast-check.mjs mock/lens.css mock/kit.css` with
`--doc-rail-stock` ignored reproduces `node research/contrast-check.mjs
mock/kit.css` exactly (16 failures, 1 warning) — these are pre-existing
`kit.css` token-pair failures (e.g. `--color-clay-ink` on `--doc-sheet-3` at
4.32:1, `--stamp-in-production-ink` on every paper/sheet ground at
1.32-1.84:1), not anything `lens.css` introduces. `lens.css` is not
permitted to restyle `kit.css`'s register, so this file cannot resolve them;
they are reported here as a pre-existing defect for the orchestrator.

The `--density-ink-*` family is not classified as TEXT by the checker's
regex (names don't end in `-ink`, don't contain `text`/`fg`) and so goes
unchecked by the automated gate, exactly as the brief anticipated. Their
ratios are hand-verified above and in `mock/lens.css`'s own comments (9.22 /
7.73 / 6.51 : 1 on `--doc-paper`).
