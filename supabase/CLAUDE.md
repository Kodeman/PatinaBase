...
- ⚠ **The 00588 invoice payer ladder has two homes.** It is written once in `00588_invoice_link_household_name.sql` (inline in the service_role-only `resolve_invoice_link`/`resolve_invoice_link_for_checkout` functions, keyed on a hashed `/pay` bearer token) and ported step for step, with line numbers cited, in `apps/designer-portal/src/lib/document/studio-billing-export.ts` — the ladder has no callable home a designer-scoped export can reach, so it is duplicated rather than shared. There is no shared helper; a change to the ladder's logic in either place must be mirrored in the other by hand.
