...
Deno.test(
  "evaluateCondition: event_occurred, no negate, event NOT occurred -> false",
  async () => {
...
Deno.test(
  "evaluateCondition: event_occurred, no negate, event occurred -> true",
  async () => {
...
Deno.test(
  "evaluateCondition: event_occurred, negate:true, event NOT occurred -> true (send the email)",
  async () => {
    const supabase = mockSupabaseForEvents([]);
    const result = await evaluateCondition(supabase, "user-1", {
      type: "event_occurred",
...
Deno.test(
  "evaluateCondition: event_occurred, negate:true, event occurred -> false (skip the email)",
  async () => {
    const supabase = mockSupabaseForEvents([{ id: "e1" }]);
    const result = await evaluateCondition(supabase, "user-1", {
      type: "event_occurred",
...
Deno.test(
  "evaluateCondition: negate:false behaves identically to negate absent",
  async () => {
    const occurred = await evaluateCondition(
      mockSupabaseForEvents([{ id: "e1" }]),
      "u",
      {
        type: "event_occurred",
        event: "hours_logged",
...
const steps = [
  {
    id: "donb_gate_onboarding-capture",
  ...
  },
  {
    id: "donb_6",
    type: "email" as const,
    config: { template_id: "onboarding-capture", delay_days: 0 },
  },
  { id: "donb_7", type: "wait" as const, config: { delay_days: 7 } },
];

Deno.test(
  "advanceEnrollmentSkippingNext: skips the paired email, advances two steps, logs skipped:true",
  async () => {
...
Deno.test(
  "advanceEnrollmentSkippingNext: reaching the end of steps marks the enrollment completed",
  async () => {
    const captured: Captured[] = [];
    const supabase = mockSupabaseCapturingUpdates(captured);
    ...
    const shortSteps = [
      { id: "c", type: "condition" as const, config: {} },
      {
        id: "e",
        type: "email" as const,
        config: { template_id: "onboarding-aesthete" },
      },
    ];
    ...
    const enrollmentUpdate = captured.find(
      (c) => c.table === "sequence_enrollments",
    );
    assertEquals(enrollmentUpdate?.payload.status, "completed");
    assertEquals(enrollmentUpdate?.payload.next_step_at, null);

    const sequenceUpdate = captured.find(
      (c) => c.table === "automated_sequences",
    );
