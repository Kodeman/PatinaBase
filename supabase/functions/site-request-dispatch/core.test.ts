...
Deno.test(
  "raw token is provider-only and the persisted SMS body is redacted",
  async () => {
    let providerBody = "";
    let persistedBody = "";
...
Deno.test(
  "quiet hours leave identifiers queued without claiming or minting",
  async () => {
    let claimedCount = 0;
    const res = await handleSiteRequestDispatch(
      request("send"),
...
Deno.test(
  "provider failure stays retryable and records only a generic safe error",
  async () => {
    let completion:
      | {
          sent: boolean;
...
Deno.test(
  "completion retries after provider acceptance instead of falsely failing",
  async () => {
    let attempts = 0;
    const res = await handleSiteRequestDispatch(
      request("send"),
...
Deno.test(
  "lifecycle sweeps retryable SMS and batched designer push outboxes",
  async () => {
    ...
    const res = await handleSiteRequestDispatch(
      ...
      deps({
        ...
        pendingDeliveryNotifications: () =>
          Promise.resolve(["88888888-8888-4888-8888-888888888888"]),
...
Deno.test(
  "quiet-hours deferral is still the outbox's own retry, not a refusal",
  async () => {
    let completion: DispatchCompletion | undefined;
    const res = await handleSiteRequestDispatch(
      request("send"),
...
Deno.test(
  "a refused sweep row is counted as refused, never as queued",
  async () => {
    const res = await handleSiteRequestDispatch(
      request("lifecycle"),
      deps({
