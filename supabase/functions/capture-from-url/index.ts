...
import { extractProduct, type ExtractedProduct } from './extract.ts';
...
function parseBody(raw: unknown): { ok: true; payload: CapturePayload } | { ok: false; error: string } {
  if (!raw || typeof raw !== 'object') return { ok: false, error: 'invalid_body' };
  const b = raw as Record<string, unknown>;

  const mode = b.mode;
  if (mode !== 'capture' && mode !== 'refresh') return { ok: false, error: 'invalid_mode' };
  ...
  if (mode === 'refresh' && !productId) return { ok: false, error: 'product_id_required' };
 ...
 */
async function callerCanSeeProduct(admin: any, callerId: string, p: ProductRow): Promise<boolean> {
