...
BEGIN
  ...
  -- 00620's statement, copied rather than paraphrased.
  WITH tiered AS (
  ...
  ASSERT v_stamped >= 3,
    'FAIL g1: the statement must have stamped at least this file''s three '
    'unambiguous fixtures (one employer, owned only, the principal); stamped '
    || v_stamped;
...
BEGIN
  ...
  ASSERT v_left = 0,
    'FAIL h1 (00620''s end state): no project whose designer''s tier ANSWERS may be '
    'left with studio_id NULL — one left behind prices ''none'' for ever and '
    'HT-3-g(3) gives its own studio no act. Count = ' || v_left;
