...
--
-- Run:
--   scripts/run-supabase-sql-test.sh supabase/tests/workflow/commercial_privacy_contract_test.sql
...
CREATE OR REPLACE FUNCTION pg_temp.auth_null_authorization_item_probe()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  INSERT INTO public.furnishing_authorization_items (
    id, commercial_document_id, source_ffe_item_id, project_room_id, name,
    ...
    vendor_name, snapshot, sort_order
  ) VALUES (
    'a3590000-0000-4000-8000-000000000099',
    'a3580000-0000-4000-8000-000000000005',
    'a3560000-0000-4000-8000-000000000004',
    'a3550000-0000-4000-8000-000000000003',
    'Auth-null definer forgery', 'Pre-execution Room', 'Seating', 'fixed',
    1, 200000, 200000, 100000, 100.00, 'Forged Vendor', '{}'::jsonb, 99
  );
END;
$$;
GRANT EXECUTE ON FUNCTION pg_temp.auth_null_authorization_item_probe() TO service_role;

CREATE OR REPLACE FUNCTION pg_temp.auth_null_signature_probe()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
...
BEGIN
  INSERT INTO public.commercial_document_signatures (
    ...
    'a35a0000-0000-4000-8000-000000000099',
    'a3570000-0000-4000-8000-000000000005', 'client',
    'a3500000-0000-4000-8000-000000000002', 'Forged Client',
    public._commercial_document_fingerprint('a3570000-0000-4000-8000-000000000005'),
    '{"via":"execute_furnishings_authorization"}'::jsonb
  );
END;
$$;
GRANT EXECUTE ON FUNCTION pg_temp.auth_null_signature_probe() TO service_role;
...
BEGIN
  BEGIN
    PERFORM pg_temp.auth_null_authorization_item_probe();
    v_detail := 'auth-null SECURITY DEFINER inserted an authorization line';
  EXCEPTION WHEN OTHERS THEN
    v_denied := true;
    v_detail := format('auth-null authorization definer rejected: %s', SQLERRM);
  END;
  INSERT INTO workflow_privacy_results VALUES (
    'C15_AUTH_NULL_DEFINER_CANNOT_INSERT_AUTHORIZATION_ITEM', v_denied, v_detail
  );
END;
$$;
...
BEGIN
  BEGIN
    PERFORM pg_temp.auth_null_signature_probe();
    v_detail := 'auth-null SECURITY DEFINER inserted a signature';
  EXCEPTION WHEN OTHERS THEN
    v_denied := true;
    v_detail := format('auth-null signature definer rejected: %s', SQLERRM);
  END;
  INSERT INTO workflow_privacy_results VALUES (
    'C16_AUTH_NULL_DEFINER_CANNOT_INSERT_SIGNATURE', v_denied, v_detail
  );
END;
$$;
...
-- PostgREST dispatches named JSON arguments. Privacy wrappers must preserve
-- the installed public API names even when their private bodies are renamed.
INSERT INTO workflow_privacy_results
SELECT
