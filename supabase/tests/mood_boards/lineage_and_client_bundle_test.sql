...
-- 00511 derives a project's studio only for a designer who belongs to exactly
-- one active design studio (prod designers hold one). The seeded designer used
-- for the activation below (Leah, a0…004) owns two: the shared "Local Dev
-- Studio" that this test's earlier continue-board probe runs against, and her
-- own auto-provisioned personal studio — a local-seed artifact. Drop the
-- personal-studio membership (keeping Local Dev Studio, which her earlier
-- fixture project belongs to) so her signed proposal activates into that one
-- studio. Transaction-scoped and rolled back; triggers suppressed so the
-- personal studio's last-owner guard does not block the cleanup.
-- Target the personal studio by exclusion: any active design studio Leah owns
-- other than the stable-seed Local Dev Studio (its id is generated by the
-- onboarding trigger and varies per reset, so it cannot be named literally).
SET LOCAL session_replication_role = replica;
DELETE FROM public.organization_members AS membership
USING public.organizations AS studio
WHERE membership.organization_id = studio.id
  ...
  AND studio.id <> 'b0000000-0000-0000-0000-000000000001';
