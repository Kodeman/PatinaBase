...
INSERT INTO _00488_expected VALUES
  ...
  (
    ...
    ARRAY['search_path=pg_catalog, public, pg_temp'],
    '484f9219d52297c4c04656b1283af3c0e18046b22d855cf94a4d1e37d121abcb',
    ARRAY['authenticated'],
  ...
  (
    ...
    'jsonb', 'v', ARRAY['search_path=pg_catalog, public, pg_temp'],
    'a93473c3853da503db51e67e658481d5aa2085d1a8b2a4e8144ef80181da528d',
    ARRAY['authenticated'],
...
BEGIN
  ...
  -- apply_scope_change must reach that predicate only through the helper.
  ASSERT (
    SELECT routine.prosrc LIKE '%_scope_change_requester_can_author%'
    FROM pg_proc AS routine
    WHERE routine.oid = 'public.apply_scope_change(uuid)'::regprocedure
  ), 'apply_scope_change no longer delegates to its studio-authority helper';
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION WHEN insufficient_privilege THEN
      -- 00483 also revokes USAGE on schema public from these roles, so
      -- the first refusal can name the schema rather than the function.
      denied := SQLERRM LIKE 'permission denied for function %'
             OR SQLERRM LIKE 'permission denied for schema %';
    END;
    ASSERT denied,
...
BEGIN
  ...
  EXCEPTION WHEN insufficient_privilege THEN
    -- 00483 also revokes USAGE on schema public from several of these roles,
    -- so the first refusal can name the schema rather than the function.
    denied := SQLERRM LIKE
...
-- Local `postgres` is rolsuper=false. On a fresh CLI stack it is already a
-- member (SET) of anon/authenticated/service_role/agent_reader/agent_writer but
-- not of the two edge roles; grant SET-only membership so the runtime sweep can
-- impersonate them exactly like the others. INHERIT FALSE keeps postgres-context
-- assertions unaffected, and the grant rolls back with the outer transaction.
GRANT edge_catalog_reader, edge_rls_user TO postgres
  WITH ADMIN FALSE, INHERIT FALSE, SET TRUE;

-- dashboard_user is a Supabase-reserved role: its memberships can be granted
-- ONLY by a superuser, so a non-superuser local `postgres` cannot SET ROLE into
-- it by any mechanism (grant, or a disposable login member — both need
...
BEGIN
  FOR probe IN
    ...
    ORDER BY signature
  LOOP
    ASSERT NOT has_function_privilege(p_role, probe.signature, 'EXECUTE'),
      format('%s holds a direct EXECUTE grant on %s despite no listed caller lane',
             p_role, probe.signature);
  END LOOP;
  ASSERT NOT has_function_privilege(
    p_role, 'public._finalize_spec_book_issue_00403(uuid)', 'EXECUTE'),
    format('%s can execute the retired internal spec-book finalizer', p_role);
END;
$$;
...
WHERE studio.type = 'design_studio'
  AND studio.id NOT IN (
    ...
    '48600000-0000-4000-8000-000000000011',
    '48600000-0000-4000-8000-000000000012'
  )
  AND EXISTS (
    SELECT 1
...
-- replication-role window and triggers are restored after them.
...
BEGIN
  ...
  ASSERT (
      failure.error_state = '42501'
      AND failure.error_message =
        'invoice milestone latch: direct detach is not allowed'
    ) OR (
      failure.error_state = '23514'
      AND failure.error_message LIKE '%chk_line_items_milestone_kind%'
    ),
    format(
      'effective authenticated current_user did not deny direct detach: %s/%s',
...
BEGIN
  ...
  -- The foreign task lives on project ...041 and is correctly hidden from this
  -- caller by RLS, so its residue is asserted after RESET ROLE below.
  ASSERT failure.error_state = '42501'
...
RESET ROLE;
DO $cross_project_sms_residue_contract$
BEGIN
  ASSERT (
    SELECT status = 'todo'
    FROM public.project_tasks
    WHERE id = '48600000-0000-4000-8000-000000000055'
  ), 'cross-project SMS apply mutated a foreign project task';
END
$cross_project_sms_residue_contract$;
SET LOCAL ROLE authenticated;
...
BEGIN
  ...
  -- cross-studio invoice, by invoice_line_items RLS first (42501).
  ASSERT failure.error_state IN ('23514', '42501')
     AND NOT EXISTS (
       SELECT 1 FROM public.invoice_line_items
       WHERE id = '48600000-0000-4000-8000-000000000075'
     ), format('direct milestone latch accepted a cross-studio invoice designer: %s/%s',
        failure.error_state, failure.error_message);
END
...
BEGIN
  ...
  -- Async dblink results must be drained to completion or the next command on
  -- the connection fails with "another command is already in progress".
  PERFORM drained.receipt
  FROM extensions.dblink_get_result('scope486_sender', false)
    AS drained(receipt text);
...
BEGIN
  ...
  PERFORM extensions.dblink_exec('invoice486_setup', $cleanup$
    ...
    WHERE id = '48680000-0000-4000-8000-000000000001';
    DELETE FROM auth.users
    WHERE id = '48680000-0000-4000-8000-000000000001';
  $cleanup$);
  PERFORM extensions.dblink_exec('invoice486_setup', $setup$
    -- The setup runs under session_replication_role = replica (FK triggers
    -- suspended), but the generator backend enforces invoices_designer_id_fkey
    -- (-> profiles) normally, so the lead designer needs a committed auth.users
    -- + profiles row, not just the FK-suspended org/project rows below.
    INSERT INTO auth.users (
      ...
      'sd488-invoice-lock-designer@test.invalid', '', now(), now(), now(),
  ...
  -- Drain the async result stream to completion before COMMIT on this same
  -- connection, or dblink reports "another command is already in progress".
  PERFORM drained.invoice_id
  FROM extensions.dblink_get_result('invoice486_generate', false)
    AS drained(invoice_id uuid);
  ...
  PERFORM extensions.dblink_send_query(
    'invoice486_detach',
    format(
      -- Also drop kind to 'adhoc' so the row clears chk_line_items_milestone_kind
      -- (kind='milestone' requires milestone_id) and actually reaches the AFTER
      -- latch, which is the detach guard under test — it rejects detaching a
      -- milestone line from a non-void invoice with "cannot detach milestone".
      'UPDATE public.invoice_line_items SET milestone_id = NULL, kind = ''adhoc'' WHERE invoice_id = %L::uuid RETURNING id',
      v_invoice_id
    )
  );
  FOR v_attempt IN 1..40 LOOP
  ...
  -- Drain the failed query's trailing result so the connection returns to idle
  -- before ROLLBACK, or dblink_exec re-raises the stored error.
  PERFORM drained.id
  FROM extensions.dblink_get_result('invoice486_detach', false)
    AS drained(id uuid);
...
BEGIN
  ...
  -- Drain the async result to completion before COMMIT on this connection.
  ...
  -- Drain the failed query's trailing result before ROLLBACK on this connection.
  PERFORM drained.id
  FROM extensions.dblink_get_result('order486_scope_insert', false)
    AS drained(id uuid);
