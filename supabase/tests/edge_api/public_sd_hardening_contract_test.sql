...
VALUES
  ...
  (
    -- 00559 rewrote the studio-discovery branch (00511's hash was
    ...
    ARRAY['search_path=pg_catalog, public, pg_temp']::text[],
    '8aca199be7f059b37c7b0148d39a4b38e0a725f77cb0aa4e939730712ca050f8',
    ARRAY[]::text[]
  ),
  (
...
BEGIN
  ASSERT NOT EXISTS (
    WITH expected(signature, root_is_inherent, locks_role_catalog) AS (
      VALUES
        ('public.set_project_studio_id()'::text, true, true),
        ('public.set_invoice_studio_id()'::text, false, false),
        ('public.create_draft_invoice(uuid,uuid,uuid,uuid,numeric,integer,text,text,jsonb)'::text, false, true),
        ('app_private.issue_invoice_for_actor(uuid,date,uuid)'::text, false, true),
        ('public._countersign_design_services_agreement_impl(uuid,text,jsonb)'::text, false, true),
        ('public._execute_furnishings_authorization_authorized(uuid,text,uuid,text)'::text, false, true),
        ('public._execute_furnishings_authorization_on_paper_authorized(uuid,text,date,uuid,uuid,jsonb)'::text, false, true),
        ('public._execute_trade_scope_authorized(uuid,text,uuid,text)'::text, false, true),
        ('public._execute_trade_scope_on_paper_authorized(uuid,text,date,uuid,uuid)'::text, false, true),
        ('public.issue_trade_draw_invoice(uuid)'::text, false, true)
    ),
    positions AS (
      SELECT expected.*,
        LEAST(
    ...
    SELECT 1
    FROM positions
    WHERE user_role_position = 0
       OR membership_position IS NULL
       OR studio_position = 0
       -- The global roles-catalog FOR SHARE is present exactly when expected:
       -- every commercial-authority function takes it EXCEPT set_invoice_studio_id,
       -- whose per-designer user_roles FOR SHARE alone carries the reassignment
       -- guard. The roles-catalog rows are shared reference data a reassignment
       -- never mutates, so locking them only serialized every invoice insert.
       OR (locks_role_catalog AND role_position = 0)
       OR (NOT locks_role_catalog AND role_position <> 0)
       OR (locks_role_catalog AND NOT (role_position < user_role_position))
       OR NOT (
         user_role_position < membership_position
       ...
       OR (
         ...
         AND (
           ...
           OR NOT (
             root_lock_position < CASE WHEN locks_role_catalog
                                       THEN role_position
                                       ELSE user_role_position END
           )
         )
       )
...
VALUES ('d4853280-0000-4000-8000-000000000001', 'SD Vendor')
...
-- transition behind the dedicated signature RPC (per-document app GUC), which a
-- multi-row fixture UPDATE cannot carry; suspend triggers for the seed, exactly
-- as the other fixture blocks in this file do. We are bare postgres here (the
-- role was RESET above), so replica mode is permitted.
SET LOCAL session_replication_role = replica;
...
-- The commercial_signatures_studio_read RLS policy exposes signatures only to
-- studio co-members of the designer, so the signing client cannot read its own
-- row back. Verify the persisted evidence as postgres (RLS-bypassing) — the same
-- lens the negative signature contract above uses for durable-state assertions.
DO $signature_guard_authenticated_success_evidence$
BEGIN
  ASSERT EXISTS (
    SELECT 1
    FROM public.commercial_document_signatures AS signature
    WHERE signature.proposal_id = 'd4853000-0000-4000-8000-000000000040'
      AND signature.signer_user_id = 'd4850000-0000-4000-8000-000000000004'
      AND signature.metadata->>'via' = 'sign_design_services_agreement'
  ), 'exact authenticated client did not persist a real design signature';
END
$signature_guard_authenticated_success_evidence$;
...
BEGIN
  ...
  -- Draft-invoice evidence for the activation kickoff is verified after RESET
  -- ROLE below; invoices RLS hides draft invoices from the signing client.
  ...
  -- REVIEW FLAG: _apply_client_decision_authorized never captures a previous
  -- value; it deliberately CLEARS this capability to '' after settling, unlike
  -- the activation/commercial capabilities that restore the caller's prior
  -- value. The security property under test — the elevated decision-write
  -- capability is not left holding the decision id — is preserved either way;
  -- assert the clear the impl actually performs. The clear-vs-restore
  -- inconsistency with sibling capabilities is a real design question raised
  -- for the reviewer, not silently accepted.
  ASSERT current_setting('app.client_decision_write_id', true) = '',
    'decision settlement leaked or failed to clear its write capability';
END
$real_authenticated_client_invoice_capabilities$;

RESET ROLE;

-- Draft invoices are invisible to the client under invoices RLS, so verify the
-- activation kickoff and decision-settlement drafts as postgres (RLS-bypassing)
-- — the drafts exist; only the client's read of them is policy-restricted.
DO $real_authenticated_client_invoice_evidence$
BEGIN
  ASSERT 1 = (
    SELECT count(*)
  ...
  ASSERT 1 = (
    ...
    WHERE milestone.id = 'd4853350-0000-4000-8000-000000000002'
      AND invoice.status = 'draft'
      AND invoice.project_id = 'd4852000-0000-4000-8000-000000000001'
      AND invoice.client_id = 'd4850000-0000-4000-8000-000000000004'
      AND invoice.designer_id = 'd4850000-0000-4000-8000-000000000001'
      AND line.amount_cents = 2000
  ), 'exact client decision settlement did not draft one milestone line/latch';
END
$real_authenticated_client_invoice_evidence$;

-- Restore designer 001's second studio membership now that the canonical
-- single-studio activations above (the addendum origin and this proposal) have
-- resolved. set_project_studio_id derives an activating project's studio from
-- the designer's sole active design_studio, so 001 had to stay unambiguous
-- through both; the later probes rely on the resulting ambiguity being real.
UPDATE public.organization_members
SET status = 'active'
WHERE id = 'd4851100-0000-4000-8000-000000000003';
...
-- These fixture invoices/lines are seeded directly. set_invoice_studio_id's
-- FOR SHARE OF user_role reads the RLS-protected user_roles table, which is
-- empty under a direct authenticated writer (FOR SHARE + RLS returns no row even
-- for the writer's own role — plain SELECT does), so a direct authenticated
-- INSERT is rejected by design; the real flow creates invoices through the
-- SECURITY DEFINER composer RPCs. Seed as postgres, which takes the trigger's
-- documented migration bypass, then return to the authenticated actor for the
-- issue_invoice / handoff RPCs below. (This FOR SHARE+RLS visibility gap in the
-- retained user_role check is pre-existing — it reproduces identically with the
-- removed roles-catalog lock present — and is flagged for review.)
RESET ROLE;
INSERT INTO public.invoices (
  id, project_id, designer_id, client_id, studio_id,
  status, currency, subtotal_cents, total_cents
