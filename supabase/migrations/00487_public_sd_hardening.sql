-- ═══════════════════════════════════════════════════════════════════════════
-- 00487 — Public SECURITY DEFINER hardening
--
-- Lineage:
...
BEGIN
  IF current_user <> 'postgres' THEN
    RAISE EXCEPTION '00487 must run as postgres; got %', current_user;
  END IF;
  ...
  LOOP
    IF to_regrole(required_role) IS NULL THEN
      RAISE EXCEPTION '00487 required role % is missing', required_role;
    END IF;
  END LOOP;

  IF to_regnamespace('app_private') IS NULL THEN
    RAISE EXCEPTION '00487 requires the app_private schema';
  END IF;
END
$preflight$;

CREATE TEMP TABLE _00487_profile (
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00487_profile AS expected
    ...
    RAISE EXCEPTION '00487 reviewed source/final routine profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    original_expected AS (
      ...
      FROM _00487_profile AS profile
    ...
    RAISE EXCEPTION '00487 reviewed source/final direct ACL drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 unreviewed source/final direct EXECUTE row exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 routine profile and ACL states are incoherent';
  END IF;
END
$profile_preflight$;

CREATE TEMP TABLE _00487_dependency_profile (
  signature text PRIMARY KEY,
  language_name text NOT NULL,
  arguments text NOT NULL,
  result_type text NOT NULL,
...
INSERT INTO _00487_dependency_profile (
...
UPDATE _00487_dependency_profile
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00487_dependency_profile AS expected
    LEFT JOIN pg_proc AS routine
      ON routine.oid = to_regprocedure(expected.signature)
    LEFT JOIN pg_roles AS owner ON owner.oid = routine.proowner
    LEFT JOIN pg_language AS language ON language.oid = routine.prolang
    ...
    RAISE EXCEPTION '00487 dependency source/final profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 dependency profile and ACL states are incoherent';
  END IF;
END
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 unexpected private invoice core overload exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 existing private invoice core profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 existing private invoice core ACL drifted';
  END IF;
END
...
BEGIN
  IF routine_oid IS NULL THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION '00487 unexpected create_draft_invoice overload exists';
    END IF;
    RETURN;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 existing atomic draft profile drifted';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00487 existing atomic draft ACL/overload drifted';
  END IF;
END
...
BEGIN
  ...
  IF NOT (source_ok OR final_ok) THEN
    RAISE EXCEPTION '00487 source/final studio trigger binding drifted';
  END IF;
  ...
  IF source_ok AND (
    ...
    RAISE EXCEPTION '00487 source trigger body/binding states are incoherent';
  END IF;
  ...
  IF final_ok AND (
    ...
    RAISE EXCEPTION '00487 final trigger body/binding states are incoherent';
  END IF;
END
...
BEGIN
  FOR routine_oid IN
    SELECT to_regprocedure(signature)::oid FROM _00487_profile
    UNION ALL
    SELECT to_regprocedure(signature)::oid
    FROM _00487_dependency_profile
    UNION ALL
...
DO $postconditions$
BEGIN
  IF (SELECT count(*) FROM _00487_profile) <> 17 THEN
    RAISE EXCEPTION '00487 routine manifest must contain exactly 17 rows';
  END IF;

  IF (SELECT count(*) FROM _00487_dependency_profile) <> 8 THEN
    RAISE EXCEPTION '00487 dependency manifest must contain exactly eight rows';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 final routine semantic/body profile is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 final direct ACL contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 unreviewed final direct EXECUTE row exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 final dependency semantic/body profile is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 final dependency direct ACL contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 private invoice core has a nonowner ACL row';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00487 private invoice core profile is wrong';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00487 atomic draft profile/ACL/overload is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 atomic draft database caller universe is not empty';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 retained trigger caller contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 service boundary authority is claim-derived';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 hardened actor boundary rewrites JWT claims';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00487 review publication reads before authorization';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 trigger authority contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 canonical root/authority/child lock order drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION '00487 trigger row capability is missing';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 commercial signature guard authority is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 sign/accept sibling invoice audit drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 private invoice core caller universe is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00487 invoice core caller metadata/actor contract is wrong';
  END IF;
END
$postconditions$;
