-- ═══════════════════════════════════════════════════════════════════════════
-- 00211 — document_state v7: proposal_updated_at (R45)
--
-- The Document needs to know when a live proposal (or any of its lines/sections)
-- was last touched, so the proposal surface can age/sort like the signed-project
-- surface does. This recreates the 00202 view body VERBATIM and appends a single
-- new FINAL column on every UNION branch:
--
--   · Shape B (proposal): proposal_updated_at = greatest(pr.updated_at, the most
--     recent proposal_items.updated_at, the most recent proposal_sections.updated_at)
--     — the live proposal version (pr.id), not the chain root. Both child tables
--     carry updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW() (proposal_items 00014,
--     proposal_sections 00063); coalesce guards the empty-children case.
--   · Shapes A / C / D: null::timestamptz as proposal_updated_at, at the SAME
--     position, so the UNION column list lines up (UNION matches by position).
--
-- Supersedes the 00202 document_state definition ADDITIVELY — every existing
-- column, the four-shape UNION order, and security_invoker are unchanged; this
-- only appends one column. No other view (margin_items) is touched. The grants
-- last issued in 00195 are re-issued explicitly here (00200/00202 relied on
-- create-or-replace grant preservation; re-issuing is idempotent and defensive).
--
-- Additive only (D7). Old shapes and consumers untouched.
-- ═══════════════════════════════════════════════════════════════════════════
...
comment on view document_state is
  'The Document read model (R1 shapes A–D). v7 (00211): + proposal_updated_at — '
  'greatest of the live proposal''s updated_at and its newest proposal_items / '
  'proposal_sections updated_at on shape B; null on shapes A/C/D (R45). '
  'Supersedes the 00202 v6 definition additively (one appended column).';

-- Re-issue grants explicitly (last set in 00195; create-or-replace preserves
-- them but re-issuing is idempotent and defends any column/owner edge case).
