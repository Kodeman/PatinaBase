-- ============================================================================
-- 00140: Carry richer proposal data on activation
--
-- Tier 2.8 from the proposal↔project gap matrix:
--   - proposals.description           → projects.notes
--   - proposals.personal_message      → projects.kickoff_message
--   - proposal_items.internal_notes   → appended to project_ffe_items.notes
--   - proposal_items.lead_time_weeks  → seeds project_ffe_items.eta
--   - proposal_palettes + swatches    → new project_palettes table (swatches
--                                       embedded as JSONB for read-mostly use)
-- ============================================================================

ALTER TABLE projects
  ADD COLUMN IF NOT EXISTS kickoff_message TEXT;

CREATE TABLE IF NOT EXISTS project_palettes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  source_palette_id UUID REFERENCES proposal_palettes(id) ON DELETE SET NULL,
  name TEXT NOT NULL,
  is_primary BOOLEAN NOT NULL DEFAULT FALSE,
  source_image_url TEXT,
  notes TEXT,
  scope_room_id UUID REFERENCES project_rooms(id) ON DELETE SET NULL,
  -- Swatches embedded as JSONB array of {hex, name, role, brand, brand_code, sort_order}
  -- mirroring palette_swatches without a relational join. Sufficient for the
  -- read-mostly post-activation use case.
...
CREATE INDEX IF NOT EXISTS idx_project_palettes_project ON project_palettes(project_id);
CREATE INDEX IF NOT EXISTS idx_project_palettes_room ON project_palettes(scope_room_id);

-- Only one primary palette per project.
CREATE UNIQUE INDEX IF NOT EXISTS idx_project_palettes_one_primary
  ON project_palettes(project_id)
  WHERE is_primary = TRUE;

ALTER TABLE project_palettes ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Inherit project access for palettes"
  ON project_palettes FOR ALL
  USING (
    EXISTS (
...
DROP TRIGGER IF EXISTS set_updated_at ON project_palettes;
CREATE TRIGGER set_updated_at
  BEFORE UPDATE ON project_palettes
...
BEGIN
  ...
  -- Palettes — copy each proposal_palettes row plus its swatches as embedded JSONB.
  FOR v_palette IN
