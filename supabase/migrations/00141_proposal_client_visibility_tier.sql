-- ============================================================================
-- 00141: Capture client visibility tier on the proposal
--
-- `projects.client_visibility_tier` (00084) historically defaulted to
-- 'milestone' on activation. The 7-step activation wizard let designers
-- override it at step 6, after the client had already signed. That's too
-- late — visibility shape should be part of what the client agrees to.
--
-- This migration adds the same column to `proposals` so the designer
-- can pick the tier while scoping. The activation RPC copies it forward.
-- Existing proposals default to 'milestone' so behavior is unchanged.
-- ============================================================================

ALTER TABLE proposals
  ADD COLUMN IF NOT EXISTS client_visibility_tier TEXT
    DEFAULT 'milestone';
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND constraint_name = 'proposals_client_visibility_tier_check'
  ) THEN
    ALTER TABLE proposals
      ADD CONSTRAINT proposals_client_visibility_tier_check
