...
COMMENT ON COLUMN public.studio_channel_consent.refusal_unanswered IS
  ...
  'YES or START (sms-inbound/pipeline.ts writeChannelConsent). '
  'THAT DOOR IS A START, AND IT REACHES THIS FLAG BY THE VERDICT, NOT BY THE '
  'STATUS COLUMN (close-out r4 MAJOR-1): the rail''s START target filter folds '
  'refusal_unanswered into the word the same way channel_consent_status() does '
  '(pipeline.ts recordVerdict), so a folded record sitting at `granted` or '
  '`not_asked` with the flag raised is in the target set. Reading the raw '
  'column there left this whole population — the one the flag was invented for '
  '— with no recipient-side door at all, permanently unsendable, while the '
  'party sheet told the designer "Only they can rejoin by replying START". '
  'The YES leg is NOT that door: it still gates on a project_parties seat at '
  '`pending`, a seat state no consent act can produce since the freeze, so it '
  'is owed to W2 with the rest of the seat readers. No RPC in this '
...
COMMENT ON FUNCTION public.channel_consent_status(uuid, text, text) IS
  ...
  'places, which is the thing R-AS exists to stop. The inbound rail applies the '
  'same fold in TypeScript (sms-inbound/pipeline.ts recordVerdict, close-out r4 '
  'MAJOR-1), so the START that lowers the flag reaches exactly the records this '
  'function calls `opted_out`; the rail reading the raw status column while the '
  'room read the verdict was this defect one layer up. SECURITY INVOKER, so the '
