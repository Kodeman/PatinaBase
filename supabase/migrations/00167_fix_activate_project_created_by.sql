-- 00167_fix_activate_project_created_by.sql
-- Fix: activate_proposal_as_project() omitted projects.created_by since the
-- 00140 rewrite, so activation failed with a NOT NULL violation on
-- projects.created_by (an earlier 00086 version set it). Re-add it, sourced
-- from the proposal's designer_id. Body is the live 00141-era definition
-- (carries client_visibility_tier + line_total_cents) plus the created_by line.
