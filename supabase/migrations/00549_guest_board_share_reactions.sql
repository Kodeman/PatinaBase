-- ═══════════════════════════════════════════════════════════════════════════
-- 00549 — Verdict-capable guest board share links
--
...
--
-- Lineage
--   create_board_share             00406 → 00434 → 00462 → 00548 → 00549
--   resolve_board_share            00406 → 00434 → 00462 → 00548 → 00549
--   guard_document_share_…payload  00462 → 00549
--   reply_to_item_feedback         00267 → 00549
--   resolve_item_feedback          00267 → 00549
--   reopen_item_feedback           00267 → 00549
--   notify_item_feedback           00267 → 00549
--   escalate_item_feedback_to_…    00271 → 00549
-- ═══════════════════════════════════════════════════════════════════════════
...
COMMENT ON COLUMN public.item_feedback.guest_share_id IS
  ...
  'ON DELETE CASCADE is deliberate (00549): revoking a link keeps every '
...
-- 00548's body verbatim, plus two computed keys. `reactions` is ABSENT — not
