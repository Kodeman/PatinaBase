-- ═══════════════════════════════════════════════════════════════════════════
-- 00268 — Refund reconciliation v1 (partially lifts the "refund state machine
--          is v2" deferral)
--
-- The consolidated Stripe rail now settles three payable types (invoice 00178,
-- po_payment 00266, direct_order 00267). `charge.refunded` has been ledger-only
...
COMMENT ON TYPE public.procurement_notification_kind IS
  ...
  'payment_refunded: fired by the stripe-webhook charge.refunded branch (00268) when a paid po_payment '
...
COMMENT ON COLUMN public.designer_earnings.reverses_invoice_payment_id IS
  'Set only on a refund CONTRA row (00268): the invoice_payments.id whose '
...
BEGIN
  ...
  -- 00268 REVERSE EFFECT 1 — earnings contra rows for refunded payments.
  ...
  ELSIF v_was_paid AND v_new_status <> 'paid' THEN
    -- 00268 REVERSE EFFECT 2 — a refund dropped the invoice below fully-paid.
...
COMMENT ON FUNCTION public.apply_invoice_payment_effects(UUID) IS
  'Single source of truth for invoice payment side effects (00178 + 00268 '
