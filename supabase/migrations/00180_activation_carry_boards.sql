-- 00180_activation_carry_boards.sql
-- Delta: activate_proposal_as_project() now carries mood boards. After the
-- palette loop, each proposal_boards row (with its proposal_board_items) is
-- snapshotted into project_boards — items denormalized into a JSONB array
-- (mirroring the project_palettes.swatches embed, see 00179) of
--   {type, x, y, width, height, z_index, rotation,
--    product_id, image_url, content, data}
-- ordered by z_index then created_at. A board's scope_room_id is remapped to
-- the new project_rooms row via the same v_scope_room_map the palette loop
-- uses (NULL when unmapped). New DECLAREs: v_board, v_board_items.
-- Body is otherwise the live 00167 revision verbatim (created_by fix on top
-- of the 00140/00141 richer-carry rewrite).
