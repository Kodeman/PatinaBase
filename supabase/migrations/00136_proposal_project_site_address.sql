-- ============================================================================
-- 00136: Capture site address in the proposal phase
--
-- Adds `proposals.project_address` and `projects.site_address`. Updates
-- `activate_proposal_as_project()` so the address copies forward at
-- activation time. The project list/header has been rendering an
-- `address` / `client_location` derivation that has no real persisted
-- source for v2 projects — this column gives it one.
--
-- Existing rows are not backfilled; both columns default NULL.
-- ============================================================================
...
-- Replace the RPC; identical to the 00097 body except for the address copy.
