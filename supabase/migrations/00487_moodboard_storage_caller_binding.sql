-- ═══════════════════════════════════════════════════════════════════════════
-- 00487 — Bind the caller in the proposal-mood-boards SELECT policy
--
...
COMMENT ON POLICY proposal_mood_boards_proposal_read ON storage.objects IS
  'Proposal-prefixed mood-board media is readable by the owning design studio '
  'and by the proposal''s client once it has been issued. Guest share '
  'surfaces are served server-side by service_role (BYPASSRLS) and need no '
  'leg here; before 00487 this policy proved only that the proposal existed '
