-- ═══════════════════════════════════════════════════════════════════════════
-- 00488 — Caller-backed public SECURITY DEFINER hardening
--
...
-- no-literal caller lane hardened by 00487.
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  IF current_user <> 'postgres' THEN
    RAISE EXCEPTION '00488 must run as postgres; got %', current_user;
  END IF;
  ...
  LOOP
    IF to_regrole(required_role) IS NULL THEN
      RAISE EXCEPTION '00488 required role % is missing', required_role;
    END IF;
  END LOOP;
END
$preflight_roles$;

CREATE TEMP TABLE _00488_routine_profile (
...
INSERT INTO _00488_routine_profile (
...
WHERE signature IN (
  ...
  -- service_role from creation; only 00488 narrows them to authenticated.
  'public.escalate_item_feedback_to_decision(uuid,uuid)',
...
CREATE TEMP TABLE _00488_dependency_profile (
  signature text PRIMARY KEY,
  arguments text NOT NULL,
  result_type text NOT NULL,
...
INSERT INTO _00488_dependency_profile VALUES
  (
  ...
  (
    ...
    -- Creation-time ALTER DEFAULT PRIVILEGES grant that 00488 revokes.
    ARRAY['service_role'], ARRAY[]::text[], true
  ),
  (
...
CREATE TEMP TABLE _00488_lock_order_profile (
  signature text PRIMARY KEY,
  arguments text NOT NULL,
  result_type text NOT NULL,
  language_name text NOT NULL,
...
INSERT INTO _00488_lock_order_profile VALUES
  (
...
BEGIN
  IF NOT pg_temp._00488_references_routine(
    ...
    'public', '_draft_invoice_from_milestone_00486'
  ) OR pg_temp._00488_references_routine(
    $source$SELECT "public"."_DRAFT_INVOICE_FROM_MILESTONE_00486"($1)$source$,
    'public', '_draft_invoice_from_milestone_00486'
  ) OR pg_temp._00488_references_routine(
    ...
    RAISE EXCEPTION '00488 routine-reference scanner contract failed';
  END IF;
END
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00488_routine_profile AS expected
    ...
    RAISE EXCEPTION '00488 reviewed source profile/body/config/ACL drifted';
  END IF;
END
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00488_dependency_profile AS expected
    LEFT JOIN pg_proc AS routine
      ON routine.oid = to_regprocedure(expected.signature)
    LEFT JOIN pg_roles AS owner ON owner.oid = routine.proowner
    LEFT JOIN pg_language AS language ON language.oid = routine.prolang
    ...
    RAISE EXCEPTION '00488 dependent relay profile/body/config/ACL drifted';
  END IF;
END
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00488_lock_order_profile AS expected
    LEFT JOIN pg_proc AS routine
      ON routine.oid = to_regprocedure(expected.signature)
    LEFT JOIN pg_roles AS owner ON owner.oid = routine.proowner
    LEFT JOIN pg_language AS language ON language.oid = routine.prolang
    ...
    RAISE EXCEPTION '00488 lock-order dependency profile/body/config/ACL drifted';
  END IF;
END
...
DECLARE
  expected _00488_routine_profile%ROWTYPE;
  app_role text;
BEGIN
  FOR expected IN SELECT * FROM _00488_routine_profile ORDER BY signature
  LOOP
    EXECUTE format(
...
DECLARE
  expected _00488_dependency_profile%ROWTYPE;
  app_role text;
BEGIN
  FOR expected IN SELECT * FROM _00488_dependency_profile ORDER BY signature
  LOOP
...
DECLARE
  expected _00488_lock_order_profile%ROWTYPE;
  app_role text;
BEGIN
  FOR expected IN SELECT * FROM _00488_lock_order_profile ORDER BY signature
  LOOP
    IF to_regprocedure(expected.signature) IS NULL THEN
      CONTINUE;
    END IF;
...
BEGIN
  IF (SELECT count(*) FROM _00488_routine_profile) <> 25 THEN
    RAISE EXCEPTION '00488 exact routine set is not 25 rows';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00488 final profile/body/config/ACL postcondition failed';
  END IF;
END
...
BEGIN
  IF (
    ...
    RAISE EXCEPTION '00488 exact invoice-line policy set drifted';
  END IF;
END
...
BEGIN
  IF (SELECT count(*) FROM _00488_dependency_profile) <> 13 THEN
    RAISE EXCEPTION '00488 exact dependent relay set is not 13 rows';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00488 dependent relay postcondition failed';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 scope-change integrity trigger binding drifted';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 invoice milestone latch trigger binding drifted';
  END IF;
  ...
  IF position(
    ...
    RAISE EXCEPTION '00488 line latch reacquired an AFTER-trigger parent lock';
  END IF;
  ...
  IF obj_description(
    ...
    'Exact invoice/project milestone latch validation: draft attachment is canonical; authenticated direct detach is denied; owner/service detach is accepted only after void released the latch; the AFTER trigger takes no parent row locks.'
  THEN
    RAISE EXCEPTION '00488 invoice milestone latch authority comment drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00488 retired finalizer still has a definer caller';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 scope-change helper caller graph drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00488 scope-change caller lost exact project argument';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 milestone-invoice legacy core caller graph drifted';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 milestone-invoice wrapper caller graph drifted';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 SMS field-effect relay caller graph drifted';
  END IF;
END
...
BEGIN
  IF (SELECT count(*) FROM _00488_lock_order_profile) <> 4 OR EXISTS (
    SELECT 1
    FROM _00488_lock_order_profile AS expected
    LEFT JOIN pg_proc AS routine
      ON routine.oid = to_regprocedure(expected.signature)
    ...
    RAISE EXCEPTION '00488 lock-order dependency postcondition failed';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00488 lock-order trigger binding drifted';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00488 production authority caller graph drifted';
  END IF;
END
