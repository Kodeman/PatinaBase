-- ═══════════════════════════════════════════════════════════════════════════
-- 00489 — Studio-aware invoice number uniqueness
--
...
-- 00487 trade-draw access-control suite: designer 001 owns studio
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION
      '00489 studio-aware invoice uniqueness indexes are missing';
  END IF;
  IF to_regclass('public.uniq_invoices_designer_number') IS NOT NULL THEN
    RAISE EXCEPTION
      '00489 legacy per-designer invoice number index was not retired';
  END IF;
END
$postcondition$;
