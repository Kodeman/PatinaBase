-- ═══════════════════════════════════════════════════════════════════════════
-- 00230 — document_state: surface proposal OPEN COUNT on the Desk grain (R71 / Phase 2)
-- ═══════════════════════════════════════════════════════════════════════════
-- The watch view (R71) shows opened-xN inline in the document; the Desk only
-- knew first-open (proposal_viewed_at). This APPENDS two read-only rollups over
-- proposal_engagement to EVERY shape so the Desk chip can read "Opened 3x, last
-- Jun 22":
--   proposal_open_count      : count of 'opened' engagement events (shape B only;
--                              0 on A/C/D to keep the four-shape UNION aligned).
--   proposal_last_opened_at  : max('opened' event ts); null elsewhere.
--
-- Additive only (D7): recreates the 00219 v8 body VERBATIM and appends the two
-- columns at the SAME final position in all four arms (UNION matches by
-- position). Every prior column, order, and the four-shape UNION are preserved.
-- security_invoker stays true - base-table RLS on proposal_engagement applies.
-- ═══════════════════════════════════════════════════════════════════════════
...
comment on view document_state is
  'The Document read model (R1 shapes A-D). v9 (00230): + proposal_open_count / '
  'proposal_last_opened_at - opened-event rollups over proposal_engagement on '
  'shape B (0/null on A/C/D). Supersedes the 00219 v8 definition additively (two '
  'appended columns; every prior column/order unchanged).';
