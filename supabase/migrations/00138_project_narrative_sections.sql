-- ============================================================================
-- 00138: Preserve proposal narrative on the active project
--
-- Adds `project_narrative_sections` mirroring `proposal_sections` (00063).
-- `activate_proposal_as_project()` now copies each proposal section into
-- the new table so the activated project keeps the Vision / Concept /
-- Space Plan / Selections / Investment / Timeline / Terms body content
-- the designer wrote.
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS project_narrative_sections (
  ...
  source_section_id UUID REFERENCES proposal_sections(id) ON DELETE SET NULL,
  type TEXT NOT NULL,
  title TEXT NOT NULL,
  body TEXT,
...
CREATE INDEX IF NOT EXISTS idx_project_narrative_sections_project
  ON project_narrative_sections(project_id);
CREATE INDEX IF NOT EXISTS idx_project_narrative_sections_order
  ON project_narrative_sections(project_id, sort_order);

ALTER TABLE project_narrative_sections ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Inherit project access for narrative sections"
  ON project_narrative_sections FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM projects p
      WHERE p.id = project_narrative_sections.project_id
        AND (
...
DROP TRIGGER IF EXISTS set_updated_at ON project_narrative_sections;
CREATE TRIGGER set_updated_at
  BEFORE UPDATE ON project_narrative_sections
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Replace the RPC; identical to 00137 plus a final block that copies
-- proposal_sections into project_narrative_sections.
...
BEGIN
  ...
  -- Copy proposal_sections into project_narrative_sections so the project
  -- preserves the designer's authored Vision / Concept / Space Plan /
  -- Selections / Investment / Timeline / Terms content.
  FOR v_section IN
