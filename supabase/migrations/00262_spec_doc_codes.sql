-- 00262_spec_doc_codes.sql
-- Track S · S1 (Schedule & Boards Wave 1): specification doc codes.
--
-- A doc_code is the designer's short catalog reference for a scheduled line
-- (CH-01, TB-03…) — the spine of a Programa-style schedule. Additive TEXT on
-- both the pre-sale schedule (proposal_items) and its post-sale mirror
-- (project_ffe_items); free text, no constraint (imports must never break).
--
-- Two partial lookup indexes on (owning-id, doc_code) WHERE doc_code IS NOT NULL
-- — a document's codes are looked up together (twin detection, spec lookups).
--
-- Then activate_proposal_as_project() carries doc_code onto the project mirror
-- so the schedule's spine survives the sale. The function body below is the
-- TRUE latest revision (00199 — later than the 00180 the supabase/CLAUDE.md
-- note cites; 00185 dual-pricing and 00199 vendor_id both redefine past it)
-- reproduced VERBATIM, with a single surgical addition: `doc_code` in both the
-- roomed and room-less project_ffe_items inserts (columns + values). No other
-- byte changes — see the migration's own byte-diff proof in the Wave-1 report.
-- 00199's one-time vendor_id backfill is intentionally NOT repeated (doc_code
-- is brand-new; every existing mirror row is already NULL).
...
COMMENT ON COLUMN public.proposal_items.doc_code IS
  'Designer''s short spec/catalog reference for the line (e.g. CH-01). Free text; '
  'suggested from ffe_category on add but always editable. Carried to '
  'project_ffe_items.doc_code on activation (00262).';
COMMENT ON COLUMN public.project_ffe_items.doc_code IS
  'Spec/catalog reference carried from proposal_items.doc_code on activation (00262).';

CREATE INDEX IF NOT EXISTS idx_proposal_items_doc_code
  ON public.proposal_items(proposal_id, doc_code)
  WHERE doc_code IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_project_ffe_items_doc_code
  ON public.project_ffe_items(project_id, doc_code)
  WHERE doc_code IS NOT NULL;

-- ── Redefine activation to carry doc_code (00199 body verbatim + doc_code) ──
  ...
  LOOP
    ...
    LOOP
      ...
      INSERT INTO project_ffe_items (
        ...
        product_id, name, ffe_category, item_type, doc_code,
        ...
        v_item.product_id, v_item.name, v_item.ffe_category, v_item.item_type, v_item.doc_code,
        'specified',
        v_item.quantity,
        v_item.unit_price,
