-- ═══════════════════════════════════════════════════════════════════════════
-- 00296 — Agent OS: cowork_tasks → agent_tasks bridge
--
-- Data-migrates every public.cowork_tasks row (00076) into the new
-- public.agent_tasks queue (00295), preserving id, then FREEZES cowork_tasks
...
AS $$
BEGIN
  RAISE EXCEPTION 'cowork_tasks is frozen (00296) — use agent_tasks';
END;
$$;
...
COMMENT ON TABLE public.cowork_tasks IS
  'FROZEN 2026-07-12 (00296). Superseded by public.agent_tasks (00295); all rows were data-migrated there (idempotency_key = ''cowork:''||id, source=''cowork:legacy''). A BEFORE INSERT/UPDATE/DELETE trigger rejects all writes — this table is a read-only archive. Enqueue new work via enqueue_agent_task.';
