-- ═══════════════════════════════════════════════════════════════════════════
-- 00511 — Public SECURITY DEFINER hardening
--
-- Lineage:
...
BEGIN
  IF current_user <> 'postgres' THEN
    RAISE EXCEPTION '00511 must run as postgres; got %', current_user;
  END IF;
  ...
  LOOP
    IF to_regrole(required_role) IS NULL THEN
      RAISE EXCEPTION '00511 required role % is missing', required_role;
    END IF;
  END LOOP;

  IF to_regnamespace('app_private') IS NULL THEN
    RAISE EXCEPTION '00511 requires the app_private schema';
  END IF;
END
$preflight$;

CREATE TEMP TABLE _00511_profile (
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00511_profile AS expected
    ...
    RAISE EXCEPTION '00511 reviewed source/final routine profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    original_expected AS (
      ...
      FROM _00511_profile AS profile
    ...
    RAISE EXCEPTION '00511 reviewed source/final direct ACL drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 unreviewed source/final direct EXECUTE row exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 routine profile and ACL states are incoherent';
  END IF;
END
$profile_preflight$;

CREATE TEMP TABLE _00511_dependency_profile (
  signature text PRIMARY KEY,
  language_name text NOT NULL,
  arguments text NOT NULL,
  result_type text NOT NULL,
...
INSERT INTO _00511_dependency_profile (
...
UPDATE _00511_dependency_profile
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM _00511_dependency_profile AS expected
    LEFT JOIN pg_proc AS routine
      ON routine.oid = to_regprocedure(expected.signature)
    LEFT JOIN pg_roles AS owner ON owner.oid = routine.proowner
    LEFT JOIN pg_language AS language ON language.oid = routine.prolang
    ...
    RAISE EXCEPTION '00511 dependency source/final profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 dependency profile and ACL states are incoherent';
  END IF;
END
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 unexpected private invoice core overload exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 existing private invoice core profile drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 existing private invoice core ACL drifted';
  END IF;
END
...
BEGIN
  IF routine_oid IS NULL THEN
    IF EXISTS (
      ...
      RAISE EXCEPTION '00511 unexpected create_draft_invoice overload exists';
    END IF;
    RETURN;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 existing atomic draft profile drifted';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00511 existing atomic draft ACL/overload drifted';
  END IF;
END
...
BEGIN
  ...
  IF NOT (source_ok OR final_ok) THEN
    RAISE EXCEPTION '00511 source/final studio trigger binding drifted';
  END IF;
  ...
  IF source_ok AND (
    ...
    RAISE EXCEPTION '00511 source trigger body/binding states are incoherent';
  END IF;
  ...
  IF final_ok AND (
    ...
    RAISE EXCEPTION '00511 final trigger body/binding states are incoherent';
  END IF;
END
...
BEGIN
  FOR routine_oid IN
    SELECT to_regprocedure(signature)::oid FROM _00511_profile
    UNION ALL
    SELECT to_regprocedure(signature)::oid
    FROM _00511_dependency_profile
    UNION ALL
...
DO $postconditions$
BEGIN
  IF (SELECT count(*) FROM _00511_profile) <> 17 THEN
    RAISE EXCEPTION '00511 routine manifest must contain exactly 17 rows';
  END IF;

  IF (SELECT count(*) FROM _00511_dependency_profile) <> 8 THEN
    RAISE EXCEPTION '00511 dependency manifest must contain exactly eight rows';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 final routine semantic/body profile is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 final direct ACL contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 unreviewed final direct EXECUTE row exists';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 final dependency semantic/body profile is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 final dependency direct ACL contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 private invoice core has a nonowner ACL row';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00511 private invoice core profile is wrong';
  END IF;
  ...
  IF 1 <> (
    ...
    RAISE EXCEPTION '00511 atomic draft profile/ACL/overload is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 atomic draft database caller universe is not empty';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 retained trigger caller contract is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 service boundary authority is claim-derived';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 hardened actor boundary rewrites JWT claims';
  END IF;
  ...
  IF (
    ...
    RAISE EXCEPTION '00511 review publication reads before authorization';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 canonical root/authority/child lock order drifted';
  END IF;
  ...
  THEN
    RAISE EXCEPTION '00511 trigger row capability is missing';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 commercial signature guard authority is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 sign/accept sibling invoice audit drifted';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 private invoice core caller universe is wrong';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '00511 invoice core caller metadata/actor contract is wrong';
  END IF;
END
$postconditions$;
