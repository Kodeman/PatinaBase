...
--
-- Board Room · Wave 1 (Track B). Two additive columns on proposal_boards:
--
--   • sections JSONB  — ordered array of {id, name, color?}, the same shape
--     BoardCanvas's `sections` prop (BoardSection) already accepts. Items are
--     assigned to a section by a `section_id` key written INSIDE each item's
--     existing `data` JSONB (no new proposal_board_items column). The editor's
--     "Arrange" act reads both to lay items out in per-section bands.
--
--   • status TEXT     — 'active' | 'archived'. Archived boards stay editable in
--     the builder (behind an All/Archived filter) but are excluded from every
--     read-only render surface (client proposal, designer preview, drafting
--     mirror) so the client copy never shows an archived board.
--
-- Both are additive with safe defaults; existing rows become active with no
-- sections. Idempotent (IF NOT EXISTS / CHECK guarded) so the integration
-- review can re-run it.
--
-- clone_proposal is redefined below because its boards INSERT lists EXPLICIT
-- columns (00260) — a new board column does NOT carry through a revision unless
-- the function is redefined. The body is 00260's VERBATIM, with `sections,
-- status` appended to the boards INSERT column list and its VALUES only. See
-- the byte-diff discipline documented in 00260's own header.

-- ─── Columns ─────────────────────────────────────────────────────────────────

ALTER TABLE proposal_boards
...
-- ─── clone_proposal (00260 body verbatim + boards sections/status carry) ──────
-- The ONLY change from 00260 is the boards INSERT: `sections, status` added to
-- the column list and to the SELECT-less VALUES row (carried verbatim so a
-- revision inherits the source's section layout and active/archived state).
