-- ============================================================================
-- 00142: Rename proposal_items.line_total → line_total_cents
--
-- Naming parity with project_ffe_items.line_total_cents and every other
-- "money in cents" column in the schema. The value was already integer
-- cents; only the column name changes. RENAME COLUMN preserves the
-- NOT NULL constraint and any indexes.
--
-- Two stored procedures referenced the old column and are rewritten:
--   - `consume_capture()` (00130)
--   - `activate_proposal_as_project()` (latest in 00141)
-- ============================================================================

ALTER TABLE proposal_items
  RENAME COLUMN line_total TO line_total_cents;
