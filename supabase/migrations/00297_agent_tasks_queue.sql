-- ═══════════════════════════════════════════════════════════════════════════
-- 00297 — Agent OS: agent_tasks queue core
--
...
-- in the sibling migration 00298.
--
...
COMMENT ON TABLE public.agent_tasks IS
  'Agent OS task queue (00297). One durable outbox: surfaces enqueue via enqueue_agent_task, workers claim via claim_agent_tasks (FOR UPDATE SKIP LOCKED), complete via complete_agent_task, humans decide via review_agent_task. Writes only through the SECURITY DEFINER RPCs (or service_role). Supersedes cowork_tasks (00076/00298).';
...
COMMENT ON TABLE public.agent_task_audit IS
  'Append-only audit of every agent_tasks INSERT/UPDATE/DELETE (00297). actor comes from the app.actor GUC the RPCs stamp (falling back to auth.uid()/session_user).';
