...
BEGIN
  ...
  IF v_dispatch.state = 'in_flight' THEN
    ...
    -- before provider start is a known-safe pending attempt.
    UPDATE public.proposal_send_dispatches
    SET state = CASE
          WHEN provider_started_at IS NULL THEN 'pending'
          ELSE 'ambiguous'
        END,
        claim_token = NULL,
  ...
  IF v_dispatch.state IN ('delivered', 'suppressed') THEN
    RETURN jsonb_build_object(
      'claimed', false,
      'delivery_state', v_dispatch.state,
      'attempt_count', v_dispatch.provider_attempt_count,
  ...
  IF v_dispatch.state IN ('failed', 'ambiguous')
     AND (
...
BEGIN
  ...
  IF p_delivery_state NOT IN ('delivered', 'failed', 'ambiguous') THEN
    RAISE EXCEPTION 'invalid provider completion state %', p_delivery_state
      USING ERRCODE = 'check_violation';
  END IF;

  UPDATE public.proposal_send_dispatches
  SET state = p_delivery_state,
      claim_token = NULL,
      lease_expires_at = NULL,
      delivered_at = CASE
        WHEN p_delivery_state = 'delivered' THEN clock_timestamp()
        ELSE NULL
      END,
      provider_id = CASE
        WHEN p_delivery_state = 'delivered' THEN p_provider_id
        ELSE provider_id
      END,
      last_error = CASE
        WHEN p_delivery_state = 'delivered' THEN NULL
        ELSE left(COALESCE(p_error, p_delivery_state), 2000)
      END,
      updated_at = clock_timestamp()
  WHERE id = p_dispatch_id
    AND claim_token = p_claim_token
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'proposal send claim is stale, missing, or already attempted'
      USING ERRCODE = 'object_not_in_prerequisite_state';
  END IF;
