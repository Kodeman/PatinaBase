...
describe('product configuration RPC hooks', () => {
  it('authors the whole definition with optimistic concurrency', async () => {
    ...
    const mutation = useUpsertProductConfigurationDefinition('product-1') as any;
    const input = {
      mode: 'variant',
      pricingStrategy: 'base_plus_adjustments',
      expectedRevision: 2,
  ...
  it('preserves handedness in evaluation and never retries mutations', async () => {
    ...
    const input = {
      ...
      optionValueIds: [],
      components: [{ componentId: 'left-arm', quantity: 1, handedness: 'left' }],
    };
  ...
  it('returns the auto-created custom revision from save', async () => {
    const result = {
      configuration: { id: 'configuration-1', productId: 'cabinet-1', projectId: 'project-1' },
    ...
    expect(rpc).toHaveBeenCalledWith('save_product_configuration', { p_input: input });
    ...
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['product-configurations', 'custom-revisions', 'configuration-1'],
    });
  });
