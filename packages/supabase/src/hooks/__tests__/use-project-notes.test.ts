...
// rendering. `react`'s `useEffect` is stubbed to run synchronously, capture
// its dependency array, and stash its cleanup, for `useProjectNotesRealtime`.
// ─────────────────────────────────────────────────────────────────────────────
...
// Realtime channel stub — `.channel().on().subscribe()` chain. `channelFactory`
// returns a FRESH object per call so cleanup-removes-the-right-instance is a
// meaningful assertion, not a tautology against a shared singleton.
const channelOn = vi.fn();
const channelSubscribe = vi.fn();
let lastChannel: Record<string, unknown> | undefined;
const channelFactory = vi.fn(() => {
  const channel: Record<string, unknown> = {};
  channel.on = vi.fn((..._args: unknown[]) => {
    channelOn(..._args);
    return channel;
  });
...
describe('projectNotesKeys', () => {
  it('shapes the all and list keys', () => {
    ...
    expect(projectNotesKeys.list('proj-1')).toEqual(['project-notes', 'proj-1']);
  });
});
...
describe('useProjectNotes', () => {
  ...
  // Spec §10 risk 8's "refetch-on-focus fallback": the client portal sets
  // refetchOnWindowFocus: false globally, so this hook must opt back in —
  // realtime is the primary path, but a dropped socket must self-heal on
  // focus/reconnect instead of going stale for the life of the tab.
  it('opts back into refetch on focus and reconnect', () => {
  ...
  it('selects by project_id ordered by sent_at desc and maps snake_case to camelCase', async () => {
    ...
    expect(callsTo(builder, 'order')[0].args).toEqual(['sent_at', { ascending: false }]);
...
describe('useSendProjectNote', () => {
  it('inserts with author_id from the authenticated user and returns the mapped note', async () => {
    supabaseClient.auth.getUser.mockResolvedValue({ data: { user: { id: 'author-1' } } });
    setTableResult('project_notes', {
      data: {
        id: 'note-1',
        project_id: 'proj-1',
  ...
  it('defaults enclosures to an empty array when omitted', async () => {
    ...
    const builder = builders.project_notes;
    expect(callsTo(builder, 'insert')[0].args[0]).toMatchObject({ enclosures: [] });
  });
...
describe('useRetireProjectNote', () => {
  ...
  it('invalidates the project note list on success', () => {
    const config = useRetireProjectNote() as unknown as {
      onSuccess: (data: ProjectNote, variables: { noteId: string; projectId: string }) => void;
    };
    config.onSuccess({} as ProjectNote, { noteId: 'note-1', projectId: 'proj-1' });
    expect(invalidateQueries).toHaveBeenCalledWith({
      queryKey: ['project-notes', 'proj-1'],
    });
  });
...
describe('useProjectNotesRealtime', () => {
  it('subscribes to project_notes filtered by project_id, deps on [projectId, queryClient], and removes the SAME channel instance on unmount', () => {
    ...
    expect(lastEffectDeps).toEqual(['proj-1', expect.any(Object)]);

    expect(typeof lastEffectCleanup).toBe('function');
    const subscribedChannel = lastChannel;
