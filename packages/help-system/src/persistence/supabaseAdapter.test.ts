...
function makeStubClient(initial: HelpStateBlob | null = {}): {
  ...
  const selectError: { current: { message: string } | null } = {
    current: null,
  };
  const updateError: { current: { message: string } | null } = {
    current: null,
  };
      ...
      update: (values: Record<string, unknown>) => ({
        eq: async (_column: string, _value: string) => {
          if (!updateError.current) {
            lastWrite.current =
              (values.help_state as HelpStateBlob | null) ?? null;
          }
...
describe("saveHelpState", () => {
  ...
  it("does not throw when the Supabase update fails", async () => {
    ...
    const warn = vi.spyOn(console, "warn").mockImplementation(() => undefined);
    await expect(
      saveHelpState(client, "user-1", { tours: {} }),
...
describe("createSupabaseHelpStateBackends", () => {
  it("returns empty state until hydrate resolves", () => {
    ...
    expect(
      backends.featureBackend.getFeatureAnnouncementState("anything"),
    ).toBeNull();
  });
  ...
  it("hydrate populates the cache with server state", async () => {
    const { client } = makeStubClient({
      ...
      featureAnnouncements: {
        "v2-tags": { dismissedAt: "2026-05-19T00:00:00Z" },
      },
    });
    const backends = createSupabaseHelpStateBackends(client, "user-1");
    ...
    expect(
      backends.featureBackend.getFeatureAnnouncementState("v2-tags"),
    ).toEqual({
  ...
  it("clearTourState drops the record + writes the cleared blob through, leaving siblings", async () => {
    ...
    expect(backends.tourBackend.getTourState("keep")).toEqual({
      abandoned: true,
    });
  });
...
describe("migrateLocalToSupabase", () => {
  ...
  it("copies feature announcement state and clears the local key", async () => {
    ...
    expect(
      lastWrite.current?.featureAnnouncements?.["v2-batch-tagging"],
    ).toEqual({
      dismissedAt: "2026-05-19T00:00:00Z",
    });
    expect(
      window.localStorage.getItem("patina.help.feature_announcement.v1"),
    ).toBeNull();
  });
  ...
  it("sweeps patina:margin-note:* keys into the margin-note backend and clears them", async () => {
    window.localStorage.setItem(
      "patina:margin-note:doc-first-touch",
      "1747699200000",
    );
    const { client } = makeStubClient({});
    const backends = createSupabaseHelpStateBackends(client, "user-1");
