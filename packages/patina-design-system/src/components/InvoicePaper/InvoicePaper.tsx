...
export interface InvoicePaperLine {
  ...
  amount_cents: number;
  /** Fix round 1 (finding M2) — a kind='time' line's dated sub-table (HT-21)
   *  rides `metadata.attribution` (see `parseTimeSubtable` below); every
   *  other kind renders unaffected without either field. */
  kind?: string;
...
const TIME_SUBTABLE_KIND = "patina_time_subtable";
...
/**
 * A time line's dated sub-table (HT-21) — the SAME `metadata.attribution`
 * field a furnishings line uses for a plain vendor-name string, grown a
 * second, JSON-encoded shape discriminated by `kind` (see
 * `invoice-composer.ts`). Carries date/minutes/rate ONLY — never a name
 * (LEAH-15, REP-15: the homeowner gets no staffing detail).
 *
 * Fix round 1 (finding M2): mirrored from the client pay-link sheet's own
 * parser (`apps/client-portal/.../invoice-sheet.tsx`) so the printed/PDF
 * copy — this component, shared by both portals' print routes — shows the
 * same detail instead of silently dropping it. `invoice.line_items` here is
 * the raw `invoice_line_items` row (via `useInvoice`'s `select('*')`), so
 * the payload is read off `metadata.attribution`, not a flattened
 * top-level `attribution` field the way `resolve_invoice_link`'s (00588)
 * token-gated RPC shape returns it.
 */
function parseTimeSubtable(
  metadata: Record<string, unknown> | null | undefined,
): TimeSubtableRow[] | null {
  ...
  if (
    ...
    (parsed as { kind?: unknown }).kind !== TIME_SUBTABLE_KIND ||
...
export function InvoicePaper({
  ...
  const taxPercent = (Number(invoice.tax_rate) * 100)
    .toFixed(2)
    .replace(/\.?0+$/, "");
    ...
    >
      ...
      <div className="mb-10 grid grid-cols-3 gap-6">
        ...
        <div>
          ...
          <PrintLabel>
            {invoice.project_id ? "Project" : "Regarding"}
          </PrintLabel>
          <div style={{ fontSize: "0.85rem", lineHeight: 1.5 }}>
      ...
      >
        ...
        <tbody>
          {(invoice.line_items ?? []).map((line) => {
            // Fix round 1 (finding M2) — the dated sub-table (HT-21) prints
            // beneath a time line's description, matching the client
            // pay-link sheet. No member name rides here (LEAH-15/REP-15).
            const timeSubtable =
              line.kind === "time" ? parseTimeSubtable(line.metadata) : null;
            return (
              <tr key={line.id} style={{ borderBottom: "1px solid #E5E2DD" }}>
                <td className="py-2.5 pr-4">
                  ...
                  {timeSubtable && (
                    ...
                    >
                      {timeSubtable.map((row, i) => (
                        <div key={`${row.date}-${i}`}>
                          {formatInvoiceDate(row.date)} ·{" "}
                          {formatMinutesAsHours(row.minutes)} ·{" "}
                          {formatCurrency(row.rateCents, invoice.currency)}/hr
                        </div>
                      ))}
                    </div>
                  )}
      ...
      >
        Net {invoice.payment_terms_days} · {invoice.currency} · Prepared with
        Patina
      </div>
    </div>
  );
