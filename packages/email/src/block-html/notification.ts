...
export function renderNotification(props: NotificationBlockProps, _ctx: RenderContext): string {
  const detailRows = (props.details || []).map((d) =>
    `<tr>
                              <td class="ink3" style="padding:6px 0;font-family:${FONTS.sans};font-size:13px;color:${COLORS.ink3};border-bottom:1px solid ${COLORS.line};">${esc(d.key)}</td>
                              <td class="ink" style="padding:6px 0;font-family:${FONTS.sans};font-size:13px;font-weight:500;color:${COLORS.ink};text-align:right;border-bottom:1px solid ${COLORS.line};">${esc(d.value)}</td>
                            </tr>`
  ).join('\n');
