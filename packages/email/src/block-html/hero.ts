...
export function renderHero(props: HeroBlockProps, _ctx: RenderContext): string {
  return `          <tr>
            <td style="padding:40px 40px 32px;" class="mobile-padding">
              ...
              <h1 style="margin:0 0 12px;font-family:${FONTS.serif};font-size:28px;font-weight:600;line-height:1.2;letter-spacing:-0.015em;color:${COLORS.ink};">${esc(props.headline)}</h1>
              <p style="margin:0;font-family:${FONTS.sans};font-size:15px;line-height:1.6;color:${COLORS.ink3};">${esc(props.subline)}</p>
            </td>
          </tr>`;
}
