...
function renderGridItem(p: ProductGridProduct): string {
  const image = p.image_url
    ...
    : `<div style="height:180px;background:${COLORS.card};border-radius:6px;"></div>`;
                      ...
                      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                        ...
                        <tr>
                          <td style="padding:10px 0 0;">
                            ...
                            <p style="margin:0 0 4px;font-family:${FONTS.sans};font-size:14px;font-weight:600;color:${COLORS.ink};">${esc(p.product_name)}</p>
                            <p style="margin:0 0 6px;font-family:${FONTS.sans};font-size:12px;line-height:1.4;color:${COLORS.ink3};">${esc(p.description)}</p>
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                              <tr>
                                <td><span style="font-family:${FONTS.sans};font-size:15px;font-weight:600;color:${COLORS.ink};">${esc(p.price)}</span></td>
...
export function renderProductGrid(props: ProductGridProps, _ctx: RenderContext): string {
  ...
  if (products.length === 0) {
    return `          <tr>
            <td style="padding:16px 40px;text-align:center;font-family:${FONTS.sans};color:${COLORS.ink3};font-size:14px;" class="mobile-padding">
