...
export function renderMakerSpotlight(props: MakerSpotlightProps, _ctx: RenderContext): string {
  const portrait = props.portrait_url
    ...
    : `<div class="hairbg" style="width:80px;height:80px;border-radius:50%;background:${COLORS.line};margin:0 auto 12px;"></div>`;
              ...
              <table role="presentation" class="chip" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${COLORS.cardAlt};border:1px solid ${COLORS.line};border-radius:8px;overflow:hidden;">
                <tr>
                  <td style="padding:32px;text-align:center;">
                    ...
                    <h3 class="ink" style="margin:0 0 12px;font-family:${FONTS.serif};font-size:20px;font-weight:600;color:${COLORS.ink};">${esc(props.maker_name)}</h3>
                    <p class="ink3" style="margin:0 0 16px;font-family:${FONTS.sans};font-size:14px;line-height:1.6;color:${COLORS.ink3};text-align:left;">${esc(props.story)}</p>
