...
export function renderProductCard(props: ProductCardProps, _ctx: RenderContext): string {
  ...
  return `          <tr>
            <td class="px" style="padding:8px 40px 16px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border:1px solid ${COLORS.line};border-radius:8px;overflow:hidden;">
                <tr><td>${image}</td></tr>
                <tr>
                  <td style="padding:20px 24px;">
                    ...
                    ${props.product_url && props.product_url !== '#' ? `<table role="presentation" class="btn" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin-top:16px;"><tr><td align="center" style="border-radius:7px;background:${COLORS.brass};"><a href="${esc(props.product_url)}" style="display:inline-block;padding:12px 32px;font-family:${FONTS.sans};background-color:${COLORS.brass};color:#FFFFFF;text-decoration:none;border:1px solid ${COLORS.brassBorder};border-radius:7px;font-size:14px;font-weight:600;">View Details</a></td></tr></table>` : ''}
                  </td>
                </tr>
              </table>
            </td>
