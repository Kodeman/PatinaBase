...
export function renderProductCard(props: ProductCardProps, _ctx: RenderContext): string {
  const image = props.image_url
    ...
    : `<div style="height:260px;background:${COLORS.card};border-radius:8px 8px 0 0;"></div>`;
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid ${COLORS.line};border-radius:8px;overflow:hidden;">
                ...
                <tr>
                  <td style="padding:20px 24px;">
                    ...
                    <h3 style="margin:0 0 6px;font-family:${FONTS.serif};font-size:20px;font-weight:600;color:${COLORS.ink};">${esc(props.product_name)}</h3>
                    <p style="margin:0 0 12px;font-family:${FONTS.sans};font-size:14px;line-height:1.5;color:${COLORS.ink3};">${esc(props.description)}</p>
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td><span style="font-family:${FONTS.sans};font-size:18px;font-weight:600;color:${COLORS.ink};">${esc(props.price)}</span></td>
                        <td align="right"><span style="display:inline-block;padding:4px 12px;background:${COLORS.cardAlt};border:1px solid ${COLORS.line};border-radius:20px;font-family:${FONTS.sans};font-size:12px;font-weight:500;color:${COLORS.verd};">${esc(props.style_match)}</span></td>
                      </tr>
                    </table>
