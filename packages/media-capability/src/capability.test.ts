import { generateCapabilityKeyPair, mintCapability, verifyCapability } from './index';
import type { MediaCapabilityClaims } from './types';

function baseClaims(overrides: Partial<MediaCapabilityClaims> = {}): MediaCapabilityClaims {
  return {
    objectId: 'obj-0000',
    version: 1,
...
describe('@patina/media-capability', () => {
  ...
  it('rejects a structurally malformed token', async () => {
    ...
    const result = await verifyCapability(publicKeyPem, 'not-a-capability-token');
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error('unreachable');
