...
describe("tracing utilities", () => {
  ...
  describe("startRouteSpan", () => {
    it("should create span with HTTP attributes", () => {
      ...
      expect(attrs["http.route"]).toBeUndefined();
      expect(attrs["http.client_ip"]).toBeUndefined();
      expect(attrs["http.user_agent"]).toBeUndefined();
    });

    it("should not include customer identity or role attributes when authenticated", () => {
      ...
      expect(attrs["user.email_verified"]).toBeUndefined();
    });
  ...
  describe("startProxySpan", () => {
    it("should create proxy span with service attributes", () => {
      ...
      expect(attrs["http.method"]).toBe("GET");
      expect(attrs["http.url"]).toBeUndefined();
  ...
  describe("recordRetryAttempt", () => {
    ...
    it("should include only the error type when provided", () => {
      const span = createMockSpan("test");
      const error = new Error("Connection timeout");
      recordRetryAttempt(span, 2, error);

      const events = (span as any)._getEvents();
      expect(events[0].attributes["retry.error"]).toBeUndefined();
  ...
  describe("recordCacheEvent", () => {
    ...
    it("should not include a potentially identifying cache key", () => {
      const span = createMockSpan("test");
  ...
  describe("endSpanError", () => {
    it("should set error status and record exception", () => {
      ...
      expect(status.message).toBe("Request failed");
    ...
    it("should redact error messages in status", () => {
      const span = createMockSpan("test");
  ...
  describe("integration scenarios", () => {
    ...
    it("should trace failed request with circuit breaker", () => {
      ...
      expect(
        events.some((e: any) => e.name === "circuit_breaker_state_change"),
      ).toBe(true);
    });
