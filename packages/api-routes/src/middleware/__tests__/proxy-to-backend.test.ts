...
function mkRequest(
  url = "https://example.com/api/catalog/products",
  init: RequestInit = {},
) {
...
describe("proxyToBackend — happy path", () => {
  ...
  it("preserves query parameters when a fixed backend path is configured", async () => {
    const ctx = setAuthToken(mkContext(), "test-token-123");
    const req = mkRequest(
      "https://example.com/api/orders?status=paid&page=2&userId=attacker",
    );
    ...
    expect(mockFetch.mock.calls[0][0]).toBe(
      "http://localhost:3015/v1/orders?status=paid&page=2&userId=attacker",
    );
  });

  it("does not duplicate a query value explicitly set by the backend path", async () => {
    const ctx = setAuthToken(mkContext(), "test-token-123");
    const req = mkRequest("https://example.com/api/timeline?limit=25&page=2");
    ...
    await proxyToBackend(req, ctx, {
      service: {
        ...
        path: "/v1/projects/p1/timeline?limit=5",
      },
    });

    expect(mockFetch.mock.calls[0][0]).toBe(
      "http://localhost:3016/v1/projects/p1/timeline?limit=5&page=2",
    );
  });
});
...
describe("proxyToBackend — authentication", () => {
  ...
  it("A5: rejects with 401 when JWT verification fails", async () => {
    ...
    verifyJwtToken.mockRejectedValueOnce(
  ...
  it("decodes base64-prefixed Supabase cookies", async () => {
    ...
    const supabasePayload = JSON.stringify({
      access_token: "sb-token-from-b64",
    });
  ...
  it("ignores a stale foreign-project cookie and selects only the configured project", async () => {
    ...
    const configured = JSON.stringify({
      access_token: "configured-project-token",
    });
    cookiesGetAll.mockReturnValue([
  ...
  it("rebuilds forwarding headers from Cloudflare context and ignores caller spoofing", async () => {
    const ctx = setAuthToken(
      { ...mkContext(), ip: "198.51.100.99" },
      "valid-token",
    );
    const request = mkRequest(undefined, {
      headers: {
