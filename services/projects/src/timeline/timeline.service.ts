...
export class TimelineService {
  ...
  async createSegment(projectId: string, createDto: CreateTimelineSegmentDto, userId: string) {
    return this.authorization.withProjectAccess(userId, projectId, 'manage', async (tx) => {
      ...
      if (createDto.dependencies && createDto.dependencies.length > 0) {
        const existingSegments = await tx.timelineSegment.findMany({
          where: {
            projectId,
      ...
      const segment = await tx.timelineSegment.create({
        data: {
          projectId,
   ...
   */
  async getSegment(projectId: string, segmentId: string, userId: string) {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      ...
      if (
        ...
        Array.isArray(segment.dependencies) &&
        segment.dependencies.length > 0
      ) {
        dependentSegments = await tx.timelineSegment.findMany({
          where: {
            projectId,
  ...
  async updateSegment(
    ...
    return this.authorization.withProjectAccess(userId, projectId, 'manage', async (tx) => {
      const existing = await tx.timelineSegment.findFirst({
        where: {
          id: segmentId,
          projectId,
        },
      ...
      await tx.timelineSegment.updateMany({
      ...
      const updated = await tx.timelineSegment.findFirstOrThrow({
        where: { id: segmentId, projectId },
      });
  ...
  async logActivity(
    ...
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      ...
      await this.updateEngagementMetrics(projectId, logDto.activityType, tx, logDto.duration);
  ...
  async getUpcomingEvents(projectId: string, userId: string, daysAhead = 30) {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      ...
      const upcomingSegments = await tx.timelineSegment.findMany({
        where: {
          projectId,
          OR: [
            {
      ...
      const upcomingMilestones = await tx.milestone.findMany({
        where: {
          projectId,
          targetDate: {
            gte: now,
      ...
      const pendingApprovals = await tx.approvalRecord.findMany({
        where: {
          projectId,
          status: 'pending',
          dueDate: {
   ...
   */
  async getProgressMetrics(projectId: string, userId: string) {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      const project = await tx.project.findUnique({
        where: { id: projectId },
        select: {
      ...
      // Status distribution
      const statusDistribution = segments.reduce(
        (acc, seg) => {
  ...
  private async updateEngagementMetrics(
    ...
    await client.engagementMetrics.upsert({
      where: { projectId },
      create: {
        projectId,
        clientId: project.clientId,
   ...
   */
  async getImmersiveTimeline(projectId: string, userId: string): Promise<ImmersiveTimelineDto> {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      const project = await tx.project.findUnique({
        where: { id: projectId },
        select: {
      ...
      const allApprovals = await tx.approvalRecord.findMany({
        where: { projectId },
        select: { status: true },
      });
      ...
      const immersiveSegments: ImmersiveSegmentDto[] = segments.map((seg, idx) => {
        const segmentMilestone =
          milestones.find((m) => m.metadata && (m.metadata as any).segmentId === seg.id) ||
      ...
      const overdueApprovals = await tx.approvalRecord.count({
        where: {
          projectId,
          status: 'pending',
          dueDate: { lt: now },
      ...
      return {
        ...
        progress: {
          ...
          approvals: {
            ...
            percentage:
              allApprovals.length > 0 ? Math.round((approvedCount / allApprovals.length) * 100) : 0,
          },
        },
        ...
        nextMilestone: nextMilestone
          ? {
              ...
              daysUntil: Math.ceil(
                (nextMilestone.targetDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
              ),
            }
          : undefined,
        attentionRequired: {
  ...
  async getRecentCelebrations(
    ...
    userId: string,
    limit = 5,
  ): Promise<MilestoneCelebrationDto[]> {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      const project = await tx.project.findUnique({
        where: { id: projectId },
        select: { id: true },
   ...
   */
  async getSegmentMediaGallery(projectId: string, segmentId: string, userId: string) {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      const segment = await tx.timelineSegment.findFirst({
        where: { id: segmentId, projectId },
        select: {
      ...
      const milestone = await tx.milestone.findFirst({
        where: {
          projectId,
          OR: [{ metadata: { path: ['segmentId'], equals: segmentId } }],
        },
        select: { media: true },
      });
  ...
  async recordCelebrationViewed(projectId: string, milestoneId: string, userId: string) {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      const milestone = await tx.milestone.findFirst({
        where: { id: milestoneId, projectId, status: 'completed' },
        select: { id: true },
      });
      if (!milestone) {
        throw new NotFoundException('Completed milestone not found');
  ...
  private buildNarrative(segment: any): ImmersiveSegmentDto['narrative'] {
    ...
    if (!metadata?.narrative) {
      ...
      const completed =
        segment.deliverables?.filter((d: string) => metadata?.completedDeliverables?.includes(d)) ||
        [];

      const nextSteps =
        segment.deliverables?.filter(
          (d: string) => !metadata?.completedDeliverables?.includes(d),
        ) || [];
