...
export class ProgressAnalyticsService {
   ...
   */
  async getProjectProgress(projectId: string, userId: string): Promise<ProjectProgressSummaryDto> {
    return this.authorization.withProjectAccess(userId, projectId, 'read', async (tx) => {
      ...
      const estimatedEndDate =
        project.endDate || this.calculateEstimatedEndDate(project.timelineSegments);

      const daysElapsed = Math.floor((now.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
      const daysRemaining = Math.max(
        0,
        Math.floor((estimatedEndDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
      );
      ...
      const engagement = await this.getEngagementMetrics(projectId, tx);
  ...
  async getHealthIndicators(
    projectId: string,
    userId: string,
  ): Promise<ProjectHealthIndicatorDto[]> {
    ...
    const progressExpected =
      (progress.daysElapsed / (progress.daysElapsed + scheduleDaysLeft)) * 100;
    ...
    const pendingDays =
      progress.approvalMetrics.pendingList.length > 0
    ...
    indicators.push({
      ...
      message:
        progress.approvalMetrics.pendingApprovals === 0
          ? 'All approvals are up to date'
          : `${progress.approvalMetrics.pendingApprovals} approval(s) pending`,
      recommendation:
        pendingDays > 5 ? 'Follow up on pending approvals to avoid delays' : undefined,
    });
    ...
    indicators.push({
      ...
      recommendation:
        daysSinceVisit > 14 ? 'Reach out to client to maintain engagement' : undefined,
    });
  ...
  private async getMilestoneMetrics(
    projectId: string,
    milestones: any[],
  ): Promise<MilestoneMetricsDto> {
    const now = new Date();
    ...
    const onTimeCompletionRate =
      completed.length > 0 ? Math.round((onTimeCompleted.length / completed.length) * 100) : 100;
    ...
    const nextMilestone = sortedUpcoming[0]
      ? {
          ...
          daysUntil: Math.floor(
            (new Date(sortedUpcoming[0].targetDate).getTime() - now.getTime()) /
              (1000 * 60 * 60 * 24),
          ),
        }
      : undefined;
  ...
  private async getApprovalMetrics(
    ...
    const completedApprovals = approvals.filter(
      (a) => a.status !== 'pending' && (a.approvedAt || a.rejectedAt),
    );
    ...
    const approvalRate =
      totalDecided > 0 ? Math.round((approved.length / totalDecided) * 100) : 100;
  ...
  private async getEngagementMetrics(
    ...
    const viewLogs = await client.auditLog.findMany({
      where: {
        entityType: 'timeline_view',
        entityId: projectId,
    ...
    const celebrationLogs = await client.auditLog.count({
      where: {
    ...
    const galleryLogs = await client.auditLog.count({
      where: {
        entityType: 'media_gallery',
        action: 'opened',
        createdAt: { gte: thirtyDaysAgo },
  ...
  private calculateProjectHealth(
    ...
    const healthScore = Math.round(
      scheduleScore * 0.4 + milestoneScore * 0.3 + (approvalScore - pendingPenalty) * 0.3,
    );
