...
calls complete_agent_task with the right p_fatal (design §7). A second worker
with a different WORKER_ID claims disjoint tasks with zero coordination
(claim_agent_tasks uses FOR UPDATE SKIP LOCKED) — the burst-ready proof.
"""
...
def process_one(ctx: Context, task: dict[str, Any]) -> str:
    ...
    if handler is None:
        ctx.queue.complete_failed(
            task_id, f"no handler registered for {task_type}", fatal=True
        )
    ...
    try:
        ...
        ctx.queue.complete_failed(task_id, f"unexpected: {exc}", fatal=False)
        log.exception("task %s (%s): unexpected error", task_id, task_type)
        return "error"

    ctx.queue.complete_done(task_id, outcome.artifacts)
...
def run(settings: Settings, once: bool = False) -> dict[str, int]:
    ...
    tally = {"done": 0, "fatal": 0, "retry": 0, "error": 0, "no-handler": 0, "batches": 0}
    log.info(
