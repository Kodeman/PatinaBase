...
def _stage_verified_file(
    ...
    *,
    ...
    create-only object.  If that inode changes between upload chunks, the
    remote key can otherwise be committed with mixed bytes before the client
    notices.  ``frozen`` is an unlinked, mode-0600 temporary file owned only by
    this process; no network call occurs until its bytes and the live source
    metadata have both been verified.
    """
