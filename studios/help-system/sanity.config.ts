 ...
 */
function withPublishedAtStamp(PublishAction: DocumentActionComponent): DocumentActionComponent {
  const StampedPublishAction: DocumentActionComponent = (props: DocumentActionProps) => {
...
export default defineConfig({
  ...
  document: {
    actions: (prev, context) => {
      ...
      const canPublish = publisherIds.length > 0 && publisherIds.includes(context.currentUser?.id ?? '')

      return prev
        .filter((action) => canPublish || action.action !== 'publish')
        .map((action) => (action.action === 'publish' ? withPublishedAtStamp(action) : action))
    },
  },
})
